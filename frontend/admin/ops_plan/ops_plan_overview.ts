...
import {
  type OpsPlanStatusInput,
  OPS_PLAN_P1_DEFAULT_INPUTS,
  buildOpsPlanStatusViewModel
} from "./ops_plan_status.js";
...
function parsePayload(): OpsPlanOverviewPayload {
  ...
  const parsed = JSON.parse(payloadNode.textContent) as Partial<OpsPlanOverviewPayload>;
  return {
...
function renderEvidenceSummary(summary: OpsPlanOverviewPayload["evidenceSummary"]): string {
  return `
...
function renderReadonlyInteractionShell(cards: OpsPlanStatusInput[]): string {
...
export function renderOpsPlanOverview(root: HTMLElement, payload: OpsPlanOverviewPayload): void {
