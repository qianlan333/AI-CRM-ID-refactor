...
export const WORKSPACE_CANVAS_LANE_DEFINITIONS: WorkspaceCanvasLaneDefinition[] = [
...
function laneForEntity(entityType: WorkspaceEntityType): WorkspaceCanvasLaneId {
  if (entityType === "plan") return "plans";
  if (entityType === "group") return "groups";
  if (entityType === "node") return "nodes";
  if (entityType === "execution") return "executions";
  if (entityType === "push_center") return "push_center";
  return "evidence";
}
...
function fieldValue(detail: WorkspaceDetailItem | undefined, names: string[]): string {
  if (!detail) return "";
...
export function buildWorkspaceCanvasLanes(
  fixture: WorkspaceFixture,
  filtered: FilteredWorkspaceView,
