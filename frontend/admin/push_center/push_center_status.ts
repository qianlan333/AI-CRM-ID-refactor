...
export type PushCenterScenarioKey =
  | "group_ops"
  | "ops_plan_broadcast"
  | "wecom_auth"
  | "external_orders";
...
const PUSH_CENTER_STATUS_ALIASES: Record<string, EvidenceStatus> = {
  queued: "pending",
  running: "pending",
  scheduled: "pending",
  succeeded: "sent",
...
function normalizedRawStatus(rawStatus: string): string {
  return String(rawStatus || "").trim().toLowerCase().replace(/-/g, "_");
}

export function normalizePushCenterStatus(
  rawStatus: string,
...
export function toPushCenterScenario(input: PushCenterStatusInput): ScenarioEvidence {
  return {
    key: input.key,
    title: input.title,
...
export function buildPushCenterStatusViewModel(input: PushCenterStatusInput): PushCenterStatusViewModel {
...
export function operatorPromptForStatus(status: EvidenceStatus): string {
  if (status === "operator-action-required") return "需要运营处理后才能继续。";
  if (status === "retryable") return "任务可重试，但不能显示为完成。";
  if (status === "failed-terminal") return "终态失败，不能自动重试。";
  if (status === "downstream-pending") return "下游仍待处理，不能显示为已完成。";
  if (status === "governance-missing") return "真实发送已取证，但治理证据仍不完整。";
  if (status === "external-config-blocked") return "外部配置未生效，不能进入可执行队列。";
  if (status === "evidence-incomplete") return "证据仍不完整，需要继续补齐。";
  if (status === "blocked") return "当前阻塞，不能执行。";
  return "当前状态只读展示，不绑定真实执行。";
}

export function readonlyDropDoesNotMutate(input: PushCenterStatusInput): boolean {
...
export const PUSH_CENTER_P1_DEFAULT_INPUTS: PushCenterStatusInput[] = [
  {
    key: "group_ops",
...
export function defaultPushCenterViewModels(): PushCenterStatusViewModel[] {
