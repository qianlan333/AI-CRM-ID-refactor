...
EXTRA_SETTING_DEFINITIONS: dict[str, dict[str, Any]] = {
    ...
    "AICRM_WECOM_PROVIDER_TARGET_POLICY": {
        "key": "AICRM_WECOM_PROVIDER_TARGET_POLICY",
        "label": "企微 Provider 目标策略",
        "mode": "readonly",
        "input_type": "select",
        "type": "string",
        "options": ["blocked", "allowlisted_canary"],
        "description": "Provider 最后一层硬门禁；默认 blocked。仅 ID 验证运维流程可切换，任务还必须携带 CAS 授权记录并逐项命中白名单。",
    },
    ...
    "AICRM_EXTERNAL_EFFECT_ALLOWED_OWNER_USERIDS": {
        ...
        "label": "允许执行的企微 owner userid",
        "mode": "readonly",
        "input_type": "text",
        "type": "string",
        "description": "逗号或换行分隔；canary 的发送人/跟进人必须全部命中。未配置新默认 sender 时，第一个值仍兼容作为默认发送账号。",
    },
    ...
    "AICRM_EXTERNAL_EFFECT_ALLOWED_GROUP_CHAT_IDS": {
        ...
        "input_type": "textarea",
        "type": "string",
        "description": "逗号或换行分隔；任务中出现的每个 chat_id 都必须命中，不再使用 adapter 兜底放行。",
    },
    "AICRM_WECOM_CANARY_ALLOWED_MEDIA_TARGETS": {
        "key": "AICRM_WECOM_CANARY_ALLOWED_MEDIA_TARGETS",
        "label": "允许执行的企微媒体目标",
        "mode": "readonly",
        "input_type": "textarea",
        "type": "string",
        "description": "逗号或换行分隔，使用 target_type:target_id:media_type 精确值；媒体上传任务必须逐条命中。",
    },
