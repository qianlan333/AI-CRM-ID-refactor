...
export const DEFAULT_WORKSPACE_DRAFT_API_CONFIG = {
...
function sourcePlanId(fixture) {
    ...
    if (detailPlan)
        return text(detailPlan.id) || "p1_group_ops_workspace";
...
function detailForItem(fixture, entityType, detailId) {
    return fixture.detailItems.find((item) => item.entityType === entityType && item.id === detailId);
}
function itemPayload(detail, itemOrder, visibleInCurrentFilter) {
    return {
...
function selectedDraftItems(fixture, filtered, viewState) {
...
export function defaultWorkspaceDraftRequestJson() {
    ...
    return async function requestJsonWithFetch(url, options = {}) {
...
export async function listDrafts(config = DEFAULT_WORKSPACE_DRAFT_API_CONFIG, requestJson = defaultWorkspaceDraftRequestJson()) {
    return asDraftResponse(await requestJson(config.draftsUrl));
}
export async function getDraft(draftId, config = DEFAULT_WORKSPACE_DRAFT_API_CONFIG, requestJson = defaultWorkspaceDraftRequestJson()) {
    return asDraftResponse(await requestJson(`${config.draftsUrl}/${encodeURIComponent(draftId)}`));
}
export async function createDraft(payload, config = DEFAULT_WORKSPACE_DRAFT_API_CONFIG, requestJson = defaultWorkspaceDraftRequestJson()) {
    assertWorkspaceDraftPayloadSafe(payload);
    return asDraftResponse(await requestJson(config.draftsUrl, {
        method: "POST",
...
export async function updateDraft(draftId, payload, config = DEFAULT_WORKSPACE_DRAFT_API_CONFIG, requestJson = defaultWorkspaceDraftRequestJson()) {
    assertWorkspaceDraftPayloadSafe(payload);
    return asDraftResponse(await requestJson(`${config.draftsUrl}/${encodeURIComponent(draftId)}`, {
        method: "PATCH",
        body: JSON.stringify(payload)
...
export async function archiveDraft(draftId, version, config = DEFAULT_WORKSPACE_DRAFT_API_CONFIG, requestJson = defaultWorkspaceDraftRequestJson()) {
