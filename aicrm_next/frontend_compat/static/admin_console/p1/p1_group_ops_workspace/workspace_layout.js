...
import { approveGovernanceStep, buildApproveGovernanceStepPayload, buildExpireGovernanceReviewPayload, buildRejectGovernanceStepPayload, buildWorkspaceGovernanceRequestPayload, expireGovernanceReview, getDraftGovernance, getGovernanceReview, isGovernanceConflictError, rejectGovernanceStep, requestGovernance } from "./workspace_governance_api.js";
...
function copyStatusUpdate(result) {
    return {
...
function attachBundleExportHandlers(root, fixture, viewState) {
    if (typeof root.querySelectorAll !== "function")
        return;
    root.querySelectorAll("[data-workspace-toggle-copy-preview]").forEach((node) => {
        node.addEventListener("click", () => {
    ...
    root.querySelectorAll("[data-workspace-copy-bundle]").forEach((node) => {
        node.addEventListener("click", () => {
...
function governanceStepById(review, stepId) {
...
function attachDraftPersistenceHandlers(root, fixture, viewState) {
    if (typeof root.querySelectorAll !== "function")
        return;
    const renderWithDraftUpdates = (updates) => {
        renderP1GroupOpsWorkspace(root, fixture, updateWorkspaceViewState(viewState, updates));
    };
    root.querySelectorAll("[data-workspace-save-draft]").forEach((node) => {
        node.addEventListener("click", () => {
            const filtered = filterWorkspaceView(fixture, viewState);
    ...
    root.querySelectorAll("[data-workspace-save-as-new-draft]").forEach((node) => {
        node.addEventListener("click", () => {
            const filtered = filterWorkspaceView(fixture, viewState);
    ...
    root.querySelectorAll("[data-workspace-archive-draft]").forEach((node) => {
        node.addEventListener("click", () => {
            if (!viewState.currentDraftId || viewState.currentDraftVersion <= 0)
                return;
            const filtered = filterWorkspaceView(fixture, viewState);
            renderWithDraftUpdates({
                draftSaveStatus: "saving",
...
function attachGovernanceHandlers(root, fixture, viewState) {
    ...
    const refreshAfterConflict = (message) => {
    ...
    root.querySelectorAll("[data-workspace-governance-step-action]").forEach((node) => {
        node.addEventListener("click", () => {
    ...
    root.querySelectorAll("[data-workspace-expire-governance-review]").forEach((node) => {
        node.addEventListener("click", () => {
            const review = viewState.currentGovernanceReview;
            const reviewId = viewState.currentGovernanceReviewId || review?.review_id || "";
            if (!review || !reviewId) {
