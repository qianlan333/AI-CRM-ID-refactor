...
function renderGuardrailChips(guardrail) {
...
function selectedClass(card, viewState) {
...
function renderCanvasControls(viewState, lanes) {
    ...
    <section class="p1-workspace-canvas-controls" data-canvas-local-state="true" data-visible-lane-ids="${escapeHtml(viewState.visibleLaneIds.join(","))}">
      <label>
        ...
        <select data-workspace-filter="canvasSortMode">
          ${WORKSPACE_CANVAS_SORT_MODES.map((mode) => renderOption(mode, viewState.canvasSortMode)).join("")}
        </select>
      </label>
      <label>
        <span>Canvas group</span>
        <select data-workspace-filter="canvasGroupMode">
          ${WORKSPACE_CANVAS_GROUP_MODES.map((mode) => renderOption(mode, viewState.canvasGroupMode)).join("")}
        </select>
      </label>
      <label>
        <span>Density</span>
...
function renderCanvasLane(lane, viewState) {
...
export function renderGroupedWorkspaceCanvas(fixture, filtered, viewState) {
