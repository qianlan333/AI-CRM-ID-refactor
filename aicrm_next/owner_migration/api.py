...
from .application import OwnerMigrationCommand, build_owner_migration_service, clean_text, query_wecom_transfer_result
...
def _command_from_payload(payload: dict[str, Any], *, execute: bool) -> OwnerMigrationCommand:
    return OwnerMigrationCommand(
        ...
        transfer_success_msg=clean_text(payload.get("transfer_success_msg")),
        batch_size=_safe_int(payload.get("batch_size"), default=100),
        perform_wecom_transfer=_safe_bool(payload.get("perform_wecom_transfer"), default=True),
        execute=execute,
