...
from .guards import admin_auth_enforcement_enabled, admin_page_auth_redirect, current_admin_session
...
async def route_policy_required_response(
    ...
    *,
    ...
    if matched.static or matched.builtin or matched.route is None:
        return None
...
def _enforce_internal_bearer(request: Request) -> Response | None:
    expected = normalize_text(os.getenv("AUTOMATION_INTERNAL_API_TOKEN"))
    if not expected:
    ...
    request.state.service_account = "automation_internal"
    return None
...
def _rate_limit_allows(request: Request, policy: RoutePolicy) -> bool:
    client_host = normalize_text(getattr(request.client, "host", "")) or "unknown"
    return RATE_LIMITER.allow(profile=policy.rate_limit, principal=client_host, route_key=policy.key)
