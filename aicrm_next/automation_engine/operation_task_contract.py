...
def agent_material_present(config: dict[str, Any] | None) -> bool:
    ...
    return has_send_body(payload) or bool(
        text(payload.get("prompt"))
        or text(payload.get("task_prompt"))
        or text(payload.get("role_prompt"))
        or text(payload.get("user_prompt"))
        or text(payload.get("system_prompt"))
        or text(payload.get("material_prompt"))
    )
...
def agent_runtime_diagnostics(task: dict[str, Any]) -> dict[str, bool]:
    config = dict(task.get("agent_config_json") or {})
    ...
    return {
        ...
        "expected_send_body_present": bool(fallback_present or material_present),
    }


def publishable_diagnostics(task: dict[str, Any]) -> dict[str, Any]:
    ...
    if mode == "unified":
        ...
        if not agent_diag["expected_send_body_present"]:
            ...
            warnings.append("agent_fallback_or_material_required")
...
def validate_publishable_task(task: dict[str, Any]) -> dict[str, Any]:
    ...
    if diagnostics["errors"]:
        message_map = {
            ...
            "agent_runtime_content_missing": "agent 模式必须配置 fallback、requirement、description、提示词或素材",
        }
