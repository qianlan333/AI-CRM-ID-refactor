...
import tools.check_phase4av_workflow_nodes_metadata_plan as checker
...
def test_workflow_nodes_candidate_is_metadata_planning_only() -> None:
...
def test_previous_workflows_candidate_remains_paused() -> None:
    data = checker.load_yaml(PLAN_YAML)
    previous = data["previous_candidate"]
    assert previous["route_family"] == checker.WORKFLOWS
    assert previous["paused_by_pr"] == "#652"
...
def test_phase_4aw_recommendation_does_not_allow_production_work() -> None:
    data = checker.load_yaml(PLAN_YAML)
    rec = data["phase_4aw_recommendation"]
    assert rec["recommended_next_step"] == "workflow_nodes_schema_route_surface_confirmation"
