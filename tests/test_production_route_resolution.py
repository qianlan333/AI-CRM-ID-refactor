...
def test_high_risk_legacy_facade_routes_remain_production_compat_owned():
    ...
    assert _owner_for(samples, "POST", "/api/admin/automation-conversion/tasks/run-due") == "production_compat"
    assert _endpoint_for(samples, "POST", "/api/admin/automation-conversion/tasks/run-due") == "aicrm_next.production_compat.api"
    assert _owner_for(samples, "POST", "/api/admin/automation-conversion/execution-items/123/send-via-bazhuayu") == "production_compat"
    assert _endpoint_for(samples, "POST", "/api/admin/automation-conversion/execution-items/123/send-via-bazhuayu") == "aicrm_next.production_compat.api"
