...
import tools.check_phase4bq_agent_replay_metadata_plan as checker
...
def test_plan_is_agent_replay_metadata_readonly() -> None:
    data = checker.load_yaml(PLAN_YAML)
    assert data["status"] == "phase_4bq_agent_replay_metadata_planning_no_runtime_change"
    ...
    assert data["planning_scope"]["selected_subset"] == "agent_replay_metadata_readonly"
...
def test_phase_execution_state_advances_to_phase_4br() -> None:
    state = checker.load_yaml(STATE)
    assert state["active_candidate"] == checker.AGENT_REPLAY
    assert state["last_merged_pr"] == "#673"
    assert state["last_attempted_action"] == "phase_4bq_agent_replay_metadata_planning"
    assert state["last_created_pr"] == "#674"
    assert state["recommended_next_pr"] == "phase_4br_agent_replay_schema_route_surface_confirmation"
    assert state["owner_approval_required"] is False
    assert state["next_allowed_actions"] == ["phase_4br_agent_replay_schema_route_surface_confirmation"]
    assert "phase_4bq_agent_replay_metadata_planning_completed" in state["completed_steps"]


def test_agent_replay_readiness_is_planning_only_without_runtime_readiness() -> None:
...
def test_phase_4br_recommendation_does_not_allow_production_actions() -> None:
    rec = checker.load_yaml(PLAN_YAML)["phase_4br_recommendation"]
    assert rec["recommended_next_step"] == "agent_replay_schema_route_surface_confirmation"
