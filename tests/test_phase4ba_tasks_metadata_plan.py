...
import tools.check_phase4ba_tasks_metadata_plan as checker
...
def test_plan_is_tasks_metadata_only() -> None:
    data = checker.load_yaml(PLAN_YAML)
    assert data["status"] == "phase_4ba_tasks_metadata_planning_no_runtime_change"
    assert data["route_family"] == checker.TASKS
    ...
    assert data["planning_scope"]["selected_subset"] == "tasks_metadata_only"
    assert set(data["planning_scope"]["allowed_methods_for_future_contract_planning"]) == {"GET", "POST"}


def test_excluded_routes_and_behaviors_cover_execution_paths() -> None:
    data = checker.load_yaml(PLAN_YAML)
...
def test_tasks_readiness_is_planning_only_without_runtime_readiness() -> None:
...
def test_phase_4bb_recommendation_does_not_allow_production_actions() -> None:
    rec = checker.load_yaml(PLAN_YAML)["phase_4bb_recommendation"]
    assert rec["recommended_next_step"] == "tasks_schema_route_surface_confirmation"
