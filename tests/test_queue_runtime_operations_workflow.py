...
def test_identity_continuation_diagnostic_is_read_only_redacted_and_stage_specific() -> None:
    ...
    assert 'queue_id = positive_int(job["queue_id"])' in diagnostic
    assert 'job["business_type"] or "") == "identity_resolution_queue"' in diagnostic
    assert '"identity_scope_match": False' in diagnostic
