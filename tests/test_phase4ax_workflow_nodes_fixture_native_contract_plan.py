...
import tools.check_phase4ax_workflow_nodes_fixture_native_contract_plan as checker
...
def test_planned_fixture_routes_are_list_and_create_only() -> None:
    ...
    assert routes == {
        ("GET", "/api/admin/automation-conversion/workflow-nodes", "fixture_local_list"),
        ("POST", "/api/admin/automation-conversion/workflow-nodes", "fixture_local_metadata_create"),
    }
    excluded = {item["path"] for item in data["excluded_routes"]}
    assert "/api/admin/automation-conversion/workflow-nodes/{node_id}" in excluded
    assert "/api/admin/automation-conversion/workflows*" in excluded
    ...
    assert "/api/admin/automation-conversion/workflow-node-transitions*" in excluded
...
def test_fixture_seed_is_deterministic_and_non_production() -> None:
    ...
    assert {"phase4ax_welcome_node", "phase4ax_followup_node"} <= set(seed["node_codes"])
...
def test_contracts_include_idempotency_audit_rollback_and_safety() -> None:
    data = checker.load_yaml(PLAN_YAML)
    assert {"ok", "source_status", "route_owner", "workflow_nodes", "total", "count", "filters", "side_effect_safety"} <= set(data["list_contract"]["response_keys"])
    assert data["list_contract"]["disabled_nodes_included_by_default"] is True
    assert data["list_contract"]["ordering"] == "workflow_id_position_index_id_asc"
    assert {"workflow_id", "node_name", "idempotency_key"} <= set(data["create_contract"]["required_payload"])
    assert {"ok", "workflow_node", "audit_event", "rollback_payload", "idempotent_replay", "side_effect_safety"} <= set(data["create_contract"]["response_keys"])
    for field in ("missing_workflow_id_rejected", "missing_name_rejected", "duplicate_node_code_rejected", "invalid_target_audience_rejected", "invalid_trigger_mode_rejected", "dangerous_fields_rejected"):
...
def test_phase_4ay_recommendation_does_not_allow_production_work() -> None:
    data = checker.load_yaml(PLAN_YAML)
    rec = data["phase_4ay_recommendation"]
    assert rec["recommended_next_step"] == "workflow_nodes_fixture_native_implementation_owner_decision"
