...
import tools.check_phase4bo_agent_runs_fixture_native_contract_plan as checker
...
def test_plan_is_agent_runs_list_detail_fixture_only() -> None:
    data = checker.load_yaml(PLAN_YAML)
    assert data["status"] == "phase_4bo_agent_runs_fixture_native_contract_planning_no_runtime_change"
...
def test_fixture_seed_is_deterministic_and_local() -> None:
    ...
    assert {"phase4bo_run_completed_metadata", "phase4bo_run_failed_metadata"} <= set(seed["run_ids"])
...
def test_list_and_detail_contracts_are_complete() -> None:
    ...
    assert data["list_contract"]["ordering"] == "started_at_desc_id_desc"
    assert data["list_contract"]["output_payloads_excluded"] is True
    ...
    assert data["detail_contract"]["missing_run_rejected_without_side_effect"] is True
...
def test_visibility_and_side_effect_safety() -> None:
    ...
    assert visibility["run_metadata_not_production_evidence"] is True
    assert visibility["output_payloads_not_included"] is True
...
def test_agent_runs_readiness_requires_owner_decision_without_runtime_readiness() -> None:
...
def test_phase_4bp_recommendation_does_not_allow_production_actions() -> None:
    rec = checker.load_yaml(PLAN_YAML)["phase_4bp_recommendation"]
    assert rec["recommended_next_step"] == "agent_runs_fixture_native_implementation_owner_decision"
