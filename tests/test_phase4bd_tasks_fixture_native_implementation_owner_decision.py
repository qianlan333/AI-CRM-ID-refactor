...
import tools.check_phase4bd_tasks_fixture_native_implementation_owner_decision as checker
...
def test_tasks_paused_for_owner_decision() -> None:
    data = checker.load_yaml(PLAN_YAML)
    paused = data["paused_candidate"]
    assert paused["route_family"] == checker.TASKS
...
def test_next_candidate_selects_agents_metadata_planning() -> None:
    data = checker.load_yaml(PLAN_YAML)
    selected = data["next_candidate"]
    assert selected["selected_route_family"] == checker.AGENTS
    ...
    assert checker.REQUIRED_AGENT_GUARDRAILS <= set(selected["required_guardrails"])
    assert checker.REQUIRED_PHASE_4BE_SCOPE <= set(selected["phase_4be_scope"])
...
def test_tasks_readiness_paused_without_runtime_readiness() -> None:
...
def test_phase_4be_recommendation_does_not_allow_production_work() -> None:
    data = checker.load_yaml(PLAN_YAML)
    rec = data["phase_4be_recommendation"]
    assert rec["recommended_next_step"] == "agents_metadata_planning"
