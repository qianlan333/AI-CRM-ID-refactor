...
import tools.check_phase4ce_agents_repository_adapter_parity_bundle as checker
import tools.run_phase4ce_agents_adapter_parity as harness
from aicrm_next.automation_engine.repo import AGENT_TEST_DATABASE_URL_ENV, InMemoryAutomationRepository, build_automation_repository
from aicrm_next.automation_engine.agent_sqlalchemy_repository import SqlAlchemyAgentRepository
from aicrm_next.shared.errors import ContractError


ROOT = Path(__file__).resolve().parents[1]
DOC = ROOT / "docs/development/phase_4ce_agents_repository_adapter_parity_bundle.md"
PLAN_YAML = ROOT / "docs/development/phase_4ce_agents_repository_adapter_parity_bundle.yaml"
STATE = ROOT / "docs/development/phase_execution_state.yaml"
HARNESS = ROOT / "tools/run_phase4ce_agents_adapter_parity.py"


def _create_sqlite_agent_db(path: Path) -> str:
    ...
    with engine.begin() as conn:
        ...
        conn.execute(
            text(
                """
                CREATE TABLE automation_agent_idempotency (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        conn.execute(
            text(
                """
                CREATE TABLE automation_agent_audit_log (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    route_family TEXT NOT NULL,
                    operation TEXT NOT NULL,
                    operator TEXT NOT NULL,
        ...
        conn.execute(
            text(
                ...
                INSERT INTO automation_agents (
                    ...
                    agent_type, created_by, updated_by
                )
                VALUES (
                    1, 1, 1, 1, 'phase4ce_seed_agent', 'Phase 4CE seed agent',
...
def test_default_repository_remains_fixture(monkeypatch) -> None:
    monkeypatch.delenv("AICRM_AGENTS_REPO_BACKEND", raising=False)
...
def test_sqlalchemy_backend_requires_route_specific_url(monkeypatch) -> None:
    monkeypatch.setenv("AICRM_AGENTS_REPO_BACKEND", "sqlalchemy")
    monkeypatch.delenv("AICRM_AGENTS_TEST_DATABASE_URL", raising=False)
    monkeypatch.delenv("AICRM_AGENTS_STAGING_DATABASE_URL", raising=False)
    monkeypatch.setenv("DATABASE_URL", "sqlite+pysqlite:///tmp/should_not_be_used.db")
    with pytest.raises(ContractError, match="AICRM_AGENTS_TEST_DATABASE_URL"):
        build_automation_repository()


def test_sqlalchemy_repository_lists_creates_idempotently_and_audits(tmp_path: Path) -> None:
    db_url = _create_sqlite_agent_db(tmp_path / "phase4ce_local_test.db")
    repo = SqlAlchemyAgentRepository(create_engine(db_url, future=True))

    rows, total = repo.list_agents({"workflow_id": 1, "limit": 10, "offset": 0})
    assert total == 1
    assert rows[0]["agent_code"] == "phase4ce_seed_agent"
    ...
    payload = {
        ...
        "node_id": 2,
        "task_id": 2,
        "agent_name": "Adapter parity",
        "agent_code": "adapter_parity",
        "agent_type": "metadata",
        "operator": "pytest",
    }
    created = repo.create_agent(payload, idempotency_key="create-1", operator="pytest")
    replay = repo.create_agent(payload, idempotency_key="create-1", operator="pytest")
    audits = repo.list_agent_audit_events()

    assert created["agent"]["agent_code"] == "adapter_parity"
...
def test_sqlalchemy_idempotency_conflict_rejected(tmp_path: Path) -> None:
    db_url = _create_sqlite_agent_db(tmp_path / "phase4ce_conflict_local_test.db")
    repo = SqlAlchemyAgentRepository(create_engine(db_url, future=True))
    payload = {"program_id": 3, "workflow_id": 3, "node_id": 3, "agent_name": "Conflict", "agent_code": "conflict"}
    repo.create_agent(payload, idempotency_key="same-key", operator="pytest")
    with pytest.raises(ContractError, match="idempotency key conflicts"):
        repo.create_agent({**payload, "agent_name": "Conflict changed"}, idempotency_key="same-key", operator="pytest")


def test_harness_missing_env_returns_blocked_not_executed(monkeypatch) -> None:
    monkeypatch.delenv(AGENT_TEST_DATABASE_URL_ENV, raising=False)
...
def test_harness_refuses_production_looking_url(monkeypatch) -> None:
    monkeypatch.setenv(AGENT_TEST_DATABASE_URL_ENV, "postgresql://db.example/prod")
    report = harness.run_harness()
...
def test_harness_runs_against_safe_local_sqlite_url(monkeypatch, tmp_path: Path) -> None:
    db_url = _create_sqlite_agent_db(tmp_path / "phase4ce_harness_local_test.db")
    monkeypatch.setenv(AGENT_TEST_DATABASE_URL_ENV, db_url)
    report = harness.run_harness()
    assert report["ok"] is True
...
def test_yaml_records_adapter_safety_and_next_bundle() -> None:
    ...
    assert data["adapter"]["backend_flag"] == "AICRM_AGENTS_REPO_BACKEND"
    assert set(data["adapter"]["database_url_flags"]) == {"AICRM_AGENTS_TEST_DATABASE_URL", "AICRM_AGENTS_STAGING_DATABASE_URL"}
    ...
    assert data["safety"]["llm_generation_authorized"] is False
    assert data["safety"]["deepseek_adapter_authorized"] is False
    assert data["safety"]["openclaw_mcp_adapter_authorized"] is False
...
def test_phase_state_records_completed_adapter_parity() -> None:
    ...
    assert state["last_merged_pr"] == "#689"
    assert state["last_created_pr"] == "#690"
    assert checker.COMPLETED_STEP in state["completed_steps"]
    readiness = state["agents_readiness"]
