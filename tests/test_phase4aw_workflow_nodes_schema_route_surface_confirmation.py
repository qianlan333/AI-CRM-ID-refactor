...
import tools.check_phase4aw_workflow_nodes_schema_route_surface_confirmation as checker
...
def test_route_surface_is_legacy_forwarded_for_workflow_nodes_only() -> None:
    data = checker.load_yaml(PLAN_YAML)
    routes = data["confirmed_legacy_route_surface"]
    paths = {item["path"] for item in routes}
    assert paths == {"/api/admin/automation-conversion/workflow-nodes/{path:path}"}
    for item in routes:
        assert set(item["methods"]) == checker.REQUIRED_METHODS
        assert item["production_behavior"] == "legacy_forward"


def test_primary_workflow_node_table_columns_and_indexes_confirmed() -> None:
    data = checker.load_yaml(PLAN_YAML)
...
def test_phase_4ax_recommendation_does_not_allow_production_work() -> None:
    data = checker.load_yaml(PLAN_YAML)
    rec = data["phase_4ax_recommendation"]
    assert rec["recommended_next_step"] == "workflow_nodes_fixture_native_contract_planning"
