...
import tools.check_phase5ao_questionnaire_external_submit_family_acceptance as checker
...
def test_yaml_authorizations_false_and_stages_complete() -> None:
    ...
    assert {"phase_5ak_contract_fake_stub", "phase_5al_live_adapter_behind_flag", "phase_5am_staging_canary_evidence", "phase_5an_production_canary_readiness_tooling"} <= stages
...
def test_capability_matrix_and_acceptance_decision() -> None:
    ...
    assert matrix["production_public_submit_write_executed"] is False
    assert matrix["production_identity_write_executed"] is False
    assert matrix["production_tag_write_executed"] is False
...
def test_next_family_selected_and_business_continuity() -> None:
    data = checker.load_yaml(PLAN_YAML)
    assert data["next_family"]["selected_next_bundle"] == "phase_5_aggregate_acceptance_review_bundle"
