...
import tools.check_phase4cr_tasks_production_dry_run_readiness_bundle as checker


ROOT = Path(__file__).resolve().parents[1]
RUNNER = ROOT / "tools/run_phase4cr_tasks_production_readonly_dry_run.py"
PLAN = ROOT / "docs/development/phase_4cr_tasks_production_dry_run_readiness_bundle.yaml"
...
def run_runner(args: list[str] | None = None, env: dict[str, str] | None = None) -> dict:
    output = ROOT / ".tmp_phase4cr_runner_test.json"
    if output.exists():
        output.unlink()
...
def test_checker_passes_for_phase4cr_bundle() -> None:
...
def test_runner_missing_approval_returns_not_executed_missing_approval() -> None:
    data = run_runner(
        ["--read-only", "--confirm-no-writes"],
        {
            ...
            "AICRM_TASKS_READONLY_DRY_RUN_DATABASE_URL": "sqlite+pysqlite:////tmp/phase4cr_readonly.db",
        },
    )
    assert data["result_status"] == "not_executed_missing_approval"
    assert data["db_connected"] is False
...
def test_runner_does_not_fallback_to_database_url() -> None:
    data = run_runner(
        ["--read-only", "--confirm-no-writes"],
        {
            ...
            "AICRM_TASKS_TEST_DATABASE_URL": "sqlite+pysqlite:////tmp/forbidden_test.db",
            "AICRM_TASKS_STAGING_DATABASE_URL": "sqlite+pysqlite:////tmp/forbidden_stage.db",
        },
    )
    assert data["result_status"] == "not_executed_missing_database_url"
...
def test_runner_does_not_allow_write_execution_or_send() -> None:
    text = RUNNER.read_text(encoding="utf-8")
    for forbidden in (".create_task(", ".update_task(", ".delete_task(", ".run_due(", ".send(", "INSERT ", "UPDATE ", "DELETE "):
        assert forbidden not in text
    data = run_runner()
    safety = data["side_effect_safety"]
    assert safety["create_update_delete_executed"] is False
    assert safety["run_due_executed"] is False
    assert safety["task_execution_executed"] is False
    assert safety["workflow_execution_executed"] is False
    assert safety["outbound_send_executed"] is False


def test_yaml_authorizations_all_false() -> None:
    data = load_yaml(PLAN)
    assert data["route_family"] == "/api/admin/automation-conversion/tasks*"
