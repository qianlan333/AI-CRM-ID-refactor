...
def test_admin_automation_agent_crud_copy_pause_archive_contract(next_client, next_pg_schema, monkeypatch) -> None:
    ...
    created = next_client.post("/api/admin/automation-agents", json=_create_payload(), cookies=_admin_cookies())
    ...
    detail = next_client.get(f"/api/admin/automation-agents/{agent_id}", cookies=_admin_cookies())
    ...
    activated = next_client.post(f"/api/admin/automation-agents/{agent_id}/activate", cookies=_admin_cookies())
    assert activated.status_code == 200
    assert activated.json()["agent"]["status"] == "active"

    archived = next_client.delete(f"/api/admin/automation-agents/{agent_id}", cookies=_admin_cookies())
    assert archived.status_code == 200
    listed_after = next_client.get("/api/admin/automation-agents", cookies=_admin_cookies()).json()
...
def test_fixed_content_normalizes_and_rejects_non_pdf_attachment(next_client, next_pg_schema, monkeypatch) -> None:
    ...
    created = next_client.post("/api/admin/automation-agents", json=_create_payload(agent_code="fixed_agent"), cookies=_admin_cookies())
...
def test_admin_automation_agent_rows_are_soft_deleted(next_client, next_pg_schema, monkeypatch) -> None:
    ...
    created = next_client.post("/api/admin/automation-agents", json=_create_payload(agent_code="soft_delete_agent"), cookies=_admin_cookies())
    agent_id = created.json()["agent"]["id"]

    next_client.delete(f"/api/admin/automation-agents/{agent_id}", cookies=_admin_cookies())
