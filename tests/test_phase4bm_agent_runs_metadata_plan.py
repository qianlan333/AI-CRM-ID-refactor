...
import tools.check_phase4bm_agent_runs_metadata_plan as checker
...
def test_plan_is_agent_runs_metadata_readonly() -> None:
    data = checker.load_yaml(PLAN_YAML)
    assert data["status"] == "phase_4bm_agent_runs_metadata_planning_no_runtime_change"
    assert data["route_family"] == checker.AGENT_RUNS
    ...
    assert data["planning_scope"]["selected_subset"] == "agent_runs_metadata_readonly"
...
def test_agent_runs_readiness_is_planning_only_without_runtime_readiness() -> None:
...
def test_phase_4bn_recommendation_does_not_allow_production_actions() -> None:
    rec = checker.load_yaml(PLAN_YAML)["phase_4bn_recommendation"]
    assert rec["recommended_next_step"] == "agent_runs_schema_route_surface_confirmation"
