...
def test_channel_binding_large_import_requires_batch_confirmation(runtime_v2_pg_app):
    bind_channels_to_program = importlib.import_module("wecom_ability_" "service.domains.automation_conversion.channel_binding_service").bind_channels_to_program
    program_id = seed_program("runtime_v2_binding_guard")
    channel_id = seed_channel("runtime_v2_binding_guard_channel")
    seed_task(program_id, trigger_type="audience_entered", target_stage="operating", content_text="欢迎入池")
    for index in range(3):
        seed_contact(channel_id, f"wm_runtime_guard_{index:03d}", first_at="2026-01-01 08:00:00+08")

    result = bind_channels_to_program(
        program_id,
        [channel_id],
        {"binding_status": "active", "auto_enter_pool": True, "max_import_count": 2},
        operator_id="test",
    )

    assert result["history_imported"] is False
    assert result["requires_batch_import"] is True
    assert result["total_contact_count"] == 3
    assert result["imported_contact_count"] == 0
    assert result["generated_event_count"] == 0
    assert result["import_continue_token"]
    assert count("automation_event_v2") == 0
    assert count("automation_membership_v2") == 0
