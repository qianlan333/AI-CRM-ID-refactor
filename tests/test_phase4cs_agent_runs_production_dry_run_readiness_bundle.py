...
import tools.check_phase4cs_agent_runs_production_dry_run_readiness_bundle as checker


ROOT = Path(__file__).resolve().parents[1]
RUNNER = ROOT / "tools/run_phase4cs_agent_runs_production_readonly_dry_run.py"
PLAN = ROOT / "docs/development/phase_4cs_agent_runs_production_dry_run_readiness_bundle.yaml"
...
def run_runner(args: list[str] | None = None, env: dict[str, str] | None = None) -> dict:
    output = ROOT / ".tmp_phase4cs_runner_test.json"
    if output.exists():
        output.unlink()
...
def test_checker_passes_for_phase4cs_bundle() -> None:
...
def test_runner_missing_approval_returns_not_executed_missing_approval() -> None:
    data = run_runner(
        ["--read-only", "--confirm-no-writes"],
        {
            ...
            "AICRM_AGENT_RUNS_READONLY_DRY_RUN_DATABASE_URL": "sqlite+pysqlite:////tmp/phase4cs_readonly.db",
        },
    )
    assert data["result_status"] == "not_executed_missing_approval"
    assert data["db_connected"] is False
...
def test_runner_does_not_fallback_to_database_url() -> None:
    data = run_runner(
        ["--read-only", "--confirm-no-writes"],
        {
            ...
            "AICRM_AGENT_RUNS_TEST_DATABASE_URL": "sqlite+pysqlite:////tmp/forbidden_test.db",
            "AICRM_AGENT_RUNS_STAGING_DATABASE_URL": "sqlite+pysqlite:////tmp/forbidden_stage.db",
        },
    )
    assert data["result_status"] == "not_executed_missing_database_url"
...
def test_runner_does_not_allow_write_execution_send_replay_or_agent_run() -> None:
...
def test_yaml_authorizations_all_false() -> None:
    data = load_yaml(PLAN)
    assert data["route_family"] == "/api/admin/automation-conversion/agent-runs*"
...
def test_runner_blocked_evidence_records_execution_risk_flags() -> None:
    data = run_runner()
    assert data["agent_execution_triggered"] is False
    assert data["workflow_execution_triggered"] is False
    assert data["outbound_send_triggered"] is False
