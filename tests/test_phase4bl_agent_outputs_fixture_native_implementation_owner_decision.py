...
import tools.check_phase4bl_agent_outputs_fixture_native_implementation_owner_decision as checker
...
def test_agent_outputs_paused_for_owner_decision() -> None:
    data = checker.load_yaml(PLAN_YAML)
    paused = data["paused_candidate"]
    assert paused["route_family"] == checker.AGENT_OUTPUTS
...
def test_next_candidate_selects_agent_runs_metadata_planning() -> None:
    data = checker.load_yaml(PLAN_YAML)
    selected = data["next_candidate"]
    assert selected["selected_route_family"] == checker.AGENT_RUNS
    ...
    assert checker.REQUIRED_AGENT_RUNS_GUARDRAILS <= set(selected["required_guardrails"])
    assert checker.REQUIRED_PHASE_4BM_SCOPE <= set(selected["phase_4bm_scope"])
...
def test_agent_outputs_readiness_paused_without_runtime_readiness() -> None:
...
def test_phase_4bm_recommendation_does_not_allow_production_work() -> None:
    data = checker.load_yaml(PLAN_YAML)
    rec = data["phase_4bm_recommendation"]
    assert rec["recommended_next_step"] == "agent_runs_metadata_planning"
