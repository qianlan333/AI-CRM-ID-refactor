...
import tools.check_post_phase7_cleanup_workflow_nodes_evidence_refresh as checker
...
def test_workflow_nodes_route_family_and_no_behavior_change() -> None:
    data = checker.load_yaml(PLAN_YAML)
    assert data["route_family"] == "/api/admin/automation-conversion/workflow-nodes*"
    assert data["no_behavior_change"] is True


def test_evidence_refresh_records_missing_cleanup_evidence() -> None:
    evidence = checker.load_yaml(PLAN_YAML)["evidence"]
    assert evidence["phase6_owner_switch_tooling_accepted"] is True
    assert evidence["phase6e_internal_owner_switch_acceptance"] is True
    assert evidence["task_groups_blocked_comparison_reviewed"] is True
    assert evidence["owner_approval"] == "missing"
    assert evidence["latest_main_shadow_compare"] == "missing"
    assert evidence["rollback_execution_evidence"] == "missing"
    assert evidence["route_ownership_proof"] == "missing"
