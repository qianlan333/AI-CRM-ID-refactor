...
def test_phase_execution_state_advances_to_phase_4bf() -> None:
    ...
    assert state["last_merged_pr"] == "#661"
    assert state["last_attempted_action"] == "phase_4be_agents_metadata_planning"
    assert state["last_created_pr"] == "#662"
    assert state["recommended_next_pr"] == "phase_4bf_agents_schema_route_surface_confirmation"
    assert state["owner_approval_required"] is False
    assert state["next_allowed_actions"] == ["phase_4bf_agents_schema_route_surface_confirmation"]
