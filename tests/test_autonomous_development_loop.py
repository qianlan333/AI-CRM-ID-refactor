...
def test_phase_execution_state_fields_complete() -> None:
    ...
    assert data["current_phase"] == "phase_7g_first_exact_route_fallback_removal_canary"
    assert data["active_candidate"] == "task_groups_exact_route_fallback_cleanup_canary"
    assert data["capability_owner"] == "aicrm_next.automation_engine"
    assert data["last_merged_pr"] == "#785"
...
def test_next_allowed_actions_select_phase_7h_only() -> None:
    data = checker.load_yaml(STATE)
