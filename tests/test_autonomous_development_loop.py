...
def test_phase_execution_state_fields_complete() -> None:
    ...
    assert data["current_phase"] == "post_phase7_cleanup_task_groups_shadow_compare_rollback_evidence"
    assert data["active_candidate"] == "task_groups_shadow_compare_rollback_evidence"
    assert data["capability_owner"] == "aicrm_next.automation_engine"
    assert data["last_merged_pr"] == "#812"
