...
def test_phase_execution_state_fields_complete() -> None:
    ...
    assert data["current_phase"] == "phase_7j_legacy_runtime_cleanup_blocker_acceptance"
    assert data["active_candidate"] == "legacy_runtime_cleanup_blocker_acceptance"
    assert data["capability_owner"] == "aicrm_next.automation_engine"
    assert data["last_merged_pr"] == "#790"
...
def test_next_allowed_actions_select_phase_7k_only() -> None:
    data = checker.load_yaml(STATE)
