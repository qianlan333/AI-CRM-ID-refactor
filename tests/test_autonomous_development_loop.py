...
def test_phase_execution_state_fields_complete() -> None:
    ...
    assert data["current_phase"] == "phase_7b_baseline_legacy_import_remediation"
    assert data["active_candidate"] == "phase_7_baseline_legacy_import_remediation"
    assert data["capability_owner"] == "aicrm_next.automation_engine"
    assert data["last_merged_pr"] == "#774"
...
def test_completed_steps_include_phase_4al_readiness_gate() -> None:
    ...
    assert "phase_5w_media_upload_staging_live_canary_evidence_completed" in set(data["completed_steps"])
    ...
    assert "phase_5ak_questionnaire_external_submit_contract_fake_stub_completed" in set(data["completed_steps"])
    assert "phase_5al_questionnaire_external_submit_live_adapter_behind_flag_completed" in set(data["completed_steps"])
    assert "phase_5am_questionnaire_external_submit_staging_canary_evidence_completed" in set(data["completed_steps"])
    ...
    assert "phase_6e_internal_owner_switch_acceptance_completed" in set(data["completed_steps"])
    assert "phase_6f_external_adapter_enablement_readiness_completed" in set(data["completed_steps"])
    assert "phase_6g_low_risk_external_adapter_enablement_tooling_completed" in set(data["completed_steps"])
    assert "phase_6h_production_compat_exact_route_narrowing_readiness_completed" in set(data["completed_steps"])
...
def test_next_allowed_actions_select_phase_7c_only() -> None:
    data = checker.load_yaml(STATE)
