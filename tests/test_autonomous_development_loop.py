...
def test_phase_execution_state_fields_complete() -> None:
    ...
    assert data["current_phase"] == "post_phase7_cleanup_owner_evidence_collection"
    assert data["active_candidate"] == "cleanup_owner_evidence_collection"
    assert data["capability_owner"] == "aicrm_next.automation_engine"
    assert data["last_merged_pr"] == "#801"
