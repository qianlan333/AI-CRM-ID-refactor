...
def test_phase_execution_state_fields_complete() -> None:
    ...
    assert data["current_phase"] == "phase_7i_legacy_runtime_deletion_readiness"
    assert data["active_candidate"] == "legacy_runtime_deletion_readiness"
    assert data["capability_owner"] == "aicrm_next.automation_engine"
    assert data["last_merged_pr"] == "#789"
...
def test_next_allowed_actions_select_phase_7j_only() -> None:
    data = checker.load_yaml(STATE)
