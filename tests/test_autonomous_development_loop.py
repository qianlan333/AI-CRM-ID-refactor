...
def test_phase_execution_state_fields_complete() -> None:
    ...
    assert data["current_phase"] == "post_phase7_owner_feature_selection"
    assert data["active_candidate"] == "post_phase7_owner_feature_selection"
    assert data["capability_owner"] == "aicrm_next.send_content"
    assert data["last_merged_pr"] == "#795"
