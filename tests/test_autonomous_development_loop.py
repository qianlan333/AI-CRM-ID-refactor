...
def test_phase_execution_state_fields_complete() -> None:
    ...
    assert data["current_phase"] == "phase_7h_first_exact_route_production_compat_cleanup_canary"
    assert data["active_candidate"] == "task_groups_exact_route_production_compat_cleanup_canary"
    assert data["capability_owner"] == "aicrm_next.automation_engine"
    assert data["last_merged_pr"] == "#788"
...
def test_next_allowed_actions_select_phase_7i_only() -> None:
    data = checker.load_yaml(STATE)
