...
def test_phase_execution_state_fields_complete() -> None:
    ...
    assert data["current_phase"] == "phase_6h_production_compat_exact_route_narrowing_readiness"
    assert data["active_candidate"] == "production_compat_exact_route_narrowing_readiness"
    assert data["capability_owner"] == "aicrm_next.automation_engine"
    assert data["last_merged_pr"] == "#766"
