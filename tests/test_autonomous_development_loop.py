...
def test_phase_execution_state_fields_complete() -> None:
    ...
    assert data["current_phase"] == "phase_7k_final_route_ownership_manifest_cleanup"
    assert data["active_candidate"] == "final_route_ownership_manifest_cleanup"
    assert data["capability_owner"] == "aicrm_next.automation_engine"
    assert data["last_merged_pr"] == "#791"
...
def test_next_allowed_actions_select_phase_7l_only() -> None:
    data = checker.load_yaml(STATE)
