...
def test_phase_execution_state_fields_complete() -> None:
    ...
    assert data["active_candidate"] == "legacy_runtime_recheck_after_agent_outputs_exact_cleanup"
    assert data["capability_owner"] == "aicrm_next.automation_engine"
    assert data["last_merged_pr"] == "#820"
...
def test_agent_outputs_fixture_runtime_completed_with_production_readonly_readiness() -> None:
    data = checker.load_yaml(STATE)
    assert "/api/admin/automation-conversion/agent-outputs*" in {
        item["route_family"] for item in data["implemented_runtime_slices"]
    }
