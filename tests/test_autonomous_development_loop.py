...
def test_phase_execution_state_fields_complete() -> None:
    ...
    assert data["current_phase"] == "post_phase7_owner_approved_cleanup_track_activation"
    assert data["active_candidate"] == "post_phase7_owner_approved_cleanup_track"
    assert data["capability_owner"] == "aicrm_next.automation_engine"
    assert data["last_merged_pr"] == "#796"
