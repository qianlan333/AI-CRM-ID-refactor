...
def test_phase_execution_state_fields_complete() -> None:
    ...
    assert data["current_phase"] == "phase_7f_production_compat_cleanup_readiness"
    assert data["active_candidate"] == "phase_7_production_compat_cleanup_readiness"
    assert data["capability_owner"] == "aicrm_next.automation_engine"
    assert data["last_merged_pr"] == "#780"
...
def test_next_allowed_actions_select_phase_7g_only() -> None:
    data = checker.load_yaml(STATE)
