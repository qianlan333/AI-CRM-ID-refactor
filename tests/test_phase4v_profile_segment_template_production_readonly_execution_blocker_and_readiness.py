...
CHECKER = ROOT / "tools/check_phase4v_profile_segment_template_production_readonly_execution_blocker_and_readiness.py"
...
PLAN_MD = ROOT / "docs/development/phase_4v_profile_segment_template_production_readonly_execution_blocker_and_readiness.md"
...
def _load_yaml() -> dict:
    return _load_module(CHECKER, "phase4v_checker").load_yaml(PLAN_YAML)
...
def test_tool_returns_missing_approval(tmp_path: Path) -> None:
    env = _clean_env()
    ...
    assert data["blockers"]["missing_approval"] is True
    assert data["readiness"]["route_switch_ready"] is False


def test_tool_returns_config_not_reviewed(tmp_path: Path) -> None:
    env = _clean_env()
    ...
    assert data["execution"]["lower_runner_called"] is False
    assert data["blockers"]["missing_config_review"] is True


def test_tool_returns_missing_production_db(tmp_path: Path) -> None:
    env = _clean_env()
    env["AICRM_PHASE4R_PRODUCTION_READONLY_DRY_RUN_APPROVED"] = "1"
    env["AICRM_PHASE4R_PRODUCTION_CONFIG_REVIEWED"] = "1"
    env["AICRM_PROFILE_SEGMENT_TEMPLATE_REPO_BACKEND"] = "sqlalchemy"
    ...
    assert data["execution"]["lower_runner_called"] is False
    assert data["blockers"]["missing_production_db"] is True


def test_tool_refuses_without_read_only_no_write_args(tmp_path: Path) -> None:
    env = _clean_env()
    env["AICRM_PHASE4R_PRODUCTION_READONLY_DRY_RUN_APPROVED"] = "1"
    env["AICRM_PHASE4R_PRODUCTION_CONFIG_REVIEWED"] = "1"
    env["AICRM_PROFILE_SEGMENT_TEMPLATE_REPO_BACKEND"] = "sqlalchemy"
    ...
    assert data["execution"]["lower_runner_called"] is False
    assert data["blockers"]["missing_read_only_flags"] is True


def test_yaml_flags_writes_blockers_and_safety_fields() -> None:
    data = _load_yaml()
    for field in (
    ...
    assert data["blockers"]["missing_production_db"] is True
    assert data["blockers"]["missing_read_only_flags"] is True
    assert data["blockers"]["unblock_actions"]
