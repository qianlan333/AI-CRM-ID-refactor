...
import tools.check_phase4cq_workflow_nodes_production_dry_run_readiness_bundle as checker
import tools.run_phase4cq_workflow_nodes_production_readonly_dry_run as runner
...
def _clean_env() -> dict[str, str]:
    env = os.environ.copy()
    for key in (
        "AICRM_PHASE4CQ_PRODUCTION_READONLY_DRY_RUN_APPROVED",
        "AICRM_PHASE4CQ_PRODUCTION_CONFIG_REVIEWED",
        "AICRM_WORKFLOW_NODES_READONLY_DRY_RUN_DATABASE_URL",
        "AICRM_WORKFLOW_NODES_REPO_BACKEND",
        "DATABASE_URL",
...
def test_default_missing_approval_returns_blocked_evidence(monkeypatch) -> None:
    monkeypatch.delenv("AICRM_PHASE4CQ_PRODUCTION_READONLY_DRY_RUN_APPROVED", raising=False)
    monkeypatch.delenv("AICRM_PHASE4CQ_PRODUCTION_CONFIG_REVIEWED", raising=False)
    monkeypatch.delenv("AICRM_WORKFLOW_NODES_READONLY_DRY_RUN_DATABASE_URL", raising=False)
...
def test_runner_requires_config_db_backend_and_readonly_flags(tmp_path: Path) -> None:
    env = _clean_env()
    env["AICRM_PHASE4CQ_PRODUCTION_READONLY_DRY_RUN_APPROVED"] = "1"
    ...
    env["AICRM_PHASE4CQ_PRODUCTION_CONFIG_REVIEWED"] = "1"
    data = _run_tool(tmp_path, env, "--read-only", "--confirm-no-writes")
    assert data["execution"]["result_status"] == "not_executed_missing_dry_run_db"
    env["AICRM_WORKFLOW_NODES_READONLY_DRY_RUN_DATABASE_URL"] = "postgresql://user:secret@db/prod_aicrm"
...
def test_runner_does_not_use_database_url_test_or_staging_fallback(tmp_path: Path) -> None:
    ...
    env["AICRM_PHASE4CQ_PRODUCTION_READONLY_DRY_RUN_APPROVED"] = "1"
    env["AICRM_PHASE4CQ_PRODUCTION_CONFIG_REVIEWED"] = "1"
    env["AICRM_WORKFLOW_NODES_REPO_BACKEND"] = "sqlalchemy"
    env["DATABASE_URL"] = "postgresql://user:secret@db/prod_aicrm"
    env["AICRM_WORKFLOW_NODES_TEST_DATABASE_URL"] = "postgresql://user:secret@db/test_aicrm"
    env["AICRM_WORKFLOW_NODES_STAGING_DATABASE_URL"] = "postgresql://user:secret@db/stage_aicrm"
    data = _run_tool(tmp_path, env, "--read-only", "--confirm-no-writes")
    assert data["execution"]["result_status"] == "not_executed_missing_dry_run_db"
...
def test_runner_redacts_dry_run_db_url(tmp_path: Path) -> None:
    ...
    env["AICRM_PHASE4CQ_PRODUCTION_CONFIG_REVIEWED"] = "1"
    env["AICRM_WORKFLOW_NODES_REPO_BACKEND"] = "sqlalchemy"
    env["AICRM_WORKFLOW_NODES_READONLY_DRY_RUN_DATABASE_URL"] = "postgresql://user:topsecret@db/prod_aicrm"
    data = _run_tool(tmp_path, env)
    assert data["execution"]["result_status"] == "not_executed_read_only_flags_missing"
...
def test_runner_source_does_not_call_write_methods() -> None:
    ...
    assert ".create_workflow_node(" not in source
    assert ".insert(" not in source
    assert ".delete(" not in source


def test_yaml_and_phase_state_record_phase4cq_readiness() -> None:
