...
import tools.check_phase7f_production_compat_cleanup_readiness as checker
...
def test_selected_candidate_is_exact_route_readiness_only() -> None:
    data = checker.load_yaml(PLAN_YAML)
    selected = data["selected_first_production_compat_cleanup_candidate"]
    assert selected["candidate_id"] == checker.SELECTED_CANDIDATE
    assert selected["route_family"] == checker.SELECTED_ROUTE
    assert selected["production_compat_changed_in_phase_7f"] is False
    assert selected["fallback_removed_in_phase_7f"] is False
    assert selected["wildcard_cleanup_touched"] is False
    assert selected["runtime_deleted"] is False
    assert selected["delete_ready"] is False


def test_high_risk_production_compat_is_excluded() -> None:
    data = checker.load_yaml(PLAN_YAML)
    excluded = data["excluded_high_risk_production_compat"]
    for key in checker.REQUIRED_EXCLUDED_FLAGS:
        assert excluded[key] is True


def test_next_bundle_is_phase_7g_canary() -> None:
    data = checker.load_yaml(PLAN_YAML)
