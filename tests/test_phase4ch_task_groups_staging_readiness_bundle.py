...
import tools.check_phase4ch_task_groups_staging_readiness_bundle as checker
import tools.run_phase4ch_task_groups_staging_readiness as runner
...
def test_default_missing_staging_env_returns_blocked_evidence(monkeypatch) -> None:
    ...
    monkeypatch.delenv("AICRM_PHASE4CH_STAGING_SMOKE_APPROVED", raising=False)
...
def test_runner_refuses_production_looking_url(monkeypatch) -> None:
    monkeypatch.setenv("AICRM_TASK_GROUPS_STAGING_DATABASE_URL", "postgresql://db.example/prod")
    report = runner.run_preflight()
...
def test_runner_requires_backend_and_approval(monkeypatch) -> None:
    monkeypatch.setenv("AICRM_TASK_GROUPS_STAGING_DATABASE_URL", "sqlite+pysqlite:////tmp/phase4ch_stage.db")
    monkeypatch.delenv("AICRM_TASK_GROUPS_REPO_BACKEND", raising=False)
    report = runner.run_preflight()
...
def test_runner_write_request_requires_write_approval(monkeypatch) -> None:
    monkeypatch.setenv("AICRM_TASK_GROUPS_STAGING_DATABASE_URL", "sqlite+pysqlite:////tmp/phase4ch_stage.db")
    monkeypatch.setenv("AICRM_TASK_GROUPS_REPO_BACKEND", "sqlalchemy")
    monkeypatch.setenv("AICRM_PHASE4CH_STAGING_SMOKE_APPROVED", "1")
    monkeypatch.delenv("AICRM_PHASE4CH_STAGING_WRITE_APPROVED", raising=False)
...
def test_runner_never_uses_database_url_or_test_db_fallback() -> None:
    ...
    assert 'os.getenv("AICRM_TASK_GROUPS_TEST_DATABASE_URL"' not in text
    assert 'os.environ.get("AICRM_TASK_GROUPS_TEST_DATABASE_URL"' not in text


def test_yaml_records_staging_safety_and_next_bundle() -> None:
    data = checker.load_yaml(PLAN_YAML)
    assert data["bundle"]["type"] == "staging_readiness_bundle"
    assert data["staging_readiness"]["staging_database_url_flag"] == "AICRM_TASK_GROUPS_STAGING_DATABASE_URL"
...
def test_phase_state_records_completed_staging_readiness() -> None:
    ...
    assert state["last_merged_pr"] == "#693"
    assert state["last_created_pr"] == "#694"
    assert checker.COMPLETED_STEP in state["completed_steps"]
    assert any(item["route_family"] == checker.TASK_GROUPS for item in state["staging_readiness_slices"])
