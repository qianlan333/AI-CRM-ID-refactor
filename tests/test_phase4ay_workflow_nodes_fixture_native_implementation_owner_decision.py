...
import tools.check_phase4ay_workflow_nodes_fixture_native_implementation_owner_decision as checker
...
def test_workflow_nodes_paused_for_owner_decision() -> None:
    data = checker.load_yaml(PLAN_YAML)
    paused = data["paused_candidate"]
    assert paused["route_family"] == checker.ROUTE
...
def test_phase_4az_recommendation_does_not_allow_production_work() -> None:
    data = checker.load_yaml(PLAN_YAML)
    rec = data["phase_4az_recommendation"]
    assert rec["recommended_next_step"] == "select_next_low_risk_internal_write_candidate"
