...
import tools.check_phase5a_wecom_tag_adapter_contract as checker
import tools.run_phase5a_wecom_tag_adapter_contract_evidence as runner
...
def test_runner_does_not_execute_live_or_write_side_effects() -> None:
    evidence = runner.build_report()
    assert evidence["live_call_executed"] is False
    assert evidence["mark_tag_executed"] is False
    assert evidence["unmark_tag_executed"] is False
...
def test_adapter_methods_complete_and_no_live_call_allowed() -> None:
    data = checker.load_yaml(PLAN_YAML)
    methods = data["adapter_contract"]["methods"]
    assert checker.REQUIRED_METHODS == {item["name"] for item in methods}
    assert all(item["live_call_allowed"] is False for item in methods)
    dry_runs = {item["name"]: item for item in methods if item["name"].startswith("dry_run_")}
    assert dry_runs["dry_run_mark_tags"]["idempotency_required"] is True
    assert dry_runs["dry_run_unmark_tags"]["idempotency_required"] is True


def test_runner_source_has_no_live_network_or_secret_access() -> None:
    blockers = checker._runner_static_report()
    assert blockers == []


def test_changed_files_are_phase5a_allowed_if_git_diff_available() -> None:
    proc = subprocess.run(
