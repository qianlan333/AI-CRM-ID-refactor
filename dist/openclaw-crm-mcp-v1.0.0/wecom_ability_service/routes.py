...
<body>
  ...
  <script>
    ...
    function humanizeErrorMessage(rawMessage, fallback = '操作失败，请稍后重试') {
      ...
      if (message === 'question type must be single_choice, multi_choice or textarea') return '题型不正确，请重新选择题型';
    ...
    function createQuestion(type = 'single_choice', question = {}, index = 0) {
      ...
      return {
        ...
        options: normalizedType === 'textarea'
          ? []
    ...
    function renderQuestionInspector(question) {
      ...
      const optionsHtml = question.type === 'textarea'
        ? ''
      ...
      inspectorBodyEl.innerHTML = `
        ...
        <section class="config-group${question.type === 'textarea' ? ' hidden' : ''}" id="question-options-group">
          <div class="config-head">
            <div>
              <h3>选项列表</h3>
      ...
      inspectorBodyEl.querySelectorAll('.remove-option-btn').forEach((button) => {
        button.addEventListener('click', () => {
          ...
          if (!question.options.length && question.type !== 'textarea') {
            question.options = [createOption({}, 0)];
          }
          renderWorkspace();
        });
...
def questionnaire_h5_page(slug: str):
    ...
    elif not is_wechat_browser:
        env_notice = "当前为非微信环境，仅供测试。"
    page_state = {
        "slug": slug,
        "mode": page_mode,
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .questionnaire-head { margin-bottom: 18px; }
    .questionnaire-title { margin: 0 0 10px; font-size: 28px; line-height: 1.2; }
    .questionnaire-desc { margin: 0; color: var(--muted); white-space: pre-wrap; line-height: 1.7; }
    .question {
      margin: 0 0 18px;
      padding: 18px 16px;
      border-radius: 22px;
  ...
  <div class="page-shell">
    ...
    <section class="questionnaire-card">
      <div class="questionnaire-head">
        <h2 id="title" class="questionnaire-title">{{ page_state.title }}</h2>
        <p id="desc" class="questionnaire-desc">{{ page_state.description }}</p>
      </div>
