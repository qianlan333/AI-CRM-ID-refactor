...
from wecom_ability_service.db import close_db, init_app as init_db_app
from wecom_ability_service.infra.settings import DEFAULT_LAOHUANG_CHAT_WEBHOOK_URL, DEFAULT_OPENCLAW_WEBHOOK_URL
from wecom_ability_service.infra.task_queue import init_task_queue
from legacy_flask.legacy_lockdown import register_legacy_lockdown
from wecom_ability_service.http.internal_auth import register_admin_request_guards
from wecom_ability_service.mcp_adapter import mcp_bp
from wecom_ability_service.observability import attach_logging_filter, register_request_observability
from legacy_flask.routes import bp, favicon
...
def create_app(test_config: dict | None = None) -> Flask:
    legacy_assets_root = Path(__file__).resolve().parents[1] / "wecom_ability_service"
    app = Flask(
        __name__,
        instance_relative_config=False,
        template_folder=str(legacy_assets_root / "templates"),
        static_folder=str(legacy_assets_root / "static"),
    )
    ...
    app.config.from_mapping(
        ...
        LEGACY_SCHEMA_POSTGRES_PATH=str(legacy_assets_root / "schema_postgres.sql"),
    ...
    def _auto_migrate_schema() -> None:
        if not _schema_migrated[0]:
            ...
            from wecom_ability_service.db import init_db as _do_init_db
            try:
                _do_init_db()
            except Exception:
