...
DOC = ROOT / "docs/development/phase_4bq_agent_replay_metadata_plan.md"
PLAN_YAML = ROOT / "docs/development/phase_4bq_agent_replay_metadata_plan.yaml"
...
REQUIRED_INCLUDED_ROUTES = {
    ("GET", "/api/admin/automation-conversion/agent-replay", "replay_metadata_inventory_only"),
}
...
REQUIRED_CONTRACT_TRUE = {
    "metadata_contract_planning_ready",
    "replay_creation_excluded",
...
def build_report() -> dict[str, Any]:
    ...
    if data.get("status") != "phase_4bq_agent_replay_metadata_planning_no_runtime_change":
        blockers.append("status must be Phase 4BQ agent-replay metadata planning no runtime change")
    if data.get("route_family") != AGENT_REPLAY:
        blockers.append("route_family must be agent-replay")
    ...
    if previous.get("phase") != "phase_4bp_agent_runs_fixture_native_implementation_owner_decision" or previous.get("merged_pr") != "#673":
        blockers.append("previous_phase must record Phase 4BP merged as #673")
    ...
    if scope.get("selected_subset") != "agent_replay_metadata_readonly":
        blockers.append("planning_scope.selected_subset must be agent_replay_metadata_readonly")
    ...
    if not REQUIRED_INCLUDED_ROUTES <= included_routes:
        blockers.append("planning_scope.included_routes must cover replay metadata inventory")
    ...
    if state.get("active_candidate") != AGENT_REPLAY:
        blockers.append("phase_execution_state.active_candidate must remain agent-replay")
    if state.get("last_merged_pr") != "#673":
        blockers.append("phase_execution_state.last_merged_pr must record #673")
    if state.get("last_attempted_action") != "phase_4bq_agent_replay_metadata_planning":
        blockers.append("phase_execution_state.last_attempted_action must be Phase 4BQ")
    if state.get("last_created_pr") != "#674":
        blockers.append("phase_execution_state.last_created_pr must be #674")
    if state.get("recommended_next_pr") != "phase_4br_agent_replay_schema_route_surface_confirmation":
        blockers.append("phase_execution_state.recommended_next_pr must be Phase 4BR")
    if set(state.get("next_allowed_actions") or []) != {"phase_4br_agent_replay_schema_route_surface_confirmation"}:
        blockers.append("phase_execution_state.next_allowed_actions must be Phase 4BR")
    ...
    if state_update.get("phase_4bq_completed_step") not in set(state.get("completed_steps") or []):
        blockers.append("phase_execution_state.completed_steps must include Phase 4BQ completed step")
    ...
    rec = data.get("phase_4br_recommendation") if isinstance(data.get("phase_4br_recommendation"), dict) else {}
    if rec.get("recommended_next_step") != "agent_replay_schema_route_surface_confirmation":
        blockers.append("phase_4br_recommendation must recommend schema/route surface confirmation")
    for field in ("production_write_allowed", "production_route_switch_allowed", "fallback_removal_allowed", "production_write_canary_allowed"):
        if rec.get(field) is not False:
            blockers.append(f"phase_4br_recommendation.{field} must be false")
    ...
    if unexpected:
        blockers.append(f"unexpected changed files for Phase 4BQ package: {unexpected}")
...
def _write_outputs(report: dict[str, Any], output_json: str | None, output_md: str | None) -> None:
    ...
    if output_md:
        lines = [
            "# Phase 4BQ Agent Replay Metadata Planning Check",
            "",
