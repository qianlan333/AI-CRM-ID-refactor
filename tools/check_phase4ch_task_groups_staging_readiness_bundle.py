...
DOC = ROOT / "docs/development/phase_4ch_task_groups_staging_readiness_bundle.md"
PLAN_YAML = ROOT / "docs/development/phase_4ch_task_groups_staging_readiness_bundle.yaml"
STATE = ROOT / "docs/development/phase_execution_state.yaml"
RUNNER = ROOT / "tools/run_phase4ch_task_groups_staging_readiness.py"
TEST = ROOT / "tests/test_phase4ch_task_groups_staging_readiness_bundle.py"

TASK_GROUPS = "/api/admin/automation-conversion/task-groups*"
WORKFLOWS = "/api/admin/automation-conversion/workflows*"
NEXT_BUNDLE = "phase_4ci_workflows_staging_readiness_bundle"
COMPLETED_STEP = "phase_4ch_task_groups_staging_readiness_completed"
REQUIRED_CHANGED = {
...
def _contains_forbidden_env_fallback(text: str) -> bool:
    return any(
        ...
        for fragment in (
            ...
            'os.getenv("AICRM_TASK_GROUPS_TEST_DATABASE_URL"',
            'os.environ.get("AICRM_TASK_GROUPS_TEST_DATABASE_URL"',
        )
    )
...
def build_report() -> dict[str, Any]:
    ...
    if data.get("status") != "phase_4ch_task_groups_staging_readiness_bundle":
        blockers.append("status must be Phase 4CH task-groups staging readiness bundle")
    if bundle.get("type") != "staging_readiness_bundle" or bundle.get("route_family") != TASK_GROUPS:
        blockers.append("bundle must be staging_readiness_bundle for task-groups")
    ...
    expected_flags = {
        "runner_path": "tools/run_phase4ch_task_groups_staging_readiness.py",
        "staging_database_url_flag": "AICRM_TASK_GROUPS_STAGING_DATABASE_URL",
        "backend_flag": "AICRM_TASK_GROUPS_REPO_BACKEND",
        "approval_flag": "AICRM_PHASE4CH_STAGING_SMOKE_APPROVED",
        "write_approval_flag": "AICRM_PHASE4CH_STAGING_WRITE_APPROVED",
    }
    ...
    if "AICRM_TASK_GROUPS_TEST_DATABASE_URL" not in set(readiness.get("forbidden_database_url_fallbacks") or []):
    ...
    if _contains_forbidden_env_fallback(runner_text):
        blockers.append("runner must not fall back to DATABASE_URL or task-groups test DB URL")
    ...
    if state.get("active_candidate") != WORKFLOWS or state_update.get("active_candidate") != WORKFLOWS:
        blockers.append("phase state must advance active candidate to workflows staging readiness")
    if state.get("last_merged_pr") != "#693":
        blockers.append("phase state last_merged_pr must record #693")
    if state.get("last_attempted_action") != "phase_4ch_task_groups_staging_readiness_bundle":
        blockers.append("phase state last_attempted_action must be Phase 4CH")
    if state.get("last_created_pr") != "#694":
        blockers.append("phase state last_created_pr must be #694")
    if state.get("recommended_next_pr") != NEXT_BUNDLE:
        blockers.append("phase state recommended_next_pr must be Phase 4CI")
    if set(state.get("next_allowed_actions") or []) != {NEXT_BUNDLE}:
        blockers.append("phase state next_allowed_actions must be Phase 4CI")
    if COMPLETED_STEP not in set(state.get("completed_steps") or []):
        blockers.append("completed_steps must include Phase 4CH")

    top_level_staging = state.get("staging_readiness_slices") if isinstance(state.get("staging_readiness_slices"), list) else []
    if not any(isinstance(item, dict) and item.get("route_family") == TASK_GROUPS and item.get("slice") == "task_groups_staging_readiness_preflight" for item in top_level_staging):
    ...
    if task_groups.get("staging_smoke_executed") is not False:
        blockers.append("task_groups_readiness.staging_smoke_executed must be false")
    ...
    if unexpected:
        blockers.append(f"unexpected changed files for Phase 4CH: {unexpected}")
...
def _write_outputs(report: dict[str, Any], output_json: str | None, output_md: str | None) -> None:
    ...
    if output_md:
        lines = [
            "# Phase 4CH Task Groups Staging Readiness Bundle Check",
            "",
