...
from aicrm_next.automation_engine.repo import AGENT_RUN_TEST_DATABASE_URL_ENV, build_automation_repository
...
def run_harness() -> dict[str, Any]:
    db_url = str(os.getenv(AGENT_RUN_TEST_DATABASE_URL_ENV) or "").strip()
    if not db_url:
        return {
            "ok": True,
    ...
    details.append(
        {
            ...
            "status": "passed" if detail and detail["run_id"] == "phase4cg_run_completed_metadata" else "failed",
        }
    )
    missing = repo.get_agent_run("missing-run", {"visibility": "console"})
...
def _write_outputs(report: dict[str, Any], output_json: str | None, output_md: str | None) -> None:
    ...
    if output_md:
        lines = [
            "# Phase 4CG Agent Runs Adapter Parity Harness",
            "",
