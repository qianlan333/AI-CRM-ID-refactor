...
DOC = ROOT / "docs/development/phase_4bs_workflows_fixture_runtime.md"
...
DOMAIN = ROOT / "aicrm_next/automation_engine/workflows.py"
TEST = ROOT / "tests/test_phase4bs_workflows_fixture_runtime.py"
...
REQUIRED_RUNTIME_TERMS = {
    ...
    APP: [
        ...
        "_workflow_production_unavailable_payload",
    ],
    DTO: [
        "WorkflowListRequest",
    ...
    DOMAIN: [
        ...
        "workflow_projection",
        "workflow_side_effect_safety",
        "reject_dangerous_workflow_fields",
    ],
    TEST: [
        "test_fixture_repository_lists_seeded_workflows",
...
def build_report() -> dict[str, Any]:
    ...
    if state.get("active_candidate") != WORKFLOW_NODES:
        blockers.append("phase_execution_state.active_candidate must advance to workflow-nodes after workflows runtime slice")
    if state.get("last_merged_pr") != "#675":
        blockers.append("phase_execution_state.last_merged_pr must record #675")
    if state.get("last_attempted_action") != "phase_4bs_workflows_fixture_native_list_create_runtime":
        blockers.append("phase_execution_state.last_attempted_action must be Phase 4BS workflows runtime")
    if state.get("last_created_pr") != "#676":
        blockers.append("phase_execution_state.last_created_pr must be #676")
    if state.get("recommended_next_pr") != "phase_4bt_workflow_nodes_fixture_native_list_create_runtime":
        blockers.append("phase_execution_state.recommended_next_pr must advance to workflow-nodes fixture runtime")
    if set(state.get("next_allowed_actions") or []) != {"phase_4bt_workflow_nodes_fixture_native_list_create_runtime"}:
        blockers.append("phase_execution_state.next_allowed_actions must be Phase 4BT workflow-nodes runtime")
    if "phase_4bs_workflows_fixture_native_list_create_runtime_completed" not in set(state.get("completed_steps") or []):
        blockers.append("completed_steps must include Phase 4BS workflows runtime")
    ...
    if set(readiness.get("implemented_runtime_slices") or []) != {"workflows_fixture_local_list", "workflows_fixture_local_metadata_create"}:
        blockers.append("workflows_readiness.implemented_runtime_slices must include list/create")
    for field in (
    ...
    if unexpected:
        blockers.append(f"unexpected changed files for Phase 4BS workflows package: {unexpected}")
...
def _write_outputs(report: dict[str, Any], output_json: str | None, output_md: str | None) -> None:
    ...
    if output_md:
        lines = [
            "# Phase 4BS Workflows Fixture Runtime Check",
            "",
