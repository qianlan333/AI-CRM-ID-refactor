...
from tools.check_phase4bl_agent_outputs_fixture_native_implementation_owner_decision import load_yaml


DOC = ROOT / "docs/development/phase_4cm_agent_outputs_staging_readiness_bundle.md"
PLAN_YAML = ROOT / "docs/development/phase_4cm_agent_outputs_staging_readiness_bundle.yaml"
STATE = ROOT / "docs/development/phase_execution_state.yaml"
RUNNER = ROOT / "tools/run_phase4cm_agent_outputs_staging_readiness.py"
TEST = ROOT / "tests/test_phase4cm_agent_outputs_staging_readiness_bundle.py"

AGENT_OUTPUTS = "/api/admin/automation-conversion/agent-outputs*"
AGENT_RUNS = "/api/admin/automation-conversion/agent-runs*"
NEXT_BUNDLE = "phase_4cn_agent_runs_staging_readiness_bundle"
COMPLETED_STEP = "phase_4cm_agent_outputs_staging_readiness_completed"
REQUIRED_CHANGED = {
...
def _contains_forbidden_env_fallback(text: str) -> bool:
    return any(
        ...
        for fragment in (
            ...
            'os.getenv("AICRM_AGENT_OUTPUTS_TEST_DATABASE_URL"',
            'os.environ.get("AICRM_AGENT_OUTPUTS_TEST_DATABASE_URL"',
        )
    )
...
def build_report() -> dict[str, Any]:
    ...
    if data.get("status") != "phase_4cm_agent_outputs_staging_readiness_bundle":
        blockers.append("status must be Phase 4CM agent_outputs staging readiness bundle")
    if bundle.get("type") != "staging_readiness_bundle" or bundle.get("route_family") != AGENT_OUTPUTS:
        blockers.append("bundle must be staging_readiness_bundle for agent_outputs")
    ...
    expected_flags = {
        "runner_path": "tools/run_phase4cm_agent_outputs_staging_readiness.py",
        "staging_database_url_flag": "AICRM_AGENT_OUTPUTS_STAGING_DATABASE_URL",
        "backend_flag": "AICRM_AGENT_OUTPUTS_REPO_BACKEND",
        "approval_flag": "AICRM_PHASE4CM_STAGING_SMOKE_APPROVED",
        "write_approval_flag": "AICRM_PHASE4CM_STAGING_WRITE_APPROVED",
    }
    ...
    if "AICRM_AGENT_OUTPUTS_TEST_DATABASE_URL" not in set(readiness.get("forbidden_database_url_fallbacks") or []):
    ...
    if _contains_forbidden_env_fallback(runner_text):
        blockers.append("runner must not fall back to DATABASE_URL or agent_outputs test DB URL")
    ...
    if state.get("active_candidate") != AGENT_RUNS or state_update.get("active_candidate") != AGENT_RUNS:
        blockers.append("phase state must advance active candidate to agent-runs staging readiness")
    if state.get("last_merged_pr") != "#698":
        blockers.append("phase state last_merged_pr must record #698")
    if state.get("last_attempted_action") != "phase_4cm_agent_outputs_staging_readiness_bundle":
        blockers.append("phase state last_attempted_action must be Phase 4CM")
    if state.get("last_created_pr") != "#699":
        blockers.append("phase state last_created_pr must be #699")
    if state.get("recommended_next_pr") != NEXT_BUNDLE:
        blockers.append("phase state recommended_next_pr must be Phase 4CN")
    if set(state.get("next_allowed_actions") or []) != {NEXT_BUNDLE}:
        blockers.append("phase state next_allowed_actions must be Phase 4CN")
    if COMPLETED_STEP not in set(state.get("completed_steps") or []):
        blockers.append("completed_steps must include Phase 4CM")

    top_level_staging = state.get("staging_readiness_slices") if isinstance(state.get("staging_readiness_slices"), list) else []
    if not any(isinstance(item, dict) and item.get("route_family") == AGENT_OUTPUTS and item.get("slice") == "agent_outputs_staging_readiness_preflight" for item in top_level_staging):
        blockers.append("staging_readiness_slices must record agent_outputs staging readiness preflight")
    ...
    if agent_outputs.get("staging_smoke_executed") is not False:
        blockers.append("agent_outputs_readiness.staging_smoke_executed must be false")
    for field in (
        ...
        if agent_outputs.get(field) is not True:
            blockers.append(f"agent_outputs_readiness.{field} must remain true")
    for field in (
    ...
    if unexpected:
        blockers.append(f"unexpected changed files for Phase 4CM: {unexpected}")
    ...
    return {
        ...
        "blockers": blockers,
        "warnings": warnings,
        "details": {"changed_files": sorted(changed), "bundle_type": bundle.get("type"), "route_family": AGENT_OUTPUTS},
    }
...
def _write_outputs(report: dict[str, Any], output_json: str | None, output_md: str | None) -> None:
    ...
    if output_md:
        lines = [
            "# Phase 4CM Agent Outputs Staging Readiness Bundle Check",
            "",
