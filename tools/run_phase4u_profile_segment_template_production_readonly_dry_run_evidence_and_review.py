...
def _base_report(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        "ok": True,
        "status": "phase_4u_production_readonly_dry_run_evidence_and_review_no_route_switch",
        "timestamp": datetime.now(UTC).isoformat(),
        "command": "tools/run_phase4u_profile_segment_template_production_readonly_dry_run_evidence_and_review.py",
        "execution": {
            "result_status": "",
            "attempted": False,
...
def _set_blocked(report: dict[str, Any], status: str, reason: str) -> dict[str, Any]:
    ...
    report["skipped_details"] = [{"name": "phase4u_evidence_gate", "status": status, "reason": reason}]
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if os.environ.get(APPROVAL_ENV) != "1":
        return _set_blocked(report, "not_executed_missing_approval", f"{APPROVAL_ENV}=1 is required")
    if os.environ.get(CONFIG_REVIEW_ENV) != "1":
        return _set_blocked(report, "not_executed_config_not_reviewed", f"{CONFIG_REVIEW_ENV}=1 is required")
    if not os.environ.get(PRODUCTION_DB_ENV):
        return _set_blocked(report, "not_executed_missing_production_db", f"{PRODUCTION_DB_ENV} is required")
    if not args.read_only or not args.confirm_no_writes:
        return _set_blocked(report, "not_executed_read_only_flags_missing", "--read-only and --confirm-no-writes are required")
    if (os.environ.get(BACKEND_ENV) or "").strip().lower() != "sqlalchemy":
        return _set_blocked(report, "not_executed_safety_failed", f"{BACKEND_ENV}=sqlalchemy is required")
...
def _write_md(report: dict[str, Any], path: str) -> None:
    ...
    lines = [
        ...
        "",
        "## Readiness Reason",
        f"- {readiness.get('reason') or 'none'}",
        "",
        "## Blockers",
    ]
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Generate Phase 4U profile segment template read-only dry-run evidence and review.")
