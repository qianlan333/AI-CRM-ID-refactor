...
def build_report() -> dict[str, Any]:
    ...
    if state.get("active_candidate") not in {ROUTE, "/api/admin/automation-conversion/agent-runs*"}:
        blockers.append("phase_execution_state.active_candidate must be agent-outputs or later agent-runs")
    if state.get("last_merged_pr") not in {"#667", "#668"}:
        blockers.append("phase_execution_state.last_merged_pr must record #667 or later Phase 4BL state #668")
    if state.get("last_attempted_action") not in {
        ...
        blockers.append("phase_execution_state.last_attempted_action must be Phase 4BK or later Phase 4BL")
    if state.get("last_created_pr") not in {"#668", "#669"}:
        blockers.append("phase_execution_state.last_created_pr must be #668 or later Phase 4BL PR #669")
    if state.get("recommended_next_pr") not in {
        "phase_4bl_agent_outputs_fixture_native_implementation_owner_decision",
        "phase_4bm_agent_runs_metadata_planning",
    }:
        blockers.append("phase_execution_state.recommended_next_pr must be Phase 4BL or later Phase 4BM")
    if set(state.get("next_allowed_actions") or []) not in (
        {"phase_4bl_agent_outputs_fixture_native_implementation_owner_decision"},
        {"phase_4bm_agent_runs_metadata_planning"},
    ):
        blockers.append("phase_execution_state.next_allowed_actions must be Phase 4BL or later Phase 4BM")
