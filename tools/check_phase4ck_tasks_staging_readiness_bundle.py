...
from tools.check_phase4bd_tasks_fixture_native_implementation_owner_decision import load_yaml


DOC = ROOT / "docs/development/phase_4ck_tasks_staging_readiness_bundle.md"
PLAN_YAML = ROOT / "docs/development/phase_4ck_tasks_staging_readiness_bundle.yaml"
STATE = ROOT / "docs/development/phase_execution_state.yaml"
RUNNER = ROOT / "tools/run_phase4ck_tasks_staging_readiness.py"
TEST = ROOT / "tests/test_phase4ck_tasks_staging_readiness_bundle.py"

TASKS = "/api/admin/automation-conversion/tasks*"
AGENTS = "/api/admin/automation-conversion/agents*"
NEXT_BUNDLE = "phase_4cl_agents_staging_readiness_bundle"
COMPLETED_STEP = "phase_4ck_tasks_staging_readiness_completed"
REQUIRED_CHANGED = {
...
def _contains_forbidden_env_fallback(text: str) -> bool:
    return any(
        ...
        for fragment in (
            ...
            'os.getenv("AICRM_TASKS_TEST_DATABASE_URL"',
            'os.environ.get("AICRM_TASKS_TEST_DATABASE_URL"',
        )
    )
...
def build_report() -> dict[str, Any]:
    ...
    if data.get("status") != "phase_4ck_tasks_staging_readiness_bundle":
        blockers.append("status must be Phase 4CK tasks staging readiness bundle")
    if bundle.get("type") != "staging_readiness_bundle" or bundle.get("route_family") != TASKS:
        blockers.append("bundle must be staging_readiness_bundle for tasks")
    ...
    expected_flags = {
        "runner_path": "tools/run_phase4ck_tasks_staging_readiness.py",
        "staging_database_url_flag": "AICRM_TASKS_STAGING_DATABASE_URL",
        "backend_flag": "AICRM_TASKS_REPO_BACKEND",
        "approval_flag": "AICRM_PHASE4CK_STAGING_SMOKE_APPROVED",
        "write_approval_flag": "AICRM_PHASE4CK_STAGING_WRITE_APPROVED",
    }
    ...
    if "AICRM_TASKS_TEST_DATABASE_URL" not in set(readiness.get("forbidden_database_url_fallbacks") or []):
    ...
    if _contains_forbidden_env_fallback(runner_text):
        blockers.append("runner must not fall back to DATABASE_URL or tasks test DB URL")
    ...
    if state.get("active_candidate") != AGENTS or state_update.get("active_candidate") != AGENTS:
        blockers.append("phase state must advance active candidate to agents staging readiness")
    if state.get("last_merged_pr") != "#696":
        blockers.append("phase state last_merged_pr must record #696")
    if state.get("last_attempted_action") != "phase_4ck_tasks_staging_readiness_bundle":
        blockers.append("phase state last_attempted_action must be Phase 4CK")
    if state.get("last_created_pr") != "#697":
        blockers.append("phase state last_created_pr must be #697")
    if state.get("recommended_next_pr") != NEXT_BUNDLE:
        blockers.append("phase state recommended_next_pr must be Phase 4CL")
    if set(state.get("next_allowed_actions") or []) != {NEXT_BUNDLE}:
        blockers.append("phase state next_allowed_actions must be Phase 4CL")
    if COMPLETED_STEP not in set(state.get("completed_steps") or []):
        blockers.append("completed_steps must include Phase 4CK")

    top_level_staging = state.get("staging_readiness_slices") if isinstance(state.get("staging_readiness_slices"), list) else []
    if not any(isinstance(item, dict) and item.get("route_family") == TASKS and item.get("slice") == "tasks_staging_readiness_preflight" for item in top_level_staging):
        blockers.append("staging_readiness_slices must record tasks staging readiness preflight")
    tasks = state.get("tasks_readiness") if isinstance(state.get("tasks_readiness"), dict) else {}
    for field in (
        ...
        if tasks.get(field) is not True:
            blockers.append(f"tasks_readiness.{field} must be true")
    if tasks.get("staging_smoke_executed") is not False:
        blockers.append("tasks_readiness.staging_smoke_executed must be false")
    for field in ("production_owner_switch_ready", "production_write_ready", "fallback_removal_ready", "production_repository_route_enablement_ready", "delete_ready"):
        if tasks.get(field) is not False:
    ...
    if unexpected:
        blockers.append(f"unexpected changed files for Phase 4CK: {unexpected}")
...
def _write_outputs(report: dict[str, Any], output_json: str | None, output_md: str | None) -> None:
    ...
    if output_md:
        lines = [
            "# Phase 4CK Tasks Staging Readiness Bundle Check",
            "",
