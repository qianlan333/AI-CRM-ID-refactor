...
DOC = ROOT / "docs/development/phase_4cs_agent_runs_production_dry_run_readiness_bundle.md"
PLAN_YAML = ROOT / "docs/development/phase_4cs_agent_runs_production_dry_run_readiness_bundle.yaml"
STATE = ROOT / "docs/development/phase_execution_state.yaml"
RUNNER = ROOT / "tools/run_phase4cs_agent_runs_production_readonly_dry_run.py"
TEST = ROOT / "tests/test_phase4cs_agent_runs_production_dry_run_readiness_bundle.py"
AGENT_RUNS = "/api/admin/automation-conversion/agent-runs*"
AGENT_OUTPUTS = "/api/admin/automation-conversion/agent-outputs*"
COMPLETED_STEP = "phase_4cs_agent_runs_production_dry_run_readiness_completed"
NEXT_BUNDLE = "phase_4ct_agent_outputs_production_dry_run_readiness_bundle"
REQUIRED_ENV = {
...
def build_report() -> dict[str, Any]:
    ...
    if data.get("status") != "phase_4cs_agent_runs_production_readonly_dry_run_readiness":
        blockers.append("status must be phase_4cs_agent_runs_production_readonly_dry_run_readiness")
    ...
    if data.get("route_family") != AGENT_RUNS:
        blockers.append("route_family must be /api/admin/automation-conversion/agent-runs*")
    ...
    if set(authorizations) != expected_authorizations:
        blockers.append("authorizations keys must match the Phase 4CS safety contract")

    runner = data.get("runner") if isinstance(data.get("runner"), dict) else {}
    if runner.get("path") != "tools/run_phase4cs_agent_runs_production_readonly_dry_run.py":
    ...
    if set(runner.get("required_env") or []) != REQUIRED_ENV:
        blockers.append("runner.required_env must contain the Phase 4CS approval/config/backend/DB envs")
    ...
    if next_bundle.get("recommended_next_step") != NEXT_BUNDLE or next_bundle.get("route_family") != AGENT_OUTPUTS:
        blockers.append("next_bundle must point to Phase 4CT agent-outputs production dry-run readiness")

    _check_runner_static(runner_text, blockers)

    if state.get("last_merged_pr") != "#707":
        blockers.append("phase state last_merged_pr must record #707")
    if state.get("last_attempted_action") != "phase_4cs_agent_runs_production_dry_run_readiness_bundle":
        blockers.append("phase state last_attempted_action must be Phase 4CS")
    if state.get("last_created_pr") != "#708":
        blockers.append("phase state last_created_pr must be #708")
    if state.get("active_candidate") != AGENT_OUTPUTS:
        blockers.append("phase state active_candidate must advance to agent-outputs")
    if state.get("recommended_next_pr") != NEXT_BUNDLE or set(state.get("next_allowed_actions") or []) != {NEXT_BUNDLE}:
        blockers.append("phase state next action must advance to Phase 4CT agent-outputs production dry-run readiness")
    if COMPLETED_STEP not in set(state.get("completed_steps") or []):
        blockers.append("completed_steps must include Phase 4CS")
    ...
    if readiness.get("production_readonly_db_url_flag") != "AICRM_AGENT_RUNS_READONLY_DRY_RUN_DATABASE_URL":
    ...
    if readiness.get("production_readonly_approval_flag") != "AICRM_PHASE4CS_PRODUCTION_READONLY_DRY_RUN_APPROVED":
    ...
    if readiness.get("production_readonly_config_review_flag") != "AICRM_PHASE4CS_PRODUCTION_CONFIG_REVIEWED":
    ...
    if unexpected:
        blockers.append(f"unexpected changed files for Phase 4CS: {unexpected}")
    ...
    return {
        ...
        "blockers": blockers,
        "warnings": warnings,
        "details": {"changed_files": sorted(changed), "bundle_type": data.get("bundle_type"), "route_family": AGENT_RUNS},
    }
...
def _write_outputs(report: dict[str, Any], output_json: str | None, output_md: str | None) -> None:
    ...
    if output_md:
        lines = [
            "# Phase 4CS Agent Runs Production Dry-Run Readiness Bundle Check",
            "",
