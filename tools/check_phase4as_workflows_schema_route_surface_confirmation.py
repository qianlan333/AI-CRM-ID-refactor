...
MAIN_TABLE = "automation_workflow"
...
BOUNDARY_TRUE_FIELDS = {
    ...
    "workflow_node_runtime_deferred",
    "run_due_deferred",
...
def build_report() -> dict[str, Any]:
    ...
    if data.get("status") != "phase_4as_workflows_schema_route_surface_confirmation_no_runtime_change":
        blockers.append("status must be Phase 4AS workflows schema route surface confirmation no runtime change")
    ...
    if "@router.api_route(\"/api/admin/automation-conversion/workflows\"" not in compat_text:
        blockers.append("production_compat must still register workflows legacy route")
    if "@router.api_route(\"/api/admin/automation-conversion/workflows/{path:path}\"" not in compat_text:
        blockers.append("production_compat must still register workflows wildcard legacy route")

    previous = data.get("previous_phase") if isinstance(data.get("previous_phase"), dict) else {}
    if previous.get("merged_pr") != "#649" or previous.get("completed") is not True:
        blockers.append("previous_phase must record Phase 4AR merged in #649")
    ...
    if paths != {"/api/admin/automation-conversion/workflows", "/api/admin/automation-conversion/workflows/{path:path}"}:
        blockers.append("confirmed_legacy_route_surface must include exact and wildcard workflows routes")
    ...
    if table.get("name") != MAIN_TABLE:
        blockers.append("primary_table.name must be automation_workflow")
    if f"CREATE TABLE IF NOT EXISTS {MAIN_TABLE}" not in schema_text:
        blockers.append("schema_postgres.sql must define automation_workflow")
    ...
    for item in sorted(REQUIRED_COLUMNS | REQUIRED_INDEXES | DEFERRED_TABLES):
        if item not in schema_text:
            blockers.append(f"schema_postgres.sql missing referenced workflow artifact: {item}")
    ...
    for field in ("active_candidate", "last_merged_pr", "last_attempted_action", "recommended_next_pr", "owner_approval_required"):
        if state.get(field) != state_update.get(field):
            blockers.append(f"phase_execution_state.{field} must match Phase 4AS plan")
    if state_update.get("phase_4as_completed_step") not in set(state.get("completed_steps") or []):
        blockers.append("phase_execution_state.completed_steps must include Phase 4AS completed step")
    if set(state.get("next_allowed_actions") or []) != {"phase_4at_workflows_fixture_native_contract_planning"}:
        blockers.append("next_allowed_actions must advance to Phase 4AT fixture/native contract planning")
    ...
    if readiness.get("schema_route_surface_confirmed") is not True or readiness.get("fixture_native_contract_planning_ready") is not True:
        blockers.append("workflows_readiness must confirm schema surface and mark fixture planning ready")
    ...
    rec = data.get("phase_4at_recommendation") if isinstance(data.get("phase_4at_recommendation"), dict) else {}
    if rec.get("recommended_next_step") != "workflows_fixture_native_contract_planning":
        blockers.append("phase_4at_recommendation must recommend workflows fixture/native contract planning")
    for field in ("production_write_allowed", "production_route_switch_allowed", "fallback_removal_allowed", "production_write_canary_allowed"):
        if rec.get(field) is not False:
            blockers.append(f"phase_4at_recommendation.{field} must be false")
    ...
    if unexpected:
        blockers.append(f"unexpected changed files for Phase 4AS package: {unexpected}")
...
def _write_outputs(report: dict[str, Any], output_json: str | None, output_md: str | None) -> None:
    ...
    if output_md:
        lines = ["# Phase 4AS Workflows Schema Route Surface Check", "", f"- overall: {report['overall']}", f"- ok: {str(report['ok']).lower()}", "", "## Blockers", *(f"- {item}" for item in report["blockers"]), "", "## Warnings", *(f"- {item}" for item in report["warnings"])]
