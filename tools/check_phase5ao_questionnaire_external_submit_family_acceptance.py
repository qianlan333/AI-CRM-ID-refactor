...
DOC = ROOT / "docs/development/phase_5ao_questionnaire_external_submit_family_acceptance.md"
PLAN_YAML = ROOT / "docs/development/phase_5ao_questionnaire_external_submit_family_acceptance.yaml"
STATE = ROOT / "docs/development/phase_execution_state.yaml"
TEST = ROOT / "tests/test_phase5ao_questionnaire_external_submit_family_acceptance.py"
NEXT_BUNDLE = "phase_5_aggregate_acceptance_review_bundle"
COMPLETED_STEP = "phase_5ao_questionnaire_external_submit_family_acceptance_completed"
ALLOWED = {
...
def build_report() -> dict[str, Any]:
    ...
    required = {"phase_5ak_contract_fake_stub", "phase_5al_live_adapter_behind_flag", "phase_5am_staging_canary_evidence", "phase_5an_production_canary_readiness_tooling"}
    if not required <= stages:
        blockers.append("completed_stages missing questionnaire stages")
    ...
    for key in ("production_public_submit_write_executed", "production_identity_write_executed", "production_tag_write_executed", "live_oauth_callback_cutover_executed", "outbound_send_executed", "batch_submit_enabled", "batch_tag_write_enabled", "route_owner_switched", "fallback_removed", "production_compat_changed"):
    ...
    for key in ("wider_rollout_authorized", "production_public_submit_owner_switch_authorized", "production_write_default_authorized", "batch_submit_authorized", "batch_tag_write_authorized", "outbound_send_authorized", "delete_ready"):
    ...
    if state.get("last_merged_pr") != "#753":
        blockers.append("phase_execution_state.last_merged_pr must be #753")
    ...
    if unexpected:
        blockers.append(f"changed files outside Phase 5AO allowlist: {unexpected}")
