...
DOC = ROOT / "docs/development/phase_4ax_workflow_nodes_fixture_native_contract_plan.md"
PLAN_YAML = ROOT / "docs/development/phase_4ax_workflow_nodes_fixture_native_contract_plan.yaml"
...
SIDE_EFFECT_FALSE_FIELDS = {
    ...
    "node_transition_runtime_allowed",
...
def build_report() -> dict[str, Any]:
    ...
    if data.get("status") != "phase_4ax_workflow_nodes_fixture_native_contract_planning_no_runtime_change":
        blockers.append("status must be phase_4ax_workflow_nodes_fixture_native_contract_planning_no_runtime_change")
    ...
    if previous.get("merged_pr") != "#654" or previous.get("completed") is not True:
        blockers.append("previous_phase must record Phase 4AW merged in #654")
    ...
    required_excluded = {
        "/api/admin/automation-conversion/workflow-nodes/{node_id}",
    ...
    if not required_excluded <= excluded_paths:
        blockers.append("excluded_routes must include update/delete, workflows, tasks, run-due, executions, and transitions")
    ...
    if not {"phase4ax_welcome_node", "phase4ax_followup_node"} <= set(seed.get("node_codes") or []):
        blockers.append("fixture_seed.node_codes must include deterministic Phase 4AX seed codes")
    ...
    required_list_keys = {"ok", "source_status", "route_owner", "workflow_nodes", "total", "count", "filters", "side_effect_safety"}
    if not required_list_keys <= set(list_contract.get("response_keys") or []):
        blockers.append("list_contract.response_keys incomplete")
    if list_contract.get("disabled_nodes_included_by_default") is not True:
        blockers.append("list_contract must include disabled nodes by default")
    if list_contract.get("ordering") != "workflow_id_position_index_id_asc":
        blockers.append("list_contract ordering must be workflow_id_position_index_id_asc")

    create_contract = data.get("create_contract") if isinstance(data.get("create_contract"), dict) else {}
    if not {"workflow_id", "node_name", "idempotency_key"} <= set(create_contract.get("required_payload") or []):
        blockers.append("create_contract must require workflow_id, node_name, and idempotency_key")
    required_create_keys = {"ok", "workflow_node", "audit_event", "rollback_payload", "idempotent_replay", "side_effect_safety"}
    ...
    for field in (
        "missing_workflow_id_rejected",
        "missing_name_rejected",
        "duplicate_node_code_rejected",
        "invalid_target_audience_rejected",
        "invalid_trigger_mode_rejected",
    ...
    if state.get("last_merged_pr") != "#654":
        blockers.append("phase_execution_state.last_merged_pr must record #654")
    if state_update.get("phase_4ax_completed_step") not in set(state.get("completed_steps") or []):
        blockers.append("phase_execution_state.completed_steps must include Phase 4AX completed step")
    readiness = state.get("workflow_nodes_readiness") if isinstance(state.get("workflow_nodes_readiness"), dict) else {}
    for field in ("fixture_native_contract_planning_ready", "fixture_native_contract_planning_completed", "fixture_native_implementation_requires_owner_decision", "owner_decision_required"):
    ...
    rec = data.get("phase_4ay_recommendation") if isinstance(data.get("phase_4ay_recommendation"), dict) else {}
    if rec.get("recommended_next_step") != "workflow_nodes_fixture_native_implementation_owner_decision":
        blockers.append("phase_4ay_recommendation must recommend workflow-nodes fixture/native implementation owner decision")
    for field in ("production_write_allowed", "production_route_switch_allowed", "fallback_removal_allowed", "production_write_canary_allowed"):
        if rec.get(field) is not False:
            blockers.append(f"phase_4ay_recommendation.{field} must be false")
    ...
    if unexpected:
        blockers.append(f"unexpected changed files for Phase 4AX package: {unexpected}")
...
def _write_outputs(report: dict[str, Any], output_json: str | None, output_md: str | None) -> None:
    ...
    if output_md:
        lines = [
            "# Phase 4AX Workflow Nodes Fixture Native Contract Plan Check",
            "",
