...
ADAPTER = ROOT / "aicrm_next/automation_engine/agent_output_sqlalchemy_repository.py"
HARNESS = ROOT / "tools/run_phase4cf_agent_outputs_adapter_parity.py"
TEST = ROOT / "tests/test_phase4cf_agent_outputs_repository_adapter_parity_bundle.py"

AGENT_OUTPUTS = "/api/admin/automation-conversion/agent-outputs*"
AGENT_RUNS = "/api/admin/automation-conversion/agent-runs*"
NEXT_BUNDLE = "phase_4cg_agent_runs_repository_adapter_parity_bundle"
COMPLETED_STEP = "phase_4cf_agent_outputs_repository_adapter_parity_completed"
REQUIRED_CHANGED = {
...
def build_report() -> dict[str, Any]:
    ...
    if data.get("status") != "phase_4cf_agent_outputs_repository_adapter_parity_bundle":
        blockers.append("status must be Phase 4CF agent outputs repository adapter parity bundle")
    if bundle.get("type") != "repository_adapter_parity_bundle" or bundle.get("route_family") != AGENT_OUTPUTS:
        blockers.append("bundle must be repository_adapter_parity_bundle for agent-outputs")
    ...
    if adapter.get("backend_flag") != "AICRM_AGENT_OUTPUTS_REPO_BACKEND":
        blockers.append("adapter backend flag must be route-specific")
    if set(adapter.get("database_url_flags") or []) != {"AICRM_AGENT_OUTPUTS_TEST_DATABASE_URL", "AICRM_AGENT_OUTPUTS_STAGING_DATABASE_URL"}:
    ...
    if set(adapter.get("supported_operations") or []) != {"list_agent_outputs", "get_agent_output"}:
    ...
    if "SqlAlchemyAgentOutputRepository" not in adapter_text:
        blockers.append("adapter must define SqlAlchemyAgentOutputRepository")
    if "automation_agent_outputs" not in adapter_text:
        blockers.append("adapter must read automation_agent_outputs")
    if "create_agent_output" in adapter_text or "INSERT INTO automation_agent_outputs" in adapter_text:
        blockers.append("adapter must not add agent-output writes")
    if "AGENT_OUTPUT_TEST_DATABASE_URL_ENV" not in repo_text or "AGENT_OUTPUT_STAGING_DATABASE_URL_ENV" not in repo_text:
        blockers.append("repo must expose route-specific agent-output DB URL flags")
    ...
    if state.get("active_candidate") != AGENT_RUNS or state_update.get("active_candidate") != AGENT_RUNS:
        blockers.append("phase state must advance active candidate to agent-runs")
    if state.get("last_merged_pr") != "#690":
        blockers.append("phase state last_merged_pr must record #690")
    if state.get("last_attempted_action") != "phase_4cf_agent_outputs_repository_adapter_parity_bundle":
        blockers.append("phase state last_attempted_action must be Phase 4CF")
    if state.get("last_created_pr") != "#691":
        blockers.append("phase state last_created_pr must be #691")
    if state.get("recommended_next_pr") != NEXT_BUNDLE:
        blockers.append("phase state recommended_next_pr must be Phase 4CG")
    if set(state.get("next_allowed_actions") or []) != {NEXT_BUNDLE}:
        blockers.append("phase state next_allowed_actions must be Phase 4CG")
    if COMPLETED_STEP not in set(state.get("completed_steps") or []):
        blockers.append("completed_steps must include Phase 4CF")
    ...
    if unexpected:
        blockers.append(f"unexpected changed files for Phase 4CF: {unexpected}")
...
def _write_outputs(report: dict[str, Any], output_json: str | None, output_md: str | None) -> None:
    ...
    if output_md:
        lines = [
            "# Phase 4CF Agent Outputs Repository Adapter Parity Bundle Check",
            "",
