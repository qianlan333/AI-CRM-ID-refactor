...
DOC = ROOT / "docs/development/phase_4bt_workflow_nodes_fixture_runtime.md"
...
DOMAIN = ROOT / "aicrm_next/automation_engine/workflow_nodes.py"
TEST = ROOT / "tests/test_phase4bt_workflow_nodes_fixture_runtime.py"
...
REQUIRED_RUNTIME_TERMS = {
    ...
    APP: [
        ...
        "_workflow_node_production_unavailable_payload",
    ],
    DTO: [
    ...
    REPO: [
        "list_workflow_nodes",
        "create_workflow_node",
        "list_workflow_node_audit_events",
        "phase4ax_entry_node",
    ...
    DOMAIN: [
        ...
        "reject_dangerous_workflow_node_fields",
    ],
    TEST: [
        "test_fixture_repository_lists_seeded_workflow_nodes",
...
def build_report() -> dict[str, Any]:
    ...
    if state.get("active_candidate") != TASKS:
        blockers.append("phase_execution_state.active_candidate must advance to tasks after workflow-nodes runtime slice")
    if state.get("last_merged_pr") != "#676":
        blockers.append("phase_execution_state.last_merged_pr must record #676")
    if state.get("last_attempted_action") != "phase_4bt_workflow_nodes_fixture_native_list_create_runtime":
        blockers.append("phase_execution_state.last_attempted_action must be Phase 4BT workflow-nodes runtime")
    if state.get("last_created_pr") != "#679":
        blockers.append("phase_execution_state.last_created_pr must be #679")
    if state.get("recommended_next_pr") != "phase_4bu_tasks_fixture_native_list_create_runtime":
        blockers.append("phase_execution_state.recommended_next_pr must advance to tasks fixture runtime")
    if set(state.get("next_allowed_actions") or []) != {"phase_4bu_tasks_fixture_native_list_create_runtime"}:
        blockers.append("phase_execution_state.next_allowed_actions must be Phase 4BU tasks runtime")
    if "phase_4bt_workflow_nodes_fixture_native_list_create_runtime_completed" not in set(state.get("completed_steps") or []):
        blockers.append("completed_steps must include Phase 4BT workflow-nodes runtime")
    ...
    if set(readiness.get("implemented_runtime_slices") or []) != {
        ...
        blockers.append("workflow_nodes_readiness.implemented_runtime_slices must include list/create")
    for field in (
    ...
    if unexpected:
        blockers.append(f"unexpected changed files for Phase 4BT workflow-nodes package: {unexpected}")
...
def _write_outputs(report: dict[str, Any], output_json: str | None, output_md: str | None) -> None:
    ...
    if output_md:
        lines = [
            "# Phase 4BT Workflow Nodes Fixture Runtime Check",
            "",
