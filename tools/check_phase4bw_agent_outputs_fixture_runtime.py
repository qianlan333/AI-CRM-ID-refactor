...
DOC = ROOT / "docs/development/phase_4bw_agent_outputs_fixture_runtime.md"
...
DOMAIN = ROOT / "aicrm_next/automation_engine/agent_outputs.py"
TEST = ROOT / "tests/test_phase4bw_agent_outputs_fixture_runtime.py"
...
REQUIRED_RUNTIME_TERMS = {
    ...
    APP: [
        ...
        "_agent_output_production_unavailable_payload",
    ],
    DTO: [
    ...
    DOMAIN: [
        "agent_output_projection",
        "agent_output_side_effect_safety",
        "normalize_agent_output_filters",
        "reject_dangerous_agent_output_fields",
    ],
    TEST: [
        "test_fixture_repository_lists_seeded_agent_outputs",
...
def _require_bool(readiness: dict[str, Any], blockers: list[str], field: str, expected: bool) -> None:
    if readiness.get(field) is not expected:
        blockers.append(f"agent_outputs_readiness.{field} must be {str(expected).lower()}")
...
def build_report() -> dict[str, Any]:
    ...
    if state.get("active_candidate") != AGENT_RUNS:
        blockers.append("phase_execution_state.active_candidate must advance to agent-runs after agent-outputs runtime slice")
    if state.get("last_merged_pr") != "#681":
        blockers.append("phase_execution_state.last_merged_pr must record #681")
    if state.get("last_attempted_action") != "phase_4bw_agent_outputs_fixture_native_list_detail_runtime":
        blockers.append("phase_execution_state.last_attempted_action must be Phase 4BW agent-outputs runtime")
    if state.get("last_created_pr") != "#683":
        blockers.append("phase_execution_state.last_created_pr must be #683")
    if state.get("recommended_next_pr") != "phase_4bx_agent_runs_fixture_native_list_detail_runtime":
        blockers.append("phase_execution_state.recommended_next_pr must advance to agent-runs fixture runtime")
    if set(state.get("next_allowed_actions") or []) != {"phase_4bx_agent_runs_fixture_native_list_detail_runtime"}:
        blockers.append("phase_execution_state.next_allowed_actions must be Phase 4BX agent-runs runtime")
    if "phase_4bw_agent_outputs_fixture_native_list_detail_runtime_completed" not in set(state.get("completed_steps") or []):
        blockers.append("completed_steps must include Phase 4BW agent-outputs runtime")
    ...
    if set(readiness.get("implemented_runtime_slices") or []) != {
        ...
        blockers.append("agent_outputs_readiness.implemented_runtime_slices must include list/detail")
    if readiness.get("paused_by_pr") != "#683":
    ...
    if unexpected:
        blockers.append(f"unexpected changed files for Phase 4BW agent-outputs package: {unexpected}")
    ...
    return {
        ...
        "details": {
            ...
            "route_family": AGENT_OUTPUTS,
            "next_route_family": AGENT_RUNS,
...
def _write_outputs(report: dict[str, Any], output_json: str | None, output_md: str | None) -> None:
    ...
    if output_md:
        lines = [
            "# Phase 4BW Agent Outputs Fixture Runtime Check",
            "",
