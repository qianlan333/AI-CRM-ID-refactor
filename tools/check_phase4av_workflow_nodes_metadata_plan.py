...
def build_report() -> dict[str, Any]:
    ...
    for field in ("active_candidate", "last_merged_pr", "last_attempted_action", "recommended_next_pr", "owner_approval_required"):
        if state.get(field) != state_update.get(field):
            blockers.append(f"phase_execution_state.{field} must match Phase 4AV plan")
    ...
    if set(state.get("next_allowed_actions") or []) != {"phase_4aw_workflow_nodes_schema_route_surface_confirmation"}:
        blockers.append("next_allowed_actions must advance to Phase 4AW workflow-nodes schema route surface confirmation")
