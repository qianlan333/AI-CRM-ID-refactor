...
def build_report(args: argparse.Namespace) -> dict[str, object]:
    ...
    missing_args = [
        flag
        for flag, present in (
            ("--confirm-owner-switch-canary", args.confirm_owner_switch_canary),
            ...
            ("--confirm-rollback-ready", args.confirm_rollback_ready),
        )
        if not present
    ]
    blocked = bool(missing_env or missing_args)
    return {
        "overall": "BLOCKED" if blocked else "READY_FOR_OWNER_REVIEW",
        "ok": True,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--confirm-owner-switch-canary", action="store_true")
