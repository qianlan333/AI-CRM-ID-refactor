...
from tools.check_phase4bp_agent_runs_fixture_native_implementation_owner_decision import load_yaml


DOC = ROOT / "docs/development/phase_4cn_agent_runs_staging_readiness_bundle.md"
PLAN_YAML = ROOT / "docs/development/phase_4cn_agent_runs_staging_readiness_bundle.yaml"
STATE = ROOT / "docs/development/phase_execution_state.yaml"
RUNNER = ROOT / "tools/run_phase4cn_agent_runs_staging_readiness.py"
TEST = ROOT / "tests/test_phase4cn_agent_runs_staging_readiness_bundle.py"

AGENT_RUNS = "/api/admin/automation-conversion/agent-runs*"
TASK_GROUPS = "/api/admin/automation-conversion/task-groups*"
NEXT_BUNDLE = "phase_4co_task_groups_production_dry_run_readiness_bundle"
COMPLETED_STEP = "phase_4cn_agent_runs_staging_readiness_completed"
REQUIRED_CHANGED = {
...
def _contains_forbidden_env_fallback(text: str) -> bool:
    return any(
        ...
        for fragment in (
            ...
            'os.getenv("AICRM_AGENT_RUNS_TEST_DATABASE_URL"',
            'os.environ.get("AICRM_AGENT_RUNS_TEST_DATABASE_URL"',
        )
    )
...
def build_report() -> dict[str, Any]:
    ...
    if data.get("status") != "phase_4cn_agent_runs_staging_readiness_bundle":
        blockers.append("status must be Phase 4CN agent_runs staging readiness bundle")
    if bundle.get("type") != "staging_readiness_bundle" or bundle.get("route_family") != AGENT_RUNS:
        blockers.append("bundle must be staging_readiness_bundle for agent_runs")
    ...
    expected_flags = {
        "runner_path": "tools/run_phase4cn_agent_runs_staging_readiness.py",
        "staging_database_url_flag": "AICRM_AGENT_RUNS_STAGING_DATABASE_URL",
        "backend_flag": "AICRM_AGENT_RUNS_REPO_BACKEND",
        "approval_flag": "AICRM_PHASE4CN_STAGING_SMOKE_APPROVED",
        "write_approval_flag": "AICRM_PHASE4CN_STAGING_WRITE_APPROVED",
    }
    ...
    if "AICRM_AGENT_RUNS_TEST_DATABASE_URL" not in set(readiness.get("forbidden_database_url_fallbacks") or []):
    ...
    if _contains_forbidden_env_fallback(runner_text):
        blockers.append("runner must not fall back to DATABASE_URL or agent_runs test DB URL")
    ...
    if state.get("active_candidate") != TASK_GROUPS or state_update.get("active_candidate") != TASK_GROUPS:
        blockers.append("phase state must advance active candidate to task-groups production dry-run readiness")
    if state.get("last_merged_pr") != "#699":
        blockers.append("phase state last_merged_pr must record #699")
    if state.get("last_attempted_action") != "phase_4cn_agent_runs_staging_readiness_bundle":
        blockers.append("phase state last_attempted_action must be Phase 4CN")
    if state.get("last_created_pr") != "#700":
        blockers.append("phase state last_created_pr must be #700")
    if state.get("recommended_next_pr") != NEXT_BUNDLE:
        blockers.append("phase state recommended_next_pr must be Phase 4CO")
    if set(state.get("next_allowed_actions") or []) != {NEXT_BUNDLE}:
        blockers.append("phase state next_allowed_actions must be Phase 4CO")
    if COMPLETED_STEP not in set(state.get("completed_steps") or []):
        blockers.append("completed_steps must include Phase 4CN")

    top_level_staging = state.get("staging_readiness_slices") if isinstance(state.get("staging_readiness_slices"), list) else []
    if not any(isinstance(item, dict) and item.get("route_family") == AGENT_RUNS and item.get("slice") == "agent_runs_staging_readiness_preflight" for item in top_level_staging):
        blockers.append("staging_readiness_slices must record agent_runs staging readiness preflight")
    ...
    if agent_runs.get("staging_smoke_executed") is not False:
        blockers.append("agent_runs_readiness.staging_smoke_executed must be false")
    for field in (
        ...
        if agent_runs.get(field) is not True:
            blockers.append(f"agent_runs_readiness.{field} must remain true")
    for field in (
    ...
    if unexpected:
        blockers.append(f"unexpected changed files for Phase 4CN: {unexpected}")
...
def _write_outputs(report: dict[str, Any], output_json: str | None, output_md: str | None) -> None:
    ...
    if output_md:
        lines = [
            "# Phase 4CN Agent Runs Staging Readiness Bundle Check",
            "",
