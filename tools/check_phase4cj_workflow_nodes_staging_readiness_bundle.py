...
from tools.check_phase4ay_workflow_nodes_fixture_native_implementation_owner_decision import load_yaml


DOC = ROOT / "docs/development/phase_4cj_workflow_nodes_staging_readiness_bundle.md"
PLAN_YAML = ROOT / "docs/development/phase_4cj_workflow_nodes_staging_readiness_bundle.yaml"
STATE = ROOT / "docs/development/phase_execution_state.yaml"
RUNNER = ROOT / "tools/run_phase4cj_workflow_nodes_staging_readiness.py"
TEST = ROOT / "tests/test_phase4cj_workflow_nodes_staging_readiness_bundle.py"

WORKFLOW_NODES = "/api/admin/automation-conversion/workflow-nodes*"
TASKS = "/api/admin/automation-conversion/tasks*"
NEXT_BUNDLE = "phase_4ck_tasks_staging_readiness_bundle"
COMPLETED_STEP = "phase_4cj_workflow_nodes_staging_readiness_completed"
REQUIRED_CHANGED = {
...
def _contains_forbidden_env_fallback(text: str) -> bool:
    return any(
        ...
        for fragment in (
            ...
            'os.getenv("AICRM_WORKFLOW_NODES_TEST_DATABASE_URL"',
            'os.environ.get("AICRM_WORKFLOW_NODES_TEST_DATABASE_URL"',
        )
    )
...
def build_report() -> dict[str, Any]:
    ...
    if data.get("status") != "phase_4cj_workflow_nodes_staging_readiness_bundle":
        blockers.append("status must be Phase 4CJ workflow-nodes staging readiness bundle")
    if bundle.get("type") != "staging_readiness_bundle" or bundle.get("route_family") != WORKFLOW_NODES:
        blockers.append("bundle must be staging_readiness_bundle for workflow-nodes")
    ...
    expected_flags = {
        "runner_path": "tools/run_phase4cj_workflow_nodes_staging_readiness.py",
        "staging_database_url_flag": "AICRM_WORKFLOW_NODES_STAGING_DATABASE_URL",
        "backend_flag": "AICRM_WORKFLOW_NODES_REPO_BACKEND",
        "approval_flag": "AICRM_PHASE4CJ_STAGING_SMOKE_APPROVED",
        "write_approval_flag": "AICRM_PHASE4CJ_STAGING_WRITE_APPROVED",
    }
    ...
    if "AICRM_WORKFLOW_NODES_TEST_DATABASE_URL" not in set(readiness.get("forbidden_database_url_fallbacks") or []):
    ...
    if _contains_forbidden_env_fallback(runner_text):
        blockers.append("runner must not fall back to DATABASE_URL or workflow-nodes test DB URL")
    ...
    if state.get("active_candidate") != TASKS or state_update.get("active_candidate") != TASKS:
        blockers.append("phase state must advance active candidate to tasks staging readiness")
    if state.get("last_merged_pr") != "#695":
        blockers.append("phase state last_merged_pr must record #695")
    if state.get("last_attempted_action") != "phase_4cj_workflow_nodes_staging_readiness_bundle":
        blockers.append("phase state last_attempted_action must be Phase 4CJ")
    if state.get("last_created_pr") != "#696":
        blockers.append("phase state last_created_pr must be #696")
    if state.get("recommended_next_pr") != NEXT_BUNDLE:
        blockers.append("phase state recommended_next_pr must be Phase 4CK")
    if set(state.get("next_allowed_actions") or []) != {NEXT_BUNDLE}:
        blockers.append("phase state next_allowed_actions must be Phase 4CK")
    if COMPLETED_STEP not in set(state.get("completed_steps") or []):
        blockers.append("completed_steps must include Phase 4CJ")

    top_level_staging = state.get("staging_readiness_slices") if isinstance(state.get("staging_readiness_slices"), list) else []
    if not any(isinstance(item, dict) and item.get("route_family") == WORKFLOW_NODES and item.get("slice") == "workflow_nodes_staging_readiness_preflight" for item in top_level_staging):
        blockers.append("staging_readiness_slices must record workflow-nodes staging readiness preflight")
    workflow_nodes = state.get("workflow_nodes_readiness") if isinstance(state.get("workflow_nodes_readiness"), dict) else {}
    for field in (
        ...
        if workflow_nodes.get(field) is not True:
            blockers.append(f"workflow_nodes_readiness.{field} must be true")
    if workflow_nodes.get("staging_smoke_executed") is not False:
        blockers.append("workflow_nodes_readiness.staging_smoke_executed must be false")
    for field in ("production_owner_switch_ready", "production_write_ready", "fallback_removal_ready", "production_repository_route_enablement_ready", "delete_ready"):
        if workflow_nodes.get(field) is not False:
    ...
    if unexpected:
        blockers.append(f"unexpected changed files for Phase 4CJ: {unexpected}")
    ...
    return {
        ...
        "blockers": blockers,
        "warnings": warnings,
        "details": {"changed_files": sorted(changed), "bundle_type": bundle.get("type"), "route_family": WORKFLOW_NODES},
    }
...
def _write_outputs(report: dict[str, Any], output_json: str | None, output_md: str | None) -> None:
    ...
    if output_md:
        lines = [
            "# Phase 4CJ Workflow Nodes Staging Readiness Bundle Check",
            "",
