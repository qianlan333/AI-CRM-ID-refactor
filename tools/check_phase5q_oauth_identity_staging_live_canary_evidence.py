...
PLAN_YAML = ROOT / "docs/development/phase_5q_oauth_identity_staging_live_canary_evidence.yaml"
STATE = ROOT / "docs/development/phase_execution_state.yaml"
STAGING_RUNNER = ROOT / "tools/run_phase5q_oauth_identity_staging_live_canary_evidence.py"
PRODUCTION_REVIEW_RUNNER = ROOT / "tools/run_phase5q_oauth_identity_production_live_readiness_review.py"
TEST = ROOT / "tests/test_phase5q_oauth_identity_staging_live_canary_evidence.py"
ROUTE_FAMILY = "/api/h5/wechat/oauth*"
NEXT_BUNDLE = "phase_5r_oauth_identity_production_canary_readiness_bundle"
COMPLETED_STEP = "phase_5q_oauth_identity_staging_live_canary_evidence_completed"
REQUIRED_ENV = {
    "AICRM_OAUTH_IDENTITY_LIVE_ADAPTER_ENABLED",
    "AICRM_OAUTH_IDENTITY_LIVE_CALLBACK_APPROVED",
    "AICRM_OAUTH_IDENTITY_CONFIG_REVIEWED",
    "AICRM_PHASE5Q_OAUTH_IDENTITY_STAGING_CANARY_APPROVED",
    "AICRM_PHASE5Q_OAUTH_IDENTITY_STAGING_TARGET_APPROVED",
}
REQUIRED_ARGS = {
...
def _runner_blockers() -> list[str]:
    ...
    for arg in REQUIRED_ARGS | {"--code", "--fake-safe-code"}:
        if arg not in text:
            blockers.append(f"staging runner missing required arg: {arg}")
    for env in REQUIRED_ENV | {"AICRM_OAUTH_IDENTITY_APP_ID", "AICRM_OAUTH_IDENTITY_APP_SECRET"}:
        if env not in text:
    ...
    for token in ("_redact", "redacted_state", "redacted_code", "token_redacted", "phase5p_staging.build_report", "production_callback_url_forbidden", "batch_replay"):
        if token not in text:
            blockers.append(f"staging runner missing safety token: {token}")
    forbidden_calls = sorted({"send", "send_message", "set_cookie", "payment", "upload_media", "openclaw", "mcp_dispatch"} & _call_names(STAGING_RUNNER))
    if forbidden_calls:
        blockers.append(f"staging runner contains forbidden side-effect calls: {forbidden_calls}")
    forbidden_tokens = ("requests", "httpx", "aiohttp", "urllib", "set_cookie", "session_write", "identity_write")
    found_tokens = sorted(token for token in forbidden_tokens if token in text and token not in {"session_write", "identity_write"})
    if found_tokens:
        blockers.append(f"staging runner contains forbidden live/network token: {found_tokens}")
    review_text = PRODUCTION_REVIEW_RUNNER.read_text(encoding="utf-8")
    for arg in ("--staging-evidence-json", "--confirm-no-production-live-oauth-call", "--confirm-no-production-callback-cutover", "--confirm-no-production-session-write", "--output-json", "--output-md"):
        if arg not in review_text:
            blockers.append(f"production review runner missing arg: {arg}")
    for token in ("production_live_oauth_call_executed", "production_callback_cutover_executed", "production_session_write_executed", "production_identity_write_executed", "route_owner_changed", "production_compat_changed", "fallback_removed"):
        if token not in review_text:
            blockers.append(f"production review runner missing output field: {token}")
    forbidden_review_tokens = {"build_live_oauth_identity_adapter", "exchange_code_live", "requests", "httpx", "aiohttp", "urllib"}
    found_review = sorted(token for token in forbidden_review_tokens if token in review_text)
    if found_review:
        blockers.append(f"production review must never call OAuth provider: {found_review}")
    return blockers
...
def build_report() -> dict[str, Any]:
    ...
    if data.get("status") != "phase_5q_oauth_identity_staging_live_canary_evidence":
    ...
    for key in ("production_live_oauth_call_authorized", "production_callback_cutover_authorized", "production_session_write_authorized", "production_identity_write_authorized", "production_compat_change_authorized", "fallback_removal_authorized"):
    ...
    for key in ("batch_replay_allowed", "production_callback_url_allowed", "production_session_write_allowed", "production_identity_write_allowed"):
    ...
    for key in ("production_live_oauth_call_executed", "production_callback_cutover_executed", "production_session_write_executed", "production_identity_write_executed"):
        if review.get(key) is not False:
            blockers.append(f"production_readiness_review.{key} must be false")
    if review.get("requires_staging_evidence_json") is not True:
        blockers.append("production_readiness_review.requires_staging_evidence_json must be true")
    if _dict(data.get("next_bundle")).get("recommended_next_step") != NEXT_BUNDLE:
        blockers.append(f"next bundle must be {NEXT_BUNDLE}")
    if state.get("last_merged_pr") != "#729":
        blockers.append("phase_execution_state.last_merged_pr must be #729")
    if state.get("last_attempted_action") != "phase_5q_oauth_identity_staging_live_canary_evidence_bundle":
        blockers.append("last_attempted_action must be Phase 5Q")
    ...
    if unexpected:
        blockers.append(f"changed files outside Phase 5Q allowlist: {unexpected}")
    for path in sorted(changed_files):
        if path in FORBIDDEN_EXACT_CHANGED or any(path.startswith(prefix) for prefix in FORBIDDEN_CHANGED_PREFIXES):
            blockers.append(f"forbidden changed file for Phase 5Q: {path}")
    ok = not blockers
...
def _write_md(report: dict[str, Any], path: str) -> None:
    lines = ["# Phase 5Q OAuth Identity Staging Live Canary Check", "", f"- overall: {report.get('overall')}", f"- ok: {str(report.get('ok')).lower()}", "", "## Blockers"]
    lines.extend([f"- {item}" for item in report.get("blockers", [])] or ["- none"])
    Path(path).write_text("\n".join(lines) + "\n", encoding="utf-8")


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Check Phase 5Q OAuth identity staging live canary evidence bundle.")
