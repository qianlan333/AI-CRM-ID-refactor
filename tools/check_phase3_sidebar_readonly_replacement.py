...
PRODUCTION_PROBE_DATABASE_URL = (
    "postgresql://sidebar:sidebar@127.0.0.1:1/aicrm_sidebar_phase3_probe"
)
...
def production_sidebar_probe_env():
    ...
    os.environ.setdefault("SECRET_KEY", "phase3-sidebar-readonly-replacement")
    os.environ.setdefault(
        "AUTOMATION_INTERNAL_API_TOKEN",
        "phase3-sidebar-readonly-replacement",
    )
    try:
        yield
    finally:
...
def check_input_errors(client: Any) -> dict[str, Any]:
    ...
    for path in TARGET_ROUTES:
        ...
        if response.status_code != 400:
            blockers.append(f"{path} without external_userid returned {response.status_code}, expected 400")
        if _contains_fixture_marker(body):
...
def check_production_unavailable_behavior(client: Any) -> dict[str, Any]:
    ...
    for path in TARGET_ROUTES:
        ...
        if response.status_code == 200 and record["source_status"] in {"fixture", "local_contract", "demo"}:
            blockers.append(
                f"{path} returned 200 fake success source_status={record['source_status']}"
            )
...
def check_static_boundaries() -> dict[str, Any]:
    ...
    identity_api = ROOT / "aicrm_next/identity_contact/api.py"
    identity_source = _read(identity_api)
    for forbidden in ("legacy_customer_read_facade", "get_customer_via_legacy"):
        if forbidden in identity_source:
            blockers.append(f"{_rel(identity_api)} directly references {forbidden}")

    customer_api = ROOT / "aicrm_next/customer_read_model/api.py"
    customer_source = _read(customer_api)
    customer_context_source = _function_source(customer_source, "get_sidebar_customer_context")
    if not customer_context_source:
        blockers.append(f"{_rel(customer_api)} missing get_sidebar_customer_context endpoint")
    for forbidden in ("legacy_customer_read_facade", "get_customer_via_legacy"):
        if forbidden in customer_context_source:
            blockers.append(
                f"{_rel(customer_api)} get_sidebar_customer_context directly references {forbidden}"
            )
    if "legacy_customer_read_facade" in customer_source:
        warnings.append(
            f"{_rel(customer_api)} still has legacy facade imports for non-target customer routes; "
            "Phase 3A keeps them out of the target customer-context endpoint"
        )
    ...
    for snippet in required_snippets:
        if snippet not in production_compat_source:
            blockers.append(
                f"{_rel(production_compat)} missing expected sidebar wildcard behavior: {snippet}"
            )
...
def write_markdown_report(report: dict[str, Any], path: Path) -> None:
    lines = [
        "# Phase 3 Sidebar Readonly Replacement Check",
        "",
