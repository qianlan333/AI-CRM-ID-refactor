...
AUTH_FALSE_FIELDS = {
    ...
    "agent_outputs_expansion_authorized",
...
REQUIRED_DECISIONS = {
    "approve_or_decline_agents_fixture_native_runtime_implementation",
    "confirm_metadata_list_create_only_scope",
    "confirm_no_detail_draft_publish_delete_route_expansion",
    "confirm_no_agent_run_execution",
    "confirm_no_agent_outputs_expansion",
...
REQUIRED_SAFE_OPTIONS = {
    "pause_agents_and_select_agent_outputs_metadata_planning",
    "owner_approves_future_agents_fixture_native_runtime_implementation_package",
    "owner_defers_agents_until_agent_runtime_boundary_plan_exists",
}
REJECTED_TRUE_FIELDS = {
    ...
    "agent_detail_draft_publish_delete_expansion",
    "agent_run_execution",
    "agent_outputs_expansion",
    "llm_generation",
    "deepseek_adapter",
    "openclaw_mcp_call",
    "workflow_execution",
...
REQUIRED_AGENT_OUTPUTS_GUARDRAILS = {
    "planning_only",
...
REQUIRED_PHASE_4BI_SCOPE = {
    "agent_outputs_metadata_planning",
    "list_detail_surface_inventory",
    "export_job_creation_exclusion",
    "file_download_exclusion",
...
def build_report() -> dict[str, Any]:
    ...
    if data.get("status") != "phase_4bh_agents_fixture_native_implementation_owner_decision_no_runtime_change":
        blockers.append("status must be Phase 4BH agents owner decision no runtime change")
    ...
    if paused.get("route_family") != AGENTS:
        blockers.append("paused_candidate must be agents")
    ...
    if next_candidate.get("selected_route_family") != AGENT_OUTPUTS:
        blockers.append("next_candidate must select agent-outputs")
    ...
    if not REQUIRED_AGENT_OUTPUTS_GUARDRAILS <= set(next_candidate.get("required_guardrails") or []):
        blockers.append("next_candidate.required_guardrails incomplete")
    if not REQUIRED_PHASE_4BI_SCOPE <= set(next_candidate.get("phase_4bi_scope") or []):
        blockers.append("next_candidate.phase_4bi_scope incomplete")
    for route in (AGENTS, AGENT_OUTPUTS):
    ...
    if state.get("active_candidate") != AGENT_OUTPUTS:
        blockers.append("phase_execution_state.active_candidate must advance to agent-outputs")
    if state.get("last_merged_pr") not in {"#664", "#665"}:
        blockers.append("phase_execution_state.last_merged_pr must record #664 or later Phase 4BI state #665")
    if state.get("last_attempted_action") not in {
        ...
        blockers.append("phase_execution_state.last_attempted_action must be Phase 4BH or later Phase 4BI")
    if state.get("last_created_pr") not in {"#665", "#666"}:
        blockers.append("phase_execution_state.last_created_pr must be #665 or later Phase 4BI PR #666")
    if state.get("recommended_next_pr") not in {
        "phase_4bi_agent_outputs_metadata_planning",
        "phase_4bj_agent_outputs_schema_route_surface_confirmation",
    }:
        blockers.append("phase_execution_state.recommended_next_pr must be Phase 4BI or later Phase 4BJ")
    if set(state.get("next_allowed_actions") or []) not in (
        {"phase_4bi_agent_outputs_metadata_planning"},
        {"phase_4bj_agent_outputs_schema_route_surface_confirmation"},
    ):
        blockers.append("phase_execution_state.next_allowed_actions must be Phase 4BI or later Phase 4BJ")
    ...
    if state_update.get("phase_4bh_completed_step") not in set(state.get("completed_steps") or []):
        blockers.append("phase_execution_state.completed_steps must include Phase 4BH completed step")

    state_paused = state.get("paused_candidates") if isinstance(state.get("paused_candidates"), list) else []
    if not any(isinstance(item, dict) and item.get("route_family") == AGENTS and item.get("owner_approval_required") is True for item in state_paused):
        blockers.append("phase_execution_state.paused_candidates must include agents owner decision pause")
    ...
    if not str(agents_readiness.get("paused_by_pr", "")).strip():
        blockers.append("agents_readiness.paused_by_pr must be recorded or pending")
    ...
    agent_outputs = state.get("agent_outputs_readiness") if isinstance(state.get("agent_outputs_readiness"), dict) else {}
    for field in ("metadata_planning_ready", "export_job_creation_excluded", "file_download_excluded", "agent_run_execution_excluded", "llm_generation_excluded", "external_call_excluded"):
        if agent_outputs.get(field) is not True:
    ...
    for field in (
        ...
        if agent_outputs.get(field) is not False:
            blockers.append(f"agent_outputs_readiness.{field} must be false")
    for field in ("metadata_planning_completed", "schema_route_surface_confirmation_ready"):
        if agent_outputs.get(field) not in {False, True}:
            blockers.append(f"agent_outputs_readiness.{field} must be boolean")

    rec = data.get("phase_4bi_recommendation") if isinstance(data.get("phase_4bi_recommendation"), dict) else {}
    if rec.get("recommended_next_step") != "agent_outputs_metadata_planning":
        blockers.append("phase_4bi_recommendation must recommend agent-outputs metadata planning")
    for field in ("production_write_allowed", "production_route_switch_allowed", "fallback_removal_allowed", "production_write_canary_allowed"):
        if rec.get(field) is not False:
            blockers.append(f"phase_4bi_recommendation.{field} must be false")
    ...
    if unexpected:
        blockers.append(f"unexpected changed files for Phase 4BH package: {unexpected}")
    ...
    return {
        ...
        "warnings": warnings,
        "details": {
            "paused_route_family": paused.get("route_family"),
            "selected_next_candidate": next_candidate.get("selected_route_family"),
...
def _write_outputs(report: dict[str, Any], output_json: str | None, output_md: str | None) -> None:
    ...
    if output_md:
        lines = [
            "# Phase 4BH Agents Fixture Native Implementation Owner Decision Check",
            "",
            f"- overall: {report['overall']}",
            f"- ok: {str(report['ok']).lower()}",
            f"- paused_route_family: {report['details'].get('paused_route_family')}",
            f"- selected_next_candidate: {report['details'].get('selected_next_candidate')}",
            "",
            "## Blockers",
