...
ALLOWED_NEXT_ACTIONS: set[str] = {"phase_7k_final_route_ownership_manifest_cleanup_bundle"}
STOP_TERM_EXEMPT_NEXT_ACTIONS: set[str] = {"phase_7k_final_route_ownership_manifest_cleanup_bundle"}
...
def build_report() -> dict[str, Any]:
    ...
    if state.get("current_phase") != "phase_7j_legacy_runtime_cleanup_blocker_acceptance":
        blockers.append("current_phase must be phase_7j_legacy_runtime_cleanup_blocker_acceptance")
    if state.get("active_candidate") != "legacy_runtime_cleanup_blocker_acceptance":
        blockers.append("active_candidate must select Phase 7J runtime cleanup blocker acceptance")
    ...
    if state.get("last_merged_pr") != "#790":
        blockers.append("last_merged_pr must record latest completed merged PR #790")
