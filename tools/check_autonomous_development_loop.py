...
def build_report() -> dict[str, Any]:
    ...
    if state.get("current_phase") != "post_phase7_owner_feature_selection":
        blockers.append("current_phase must be post_phase7_owner_feature_selection")
    if state.get("active_candidate") != "post_phase7_owner_feature_selection":
        blockers.append("active_candidate must select Post-Phase 7 owner feature selection")
    if state.get("capability_owner") != "aicrm_next.send_content":
        blockers.append("capability_owner must be aicrm_next.send_content")
    if state.get("last_merged_pr") != "#795":
        blockers.append("last_merged_pr must record latest completed merged PR #795")
