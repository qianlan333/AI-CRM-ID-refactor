...
ALLOWED_NEXT_ACTIONS: set[str] = {"phase_7f_production_compat_cleanup_readiness_bundle"}
...
def build_report() -> dict[str, Any]:
    ...
    if state.get("current_phase") != "phase_7e_fallback_cleanup_readiness":
        blockers.append("current_phase must be phase_7e_fallback_cleanup_readiness")
    if state.get("active_candidate") != "phase_7_fallback_cleanup_readiness":
        blockers.append("active_candidate must select the Phase 7E fallback cleanup readiness package")
    ...
    if state.get("last_merged_pr") != "#778":
        blockers.append("last_merged_pr must record latest completed merged PR #778")
