...
ALLOWED_NEXT_ACTIONS: set[str] = {"phase_7j_legacy_runtime_cleanup_blocker_acceptance_bundle"}
STOP_TERM_EXEMPT_NEXT_ACTIONS: set[str] = {"phase_7j_legacy_runtime_cleanup_blocker_acceptance_bundle"}
...
def build_report() -> dict[str, Any]:
    ...
    if state.get("current_phase") != "phase_7i_legacy_runtime_deletion_readiness":
        blockers.append("current_phase must be phase_7i_legacy_runtime_deletion_readiness")
    if state.get("active_candidate") != "legacy_runtime_deletion_readiness":
        blockers.append("active_candidate must select Phase 7I legacy runtime deletion readiness")
    ...
    if state.get("last_merged_pr") != "#789":
        blockers.append("last_merged_pr must record latest completed merged PR #789")
