...
ALLOWED_NEXT_ACTIONS: set[str] = {"phase_7g_first_exact_route_fallback_removal_canary_bundle"}
STOP_TERM_EXEMPT_NEXT_ACTIONS: set[str] = {"phase_7g_first_exact_route_fallback_removal_canary_bundle"}
...
def build_report() -> dict[str, Any]:
    ...
    if state.get("current_phase") != "phase_7f_production_compat_cleanup_readiness":
        blockers.append("current_phase must be phase_7f_production_compat_cleanup_readiness")
    if state.get("active_candidate") != "phase_7_production_compat_cleanup_readiness":
        blockers.append("active_candidate must select the Phase 7F production_compat cleanup readiness package")
    ...
    if state.get("last_merged_pr") != "#780":
        blockers.append("last_merged_pr must record latest completed merged PR #780")
