...
ALLOWED_NEXT_ACTIONS: set[str] = {"post_phase7_cleanup_task_groups_owner_evidence_validation_bundle"}
...
def build_report() -> dict[str, Any]:
    ...
    if state.get("current_phase") != "post_phase7_cleanup_task_groups_shadow_compare_rollback_evidence":
        blockers.append("current_phase must be post_phase7_cleanup_task_groups_shadow_compare_rollback_evidence")
    if state.get("active_candidate") != "task_groups_shadow_compare_rollback_evidence":
        blockers.append("active_candidate must select task groups shadow compare rollback evidence")
    ...
    if state.get("last_merged_pr") != "#812":
        blockers.append("last_merged_pr must record latest cleanup PR #812")
