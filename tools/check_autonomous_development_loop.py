...
ALLOWED_NEXT_ACTIONS: set[str] = {"phase_7l_final_legacy_retirement_acceptance_bundle"}
STOP_TERM_EXEMPT_NEXT_ACTIONS: set[str] = {"phase_7l_final_legacy_retirement_acceptance_bundle"}
...
def build_report() -> dict[str, Any]:
    ...
    if state.get("current_phase") != "phase_7k_final_route_ownership_manifest_cleanup":
        blockers.append("current_phase must be phase_7k_final_route_ownership_manifest_cleanup")
    if state.get("active_candidate") != "final_route_ownership_manifest_cleanup":
        blockers.append("active_candidate must select Phase 7K final route ownership cleanup")
    ...
    if state.get("last_merged_pr") != "#791":
        blockers.append("last_merged_pr must record latest completed merged PR #791")
