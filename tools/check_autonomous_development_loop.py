...
ALLOWED_NEXT_ACTIONS: set[str] = {
    "post_phase7_cleanup_workflow_nodes_evidence_refresh_bundle",
}
...
def build_report() -> dict[str, Any]:
    ...
    if state.get("current_phase") != "post_phase7_cleanup_task_groups_evidence_refresh":
        blockers.append("current_phase must be post_phase7_cleanup_task_groups_evidence_refresh")
    if state.get("active_candidate") != "task_groups_cleanup_evidence_refresh":
        blockers.append("active_candidate must select task-groups cleanup evidence refresh")
    ...
    if state.get("last_merged_pr") != "#797":
        blockers.append("last_merged_pr must record latest completed merged PR #797")
