...
def _validate_current_state(state: dict[str, Any], blockers: list[str]) -> None:
    ...
    if state.get("last_merged_pr") != "#856":
        blockers.append("last_merged_pr must record merged cleanup PR #856")
    if state.get("last_merged_cleanup_wave") != "runtime_switch_archaeology_cleanup_wave14":
        blockers.append("last_merged_cleanup_wave must record wave 14 runtime-switch archaeology cleanup")
    if state.get("recommended_next_pr") != "architecture_documentation_compaction_wave15":
        blockers.append("recommended_next_pr must name wave 15 architecture documentation compaction")
