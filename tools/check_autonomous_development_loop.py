...
ALLOWED_NEXT_ACTIONS: set[str] = {"post_phase7_cleanup_owner_evidence_package_blocker_acceptance_bundle"}
...
def build_report() -> dict[str, Any]:
    ...
    if state.get("current_phase") != "post_phase7_cleanup_owner_evidence_package_generation":
        blockers.append("current_phase must be post_phase7_cleanup_owner_evidence_package_generation")
    if state.get("active_candidate") != "cleanup_owner_evidence_package_generation":
        blockers.append("active_candidate must select cleanup owner evidence package generation")
    ...
    if state.get("last_merged_pr") != "#806":
        blockers.append("last_merged_pr must record latest cleanup PR #806")
