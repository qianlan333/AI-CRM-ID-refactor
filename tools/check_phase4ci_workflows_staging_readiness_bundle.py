...
from tools.check_phase4au_workflows_fixture_native_implementation_owner_decision import load_yaml
...
TEST = ROOT / "tests/test_phase4ci_workflows_staging_readiness_bundle.py"

WORKFLOWS = "/api/admin/automation-conversion/workflows*"
WORKFLOW_NODES = "/api/admin/automation-conversion/workflow-nodes*"
NEXT_BUNDLE = "phase_4cj_workflow_nodes_staging_readiness_bundle"
COMPLETED_STEP = "phase_4ci_workflows_staging_readiness_completed"
REQUIRED_CHANGED = {
...
def _contains_forbidden_env_fallback(text: str) -> bool:
    return any(
        ...
        for fragment in (
            ...
            'os.getenv("AICRM_WORKFLOWS_TEST_DATABASE_URL"',
            'os.environ.get("AICRM_WORKFLOWS_TEST_DATABASE_URL"',
        )
    )
...
def build_report() -> dict[str, Any]:
    ...
    if data.get("status") != "phase_4ci_workflows_staging_readiness_bundle":
        blockers.append("status must be Phase 4CI workflows staging readiness bundle")
    if bundle.get("type") != "staging_readiness_bundle" or bundle.get("route_family") != WORKFLOWS:
        blockers.append("bundle must be staging_readiness_bundle for workflows")
    ...
    expected_flags = {
        "runner_path": "tools/run_phase4ci_workflows_staging_readiness.py",
        "staging_database_url_flag": "AICRM_WORKFLOWS_STAGING_DATABASE_URL",
        "backend_flag": "AICRM_WORKFLOWS_REPO_BACKEND",
        "approval_flag": "AICRM_PHASE4CI_STAGING_SMOKE_APPROVED",
        "write_approval_flag": "AICRM_PHASE4CI_STAGING_WRITE_APPROVED",
    }
    ...
    if "AICRM_WORKFLOWS_TEST_DATABASE_URL" not in set(readiness.get("forbidden_database_url_fallbacks") or []):
    ...
    if _contains_forbidden_env_fallback(runner_text):
        blockers.append("runner must not fall back to DATABASE_URL or workflows test DB URL")
    ...
    if state.get("active_candidate") != WORKFLOW_NODES or state_update.get("active_candidate") != WORKFLOW_NODES:
        blockers.append("phase state must advance active candidate to workflow-nodes staging readiness")
    if state.get("last_merged_pr") != "#694":
        blockers.append("phase state last_merged_pr must record #694")
    if state.get("last_attempted_action") != "phase_4ci_workflows_staging_readiness_bundle":
        blockers.append("phase state last_attempted_action must be Phase 4CI")
    if state.get("last_created_pr") != "#695":
        blockers.append("phase state last_created_pr must be #695")
    if state.get("recommended_next_pr") != NEXT_BUNDLE:
        blockers.append("phase state recommended_next_pr must be Phase 4CJ")
    if set(state.get("next_allowed_actions") or []) != {NEXT_BUNDLE}:
        blockers.append("phase state next_allowed_actions must be Phase 4CJ")
    if COMPLETED_STEP not in set(state.get("completed_steps") or []):
        blockers.append("completed_steps must include Phase 4CI")

    top_level_staging = state.get("staging_readiness_slices") if isinstance(state.get("staging_readiness_slices"), list) else []
    if not any(isinstance(item, dict) and item.get("route_family") == WORKFLOWS and item.get("slice") == "workflows_staging_readiness_preflight" for item in top_level_staging):
        blockers.append("staging_readiness_slices must record workflows staging readiness preflight")
    ...
    if workflows.get("staging_smoke_executed") is not False:
        blockers.append("workflows_readiness.staging_smoke_executed must be false")
    ...
    if unexpected:
        blockers.append(f"unexpected changed files for Phase 4CI: {unexpected}")
...
def _write_outputs(report: dict[str, Any], output_json: str | None, output_md: str | None) -> None:
    ...
    if output_md:
        lines = [
            "# Phase 4CI Workflows Staging Readiness Bundle Check",
            "",
