...
from aicrm_next.automation_engine.repo import AGENT_TEST_DATABASE_URL_ENV, build_automation_repository
...
def run_harness() -> dict[str, Any]:
    db_url = str(os.getenv(AGENT_TEST_DATABASE_URL_ENV) or "").strip()
    if not db_url:
        return {
            "ok": True,
    ...
    repo = build_automation_repository(agent_backend="sqlalchemy", agent_engine=engine)
    rows, total = repo.list_agents({"workflow_id": 1, "limit": 20, "offset": 0})
    details.append({"name": "list_agents", "status": "passed", "total": total, "row_count": len(rows)})
    payload = {
        "program_id": 9,
        "workflow_id": 9,
        "node_id": 9,
        "task_id": 9,
        "agent_name": "Phase 4CE adapter parity",
        "agent_code": "phase4ce_adapter_parity",
        "agent_type": "metadata",
        "metadata": {"source": "phase4ce_test_db"},
        "operator": "phase4ce",
    }
    created = repo.create_agent(payload, idempotency_key="phase4ce-create", operator="phase4ce")
    replay = repo.create_agent(payload, idempotency_key="phase4ce-create", operator="phase4ce")
    audit_events = repo.list_agent_audit_events()
    checks = {
        "create_with_idempotency": created.get("idempotent_replay") is False,
        "idempotency_replay": replay.get("idempotent_replay") is True and replay["agent"]["id"] == created["agent"]["id"],
...
def _write_outputs(report: dict[str, Any], output_json: str | None, output_md: str | None) -> None:
    ...
    if output_md:
        lines = [
            "# Phase 4CE Agents Adapter Parity Harness",
            "",
