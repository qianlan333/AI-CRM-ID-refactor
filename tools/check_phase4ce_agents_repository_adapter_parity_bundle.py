...
ADAPTER = ROOT / "aicrm_next/automation_engine/agent_sqlalchemy_repository.py"
HARNESS = ROOT / "tools/run_phase4ce_agents_adapter_parity.py"
TEST = ROOT / "tests/test_phase4ce_agents_repository_adapter_parity_bundle.py"

AGENTS = "/api/admin/automation-conversion/agents*"
AGENT_OUTPUTS = "/api/admin/automation-conversion/agent-outputs*"
NEXT_BUNDLE = "phase_4cf_agent_outputs_repository_adapter_parity_bundle"
COMPLETED_STEP = "phase_4ce_agents_repository_adapter_parity_completed"
REQUIRED_CHANGED = {
...
def build_report() -> dict[str, Any]:
    ...
    if data.get("status") != "phase_4ce_agents_repository_adapter_parity_bundle":
        blockers.append("status must be Phase 4CE agents repository adapter parity bundle")
    if bundle.get("type") != "repository_adapter_parity_bundle" or bundle.get("route_family") != AGENTS:
        blockers.append("bundle must be repository_adapter_parity_bundle for agents")
    ...
    if adapter.get("backend_flag") != "AICRM_AGENTS_REPO_BACKEND":
        blockers.append("adapter backend flag must be route-specific")
    if set(adapter.get("database_url_flags") or []) != {"AICRM_AGENTS_TEST_DATABASE_URL", "AICRM_AGENTS_STAGING_DATABASE_URL"}:
    ...
    if "SqlAlchemyAgentRepository" not in adapter_text:
        blockers.append("adapter must define SqlAlchemyAgentRepository")
    if "automation_agents" not in adapter_text or "automation_agent_idempotency" not in adapter_text or "automation_agent_audit_log" not in adapter_text:
        blockers.append("adapter must cover agent main/idempotency/audit tables")
    if "AGENT_TEST_DATABASE_URL_ENV" not in repo_text or "AGENT_STAGING_DATABASE_URL_ENV" not in repo_text:
        blockers.append("repo must expose route-specific agent DB URL flags")
    ...
    if state.get("active_candidate") != AGENT_OUTPUTS or state_update.get("active_candidate") != AGENT_OUTPUTS:
        blockers.append("phase state must advance active candidate to agent-outputs")
    if state.get("last_merged_pr") != "#689":
        blockers.append("phase state last_merged_pr must record #689")
    if state.get("last_attempted_action") != "phase_4ce_agents_repository_adapter_parity_bundle":
        blockers.append("phase state last_attempted_action must be Phase 4CE")
    if state.get("last_created_pr") != "#690":
        blockers.append("phase state last_created_pr must be #690")
    if state.get("recommended_next_pr") != NEXT_BUNDLE:
        blockers.append("phase state recommended_next_pr must be Phase 4CF")
    if set(state.get("next_allowed_actions") or []) != {NEXT_BUNDLE}:
        blockers.append("phase state next_allowed_actions must be Phase 4CF")
    if COMPLETED_STEP not in set(state.get("completed_steps") or []):
        blockers.append("completed_steps must include Phase 4CE")
    ...
    if unexpected:
        blockers.append(f"unexpected changed files for Phase 4CE: {unexpected}")
...
def _write_outputs(report: dict[str, Any], output_json: str | None, output_md: str | None) -> None:
    ...
    if output_md:
        lines = [
            "# Phase 4CE Agents Repository Adapter Parity Bundle Check",
            "",
