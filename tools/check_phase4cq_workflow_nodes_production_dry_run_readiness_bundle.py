...
DOC = ROOT / "docs/development/phase_4cq_workflow_nodes_production_dry_run_readiness_bundle.md"
PLAN_YAML = ROOT / "docs/development/phase_4cq_workflow_nodes_production_dry_run_readiness_bundle.yaml"
STATE = ROOT / "docs/development/phase_execution_state.yaml"
RUNNER = ROOT / "tools/run_phase4cq_workflow_nodes_production_readonly_dry_run.py"
TEST = ROOT / "tests/test_phase4cq_workflow_nodes_production_dry_run_readiness_bundle.py"
...
NEXT_BUNDLE = "phase_4cr_tasks_production_dry_run_readiness_bundle"
REQUIRED_CHANGED = {
...
def build_report() -> dict[str, Any]:
    ...
    if data.get("status") != "phase_4cq_workflow_nodes_production_dry_run_readiness_bundle":
        blockers.append("status must be Phase 4CQ workflow-nodes production dry-run readiness bundle")
    if bundle.get("type") != "production_read_only_dry_run_readiness_bundle" or bundle.get("route_family") != WORKFLOW_NODES:
        blockers.append("bundle must be production_read_only_dry_run_readiness_bundle for workflow-nodes")
    ...
    if runtime.get("runner_path") != "tools/run_phase4cq_workflow_nodes_production_readonly_dry_run.py":
    ...
    forbidden_runner_fragments = (
        ...
        'AICRM_WORKFLOW_NODES_TEST_DATABASE_URL',
        'AICRM_WORKFLOW_NODES_STAGING_DATABASE_URL',
    ...
    if state.get("last_merged_pr") != "#705":
        blockers.append("phase state last_merged_pr must record #705")
    if state.get("last_attempted_action") != "phase_4cq_workflow_nodes_production_dry_run_readiness_bundle":
        blockers.append("phase state last_attempted_action must be Phase 4CQ")
    if state.get("last_created_pr") != "#706":
        blockers.append("phase state last_created_pr must be #706")
    if state.get("active_candidate") != TASKS:
        blockers.append("phase state active_candidate must advance to tasks")
    if state.get("recommended_next_pr") != NEXT_BUNDLE or set(state.get("next_allowed_actions") or []) != {NEXT_BUNDLE}:
        blockers.append("phase state next action must advance to Phase 4CR tasks production dry-run readiness")
    if COMPLETED_STEP not in set(state.get("completed_steps") or []):
        blockers.append("completed_steps must include Phase 4CQ")
    ...
    if not any(isinstance(item, dict) and item.get("route_family") == WORKFLOW_NODES and item.get("slice") == "workflow_nodes_production_readonly_dry_run_readiness" for item in dry_run_slices):
        blockers.append("production_dry_run_readiness_slices must record workflow-nodes readiness")
    ...
    if workflow_nodes.get("production_readonly_dry_run_executed") is not False:
        blockers.append("workflow_nodes_readiness.production_readonly_dry_run_executed must be false")
    if workflow_nodes.get("production_readonly_db_url_flag") != "AICRM_WORKFLOW_NODES_READONLY_DRY_RUN_DATABASE_URL":
        blockers.append("workflow_nodes_readiness production readonly DB flag mismatch")
    ...
    if unexpected:
        blockers.append(f"unexpected changed files for Phase 4CQ: {unexpected}")
...
def _write_outputs(report: dict[str, Any], output_json: str | None, output_md: str | None) -> None:
    ...
    if output_md:
        lines = [
            "# Phase 4CQ Workflow Nodes Production Dry-Run Readiness Bundle Check",
            "",
