...
def build_report() -> dict[str, Any]:
    ...
    if data.get("status") != "phase_4bc_tasks_fixture_native_contract_planning_no_runtime_change":
        blockers.append("status must be Phase 4BC tasks fixture native contract planning no runtime change")
    ...
    if previous.get("phase") != "phase_4bb_tasks_schema_route_surface_confirmation" or previous.get("merged_pr") != "#659":
        blockers.append("previous_phase must record Phase 4BB merged in #659")
    ...
    if not {"/api/admin/automation-conversion/tasks/{task_id}", "/api/admin/automation-conversion/tasks/run-due", "/api/admin/automation-conversion/executions*"} <= excluded_paths:
        blockers.append("excluded_routes must include detail, run-due, and executions")
    ...
    if not {"phase4bc_daily_followup_task", "phase4bc_audience_entered_task"} <= set(seed.get("task_codes") or []):
        blockers.append("fixture_seed.task_codes incomplete")
    ...
    if list_contract.get("archived_tasks_excluded_by_default") is not True:
        blockers.append("list_contract must exclude archived tasks by default")
    if not {"ok", "tasks", "groups", "side_effect_safety"} <= set(list_contract.get("response_keys") or []):
    ...
    if not {"task_name", "idempotency_key"} <= set(create_contract.get("required_payload") or []):
        blockers.append("create_contract.required_payload must include task_name and idempotency_key")
    for field in ("missing_name_rejected", "invalid_status_rejected", "dangerous_fields_rejected", "execution_fields_rejected"):
    ...
    if state.get("last_merged_pr") != "#659":
        blockers.append("phase_execution_state.last_merged_pr must record #659")
    if state.get("last_attempted_action") != "phase_4bc_tasks_fixture_native_contract_planning":
        blockers.append("phase_execution_state.last_attempted_action must be Phase 4BC")
    if state.get("recommended_next_pr") != "phase_4bd_tasks_fixture_native_implementation_owner_decision":
        blockers.append("phase_execution_state.recommended_next_pr must be Phase 4BD owner decision")
    if set(state.get("next_allowed_actions") or []) != {"phase_4bd_tasks_fixture_native_implementation_owner_decision"}:
        blockers.append("phase_execution_state.next_allowed_actions must be Phase 4BD owner decision")
    ...
    if state_update.get("phase_4bc_completed_step") not in set(state.get("completed_steps") or []):
        blockers.append("phase_execution_state.completed_steps must include Phase 4BC completed step")

    readiness = state.get("tasks_readiness") if isinstance(state.get("tasks_readiness"), dict) else {}
    for field in ("fixture_native_contract_planning_completed", "fixture_native_implementation_requires_owner_decision", "owner_decision_required"):
    ...
    rec = data.get("phase_4bd_recommendation") if isinstance(data.get("phase_4bd_recommendation"), dict) else {}
    if rec.get("recommended_next_step") != "tasks_fixture_native_implementation_owner_decision":
        blockers.append("phase_4bd_recommendation must recommend owner decision before runtime implementation")
    for field in ("production_write_allowed", "production_route_switch_allowed", "fallback_removal_allowed", "production_write_canary_allowed"):
        if rec.get(field) is not False:
            blockers.append(f"phase_4bd_recommendation.{field} must be false")
    ...
    if unexpected:
        blockers.append(f"unexpected changed files for Phase 4BC package: {unexpected}")
...
def _write_outputs(report: dict[str, Any], output_json: str | None, output_md: str | None) -> None:
    ...
    if output_md:
        lines = ["# Phase 4BC Tasks Fixture Native Contract Check", "", f"- overall: {report['overall']}", f"- ok: {str(report['ok']).lower()}", "", "## Blockers", *(f"- {item}" for item in report["blockers"]), "", "## Warnings", *(f"- {item}" for item in report["warnings"])]
