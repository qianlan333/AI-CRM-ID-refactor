...
ADAPTER = ROOT / "aicrm_next/automation_engine/workflow_node_sqlalchemy_repository.py"
HARNESS = ROOT / "tools/run_phase4cc_workflow_nodes_adapter_parity.py"
TEST = ROOT / "tests/test_phase4cc_workflow_nodes_repository_adapter_parity_bundle.py"

WORKFLOW_NODES = "/api/admin/automation-conversion/workflow-nodes*"
TASKS = "/api/admin/automation-conversion/tasks*"
NEXT_BUNDLE = "phase_4cd_tasks_repository_adapter_parity_bundle"
COMPLETED_STEP = "phase_4cc_workflow_nodes_repository_adapter_parity_completed"
REQUIRED_CHANGED = {
...
def build_report() -> dict[str, Any]:
    ...
    if data.get("status") != "phase_4cc_workflow_nodes_repository_adapter_parity_bundle":
        blockers.append("status must be Phase 4CC workflow-nodes repository adapter parity bundle")
    if bundle.get("type") != "repository_adapter_parity_bundle" or bundle.get("route_family") != WORKFLOW_NODES:
        blockers.append("bundle must be repository_adapter_parity_bundle for workflow-nodes")
    ...
    if adapter.get("backend_flag") != "AICRM_WORKFLOW_NODES_REPO_BACKEND":
        blockers.append("adapter backend flag must be route-specific")
    if set(adapter.get("database_url_flags") or []) != {"AICRM_WORKFLOW_NODES_TEST_DATABASE_URL", "AICRM_WORKFLOW_NODES_STAGING_DATABASE_URL"}:
    ...
    if "SqlAlchemyWorkflowNodeRepository" not in adapter_text:
        blockers.append("adapter must define SqlAlchemyWorkflowNodeRepository")
    if "automation_workflow_nodes" not in adapter_text or "automation_workflow_node_idempotency" not in adapter_text or "automation_workflow_node_audit_log" not in adapter_text:
        blockers.append("adapter must cover workflow-node main/idempotency/audit tables")
    if "WORKFLOW_NODE_TEST_DATABASE_URL_ENV" not in repo_text or "WORKFLOW_NODE_STAGING_DATABASE_URL_ENV" not in repo_text:
        blockers.append("repo must expose route-specific workflow-node DB URL flags")
    ...
    if state.get("active_candidate") != TASKS or state_update.get("active_candidate") != TASKS:
        blockers.append("phase state must advance active candidate to tasks")
    if state.get("last_merged_pr") != "#687":
        blockers.append("phase state last_merged_pr must record #687")
    if state.get("last_attempted_action") != "phase_4cc_workflow_nodes_repository_adapter_parity_bundle":
        blockers.append("phase state last_attempted_action must be Phase 4CC")
    if state.get("last_created_pr") != "#688":
        blockers.append("phase state last_created_pr must be #688")
    if state.get("recommended_next_pr") != NEXT_BUNDLE:
        blockers.append("phase state recommended_next_pr must be Phase 4CD")
    if set(state.get("next_allowed_actions") or []) != {NEXT_BUNDLE}:
        blockers.append("phase state next_allowed_actions must be Phase 4CD")
    if COMPLETED_STEP not in set(state.get("completed_steps") or []):
        blockers.append("completed_steps must include Phase 4CC")
    ...
    if unexpected:
        blockers.append(f"unexpected changed files for Phase 4CC: {unexpected}")
...
def _write_outputs(report: dict[str, Any], output_json: str | None, output_md: str | None) -> None:
    ...
    if output_md:
        lines = [
            "# Phase 4CC Workflow Nodes Repository Adapter Parity Bundle Check",
            "",
