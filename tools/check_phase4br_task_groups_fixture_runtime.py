...
DOC = ROOT / "docs/development/phase_4br_task_groups_fixture_runtime.md"
...
DOMAIN = ROOT / "aicrm_next/automation_engine/task_groups.py"
TEST = ROOT / "tests/test_phase4br_task_groups_fixture_runtime.py"
...
REQUIRED_RUNTIME_TERMS = {
    ...
    APP: [
        ...
        "_task_group_production_unavailable_payload",
    ],
    DTO: [
    ...
    REPO: [
        "list_task_groups",
        "create_task_group",
        "list_task_group_audit_events",
    ],
    DOMAIN: [
        "normalize_task_group_create_payload",
        "task_group_projection",
        "task_group_side_effect_safety",
        "reject_dangerous_task_group_fields",
    ],
    TEST: [
        "test_fixture_repository_lists_seeded_task_groups",
...
def build_report() -> dict[str, Any]:
    ...
    if state.get("active_candidate") != WORKFLOWS:
        blockers.append("phase_execution_state.active_candidate must advance to workflows after task-groups runtime slice")
    if state.get("last_merged_pr") != "#674":
        blockers.append("phase_execution_state.last_merged_pr must record #674")
    if state.get("last_attempted_action") != "phase_4br_task_groups_fixture_native_list_create_runtime":
        blockers.append("phase_execution_state.last_attempted_action must be Phase 4BR task-groups runtime")
    if state.get("last_created_pr") != "#675":
        blockers.append("phase_execution_state.last_created_pr must be #675")
    if state.get("recommended_next_pr") != "phase_4bs_workflows_fixture_native_list_create_runtime":
        blockers.append("phase_execution_state.recommended_next_pr must advance to workflows fixture runtime")
    if set(state.get("next_allowed_actions") or []) != {"phase_4bs_workflows_fixture_native_list_create_runtime"}:
        blockers.append("phase_execution_state.next_allowed_actions must be Phase 4BS workflows runtime")
    if "phase_4br_task_groups_fixture_native_list_create_runtime_completed" not in set(state.get("completed_steps") or []):
        blockers.append("completed_steps must include Phase 4BR task-groups runtime")

    readiness = state.get("task_groups_readiness") if isinstance(state.get("task_groups_readiness"), dict) else {}
    for field in (
        "native_contract_planning_completed",
    ...
    if set(readiness.get("implemented_runtime_slices") or []) != {"task_groups_fixture_local_list", "task_groups_fixture_local_metadata_create"}:
        blockers.append("task_groups_readiness.implemented_runtime_slices must include list/create")
    for field in (
    ...
    if unexpected:
        blockers.append(f"unexpected changed files for Phase 4BR task-groups package: {unexpected}")
    ...
    if not REQUIRED_CHANGED <= changed:
        ...
        blockers.append(f"expected changed files missing from diff: {missing}")
...
def _write_outputs(report: dict[str, Any], output_json: str | None, output_md: str | None) -> None:
    ...
    if output_md:
        lines = [
            "# Phase 4BR Task Groups Fixture Runtime Check",
            "",
