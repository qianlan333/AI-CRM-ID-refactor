...
TEST = ROOT / "tests/test_post_phase7_cleanup_task_groups_evidence_refresh.py"
...
REQUIRED_MISSING_EVIDENCE = {
    "route_specific_owner_approval_for_fallback_removal",
    "current_route_specific_shadow_compare_evidence_from_latest_main",
    "executable_rollback_evidence_for_selected_exact_route",
    "assigned_rollback_owner_and_plan",
    "route_ownership_proof_attached_to_cleanup_record",
    "production_compat_exact_entry_cleanup_proof_after_fallback_status_resolved",
}
...
def build_report() -> dict[str, Any]:
    ...
    if data.get("status") != "post_phase7_cleanup_task_groups_evidence_refresh":
        blockers.append("status must be post_phase7_cleanup_task_groups_evidence_refresh")
    if data.get("bundle_type") != "post_phase7_cleanup_task_groups_evidence_refresh_bundle":
        blockers.append("bundle_type must be post_phase7_cleanup_task_groups_evidence_refresh_bundle")
    ...
    if data.get("cleanup_family") != "task_groups_exact_route_cleanup_evidence_refresh":
        blockers.append("cleanup_family must be task_groups_exact_route_cleanup_evidence_refresh")
    ...
    for key in (
        ...
        "phase7g_fallback_removal_blocked",
        "phase7h_production_compat_cleanup_blocked",
        "phase7i_runtime_deletion_blocked_by_retained_fallback_and_compat",
        "no_outbound_send",
        "no_timer_execution",
    ...
    for key in (
        ...
        "route_specific_owner_approval_for_cleanup",
        "shadow_compare_evidence",
        "rollback_owner",
        "rollback_plan",
    ...
    if next_bundle.get("recommended_next_step") != "post_phase7_cleanup_workflow_nodes_evidence_refresh_bundle":
        blockers.append("next_bundle.recommended_next_step must be post_phase7_cleanup_workflow_nodes_evidence_refresh_bundle")

    if state.get("current_phase") != "post_phase7_cleanup_task_groups_evidence_refresh":
        blockers.append("phase_execution_state.current_phase must be task-groups evidence refresh")
    if state.get("active_candidate") != "task_groups_cleanup_evidence_refresh":
        blockers.append("phase_execution_state.active_candidate must be task_groups_cleanup_evidence_refresh")
    if state.get("last_merged_pr") != "#797":
        blockers.append("phase_execution_state.last_merged_pr must record PR #797")
    if state.get("next_allowed_actions") != ["post_phase7_cleanup_workflow_nodes_evidence_refresh_bundle"]:
        blockers.append("phase_execution_state.next_allowed_actions must only recommend workflow-nodes evidence refresh")
    phase_state = _dict(state.get("post_phase7_cleanup_task_groups_evidence_refresh"))
    if phase_state.get("status") != "post_phase7_cleanup_task_groups_evidence_refresh_completed":
        blockers.append("state task-groups evidence refresh status must be completed")
    if phase_state.get("blocked_with_missing_evidence") is not True:
        blockers.append("state must record task-groups blocked_with_missing_evidence true")
    for key in ("fallback_removed", "production_compat_behavior_changed", "wildcard_cleanup", "legacy_runtime_deleted", "delete_ready"):
        if phase_state.get(key) is not False:
            blockers.append(f"state task-groups evidence refresh {key} must be false")

    changed = _changed_files()
    details["changed_files"] = sorted(changed)
    unexpected = sorted(path for path in changed if path not in ALLOWED_CHANGED_FILES)
    if unexpected:
        blockers.append(f"changed files outside task-groups evidence refresh allowlist: {unexpected}")
...
def _write_md(report: dict[str, Any], path: str) -> None:
    lines = ["# Post-Phase 7 Task-Groups Cleanup Evidence Refresh Check", "", f"- overall: {report['overall']}", "- blockers:"]
