...
DOC = ROOT / "docs/development/phase_4bv_agents_fixture_runtime.md"
...
DOMAIN = ROOT / "aicrm_next/automation_engine/agents.py"
TEST = ROOT / "tests/test_phase4bv_agents_fixture_runtime.py"
...
REQUIRED_RUNTIME_TERMS = {
    ...
    APP: [
        "ListAgentsQuery",
        "CreateAgentCommand",
        "_agent_production_unavailable_payload",
    ],
    DTO: [
        "AgentListRequest",
        "AgentCreateRequest",
    ...
    DOMAIN: [
        ...
        "agent_projection",
        "agent_side_effect_safety",
        "reject_dangerous_agent_fields",
    ],
    TEST: [
        "test_fixture_repository_lists_seeded_agents",
...
def _require_bool(readiness: dict[str, Any], blockers: list[str], field: str, expected: bool) -> None:
    if readiness.get(field) is not expected:
        blockers.append(f"agents_readiness.{field} must be {str(expected).lower()}")


def build_report() -> dict[str, Any]:
    blockers: list[str] = []
    warnings: list[str] = []
    for path in (DOC, STATE, RUNBOOK, API, APP, DTO, REPO, DOMAIN, TEST):
        if not path.exists():
    ...
    if state.get("active_candidate") != AGENT_OUTPUTS:
        blockers.append("phase_execution_state.active_candidate must advance to agent-outputs after agents runtime slice")
    if state.get("last_merged_pr") != "#680":
        blockers.append("phase_execution_state.last_merged_pr must record #680")
    if state.get("last_attempted_action") != "phase_4bv_agents_fixture_native_list_create_runtime":
        blockers.append("phase_execution_state.last_attempted_action must be Phase 4BV agents runtime")
    if state.get("last_created_pr") != "#681":
        blockers.append("phase_execution_state.last_created_pr must be #681")
    if state.get("recommended_next_pr") != "phase_4bw_agent_outputs_fixture_native_list_detail_runtime":
        blockers.append("phase_execution_state.recommended_next_pr must advance to agent-outputs fixture runtime")
    if set(state.get("next_allowed_actions") or []) != {"phase_4bw_agent_outputs_fixture_native_list_detail_runtime"}:
        blockers.append("phase_execution_state.next_allowed_actions must be Phase 4BW agent-outputs runtime")
    if "phase_4bv_agents_fixture_native_list_create_runtime_completed" not in set(state.get("completed_steps") or []):
        blockers.append("completed_steps must include Phase 4BV agents runtime")
    ...
    if set(readiness.get("implemented_runtime_slices") or []) != {
        ...
        blockers.append("agents_readiness.implemented_runtime_slices must include list/create")
    if readiness.get("paused_by_pr") != "#681":
    ...
    if unexpected:
        blockers.append(f"unexpected changed files for Phase 4BV agents package: {unexpected}")
    ...
    return {
        ...
        "details": {
            ...
            "route_family": AGENTS,
            "next_route_family": AGENT_OUTPUTS,
...
def _write_outputs(report: dict[str, Any], output_json: str | None, output_md: str | None) -> None:
    ...
    if output_md:
        lines = [
            "# Phase 4BV Agents Fixture Runtime Check",
            "",
