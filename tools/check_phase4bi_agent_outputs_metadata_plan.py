...
def build_report() -> dict[str, Any]:
    ...
    if state.get("last_merged_pr") not in {"#665", "#666", "#667"}:
        blockers.append("phase_execution_state.last_merged_pr must record #665 or later Phase 4BK state #667")
    if state.get("last_attempted_action") not in {
        ...
        blockers.append("phase_execution_state.last_attempted_action must be Phase 4BI or later Phase 4BK")
    if state.get("last_created_pr") not in {"#666", "#667", "#668"}:
        blockers.append("phase_execution_state.last_created_pr must be #666 or later Phase 4BK PR #668")
    if state.get("recommended_next_pr") not in {
        "phase_4bj_agent_outputs_schema_route_surface_confirmation",
        "phase_4bk_agent_outputs_fixture_native_contract_planning",
        "phase_4bl_agent_outputs_fixture_native_implementation_owner_decision",
    }:
        blockers.append("phase_execution_state.recommended_next_pr must be Phase 4BJ or later Phase 4BL")
    if set(state.get("next_allowed_actions") or []) not in (
        {"phase_4bj_agent_outputs_schema_route_surface_confirmation"},
        {"phase_4bk_agent_outputs_fixture_native_contract_planning"},
        {"phase_4bl_agent_outputs_fixture_native_implementation_owner_decision"},
    ):
        blockers.append("phase_execution_state.next_allowed_actions must be Phase 4BJ or later Phase 4BL")
