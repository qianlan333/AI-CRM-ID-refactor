...
ADAPTER = ROOT / "aicrm_next/automation_engine/workflow_sqlalchemy_repository.py"
HARNESS = ROOT / "tools/run_phase4cb_workflows_adapter_parity.py"
TEST = ROOT / "tests/test_phase4cb_workflows_repository_adapter_parity_bundle.py"

WORKFLOWS = "/api/admin/automation-conversion/workflows*"
WORKFLOW_NODES = "/api/admin/automation-conversion/workflow-nodes*"
NEXT_BUNDLE = "phase_4cc_workflow_nodes_repository_adapter_parity_bundle"
COMPLETED_STEP = "phase_4cb_workflows_repository_adapter_parity_completed"
REQUIRED_CHANGED = {
...
def build_report() -> dict[str, Any]:
    ...
    if data.get("status") != "phase_4cb_workflows_repository_adapter_parity_bundle":
        blockers.append("status must be Phase 4CB workflows repository adapter parity bundle")
    bundle = data.get("bundle") if isinstance(data.get("bundle"), dict) else {}
    if bundle.get("type") != "repository_adapter_parity_bundle" or bundle.get("route_family") != WORKFLOWS:
        blockers.append("bundle must be repository_adapter_parity_bundle for workflows")
    ...
    if adapter.get("backend_flag") != "AICRM_WORKFLOWS_REPO_BACKEND":
        blockers.append("adapter backend flag must be route-specific")
    if set(adapter.get("database_url_flags") or []) != {"AICRM_WORKFLOWS_TEST_DATABASE_URL", "AICRM_WORKFLOWS_STAGING_DATABASE_URL"}:
    ...
    if "SqlAlchemyWorkflowRepository" not in adapter_text:
        blockers.append("adapter must define SqlAlchemyWorkflowRepository")
    if "automation_workflows" not in adapter_text or "automation_workflow_idempotency" not in adapter_text or "automation_workflow_audit_log" not in adapter_text:
        blockers.append("adapter must cover workflow main/idempotency/audit tables")
    if "WORKFLOW_TEST_DATABASE_URL_ENV" not in repo_text or "WORKFLOW_STAGING_DATABASE_URL_ENV" not in repo_text:
        blockers.append("repo must expose route-specific workflow DB URL flags")
    ...
    if state.get("active_candidate") != WORKFLOW_NODES or state_update.get("active_candidate") != WORKFLOW_NODES:
        blockers.append("phase state must advance active candidate to workflow-nodes")
    if state.get("last_merged_pr") != "#686":
        blockers.append("phase state last_merged_pr must record #686")
    if state.get("last_attempted_action") != "phase_4cb_workflows_repository_adapter_parity_bundle":
        blockers.append("phase state last_attempted_action must be Phase 4CB")
    if state.get("last_created_pr") != "#687":
        blockers.append("phase state last_created_pr must be #687")
    if state.get("recommended_next_pr") != NEXT_BUNDLE:
        blockers.append("phase state recommended_next_pr must be Phase 4CC")
    if set(state.get("next_allowed_actions") or []) != {NEXT_BUNDLE}:
        blockers.append("phase state next_allowed_actions must be Phase 4CC")
    if COMPLETED_STEP not in set(state.get("completed_steps") or []):
        blockers.append("completed_steps must include Phase 4CB")
    ...
    if unexpected:
        blockers.append(f"unexpected changed files for Phase 4CB: {unexpected}")
...
def _write_outputs(report: dict[str, Any], output_json: str | None, output_md: str | None) -> None:
    ...
    if output_md:
        lines = [
            "# Phase 4CB Workflows Repository Adapter Parity Bundle Check",
            "",
