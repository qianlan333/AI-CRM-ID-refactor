...
ADAPTER = ROOT / "aicrm_next/automation_engine/agent_run_sqlalchemy_repository.py"
HARNESS = ROOT / "tools/run_phase4cg_agent_runs_adapter_parity.py"
TEST = ROOT / "tests/test_phase4cg_agent_runs_repository_adapter_parity_bundle.py"

AGENT_RUNS = "/api/admin/automation-conversion/agent-runs*"
TASK_GROUPS = "/api/admin/automation-conversion/task-groups*"
NEXT_BUNDLE = "phase_4ch_task_groups_staging_readiness_bundle"
COMPLETED_STEP = "phase_4cg_agent_runs_repository_adapter_parity_completed"
REQUIRED_CHANGED = {
...
def build_report() -> dict[str, Any]:
    ...
    if data.get("status") != "phase_4cg_agent_runs_repository_adapter_parity_bundle":
        blockers.append("status must be Phase 4CG agent-runs repository adapter parity bundle")
    if bundle.get("type") != "repository_adapter_parity_bundle" or bundle.get("route_family") != AGENT_RUNS:
        blockers.append("bundle must be repository_adapter_parity_bundle for agent-runs")
    ...
    if adapter.get("backend_flag") != "AICRM_AGENT_RUNS_REPO_BACKEND":
        blockers.append("adapter backend flag must be route-specific")
    if set(adapter.get("database_url_flags") or []) != {"AICRM_AGENT_RUNS_TEST_DATABASE_URL", "AICRM_AGENT_RUNS_STAGING_DATABASE_URL"}:
    ...
    if set(adapter.get("supported_operations") or []) != {"list_agent_runs", "get_agent_run"}:
        blockers.append("adapter supported operations must stay list/detail only")
    if adapter.get("write_operations_added") not in ([], None):
        blockers.append("adapter must not add write operations")
    ...
    if "SqlAlchemyAgentRunRepository" not in adapter_text:
        blockers.append("adapter must define SqlAlchemyAgentRunRepository")
    if "automation_agent_runs" not in adapter_text:
        blockers.append("adapter must read automation_agent_runs")
    if "create_agent_run" in adapter_text or "INSERT INTO automation_agent_runs" in adapter_text:
        blockers.append("adapter must not add agent-run writes")
    if "AGENT_RUN_TEST_DATABASE_URL_ENV" not in repo_text or "AGENT_RUN_STAGING_DATABASE_URL_ENV" not in repo_text:
        blockers.append("repo must expose route-specific agent-run DB URL flags")
    ...
    if state.get("active_candidate") != TASK_GROUPS or state_update.get("active_candidate") != TASK_GROUPS:
        blockers.append("phase state must advance active candidate to task-groups staging readiness")
    if state.get("last_merged_pr") != "#691":
        blockers.append("phase state last_merged_pr must record #691")
    if state.get("last_attempted_action") != "phase_4cg_agent_runs_repository_adapter_parity_bundle":
        blockers.append("phase state last_attempted_action must be Phase 4CG")
    if state.get("last_created_pr") != "#692":
        blockers.append("phase state last_created_pr must be #692")
    if state.get("recommended_next_pr") != NEXT_BUNDLE:
        blockers.append("phase state recommended_next_pr must be Phase 4CH")
    if set(state.get("next_allowed_actions") or []) != {NEXT_BUNDLE}:
        blockers.append("phase state next_allowed_actions must be Phase 4CH")
    if COMPLETED_STEP not in set(state.get("completed_steps") or []):
        blockers.append("completed_steps must include Phase 4CG")
    readiness = state.get("agent_runs_readiness") if isinstance(state.get("agent_runs_readiness"), dict) else {}
    for field in ("repository_adapter_parity_completed", "no_database_url_fallback", "default_backend_fixture_local", "test_db_parity_harness_completed"):
        if readiness.get(field) is not True:
            blockers.append(f"agent_runs_readiness.{field} must be true")
    if readiness.get("repository_adapter_backend_flag") != "AICRM_AGENT_RUNS_REPO_BACKEND":
        blockers.append("agent_runs_readiness.repository_adapter_backend_flag must be route-specific")
    if readiness.get("repository_adapter_test_db_url_flag") != "AICRM_AGENT_RUNS_TEST_DATABASE_URL":
        blockers.append("agent_runs_readiness.repository_adapter_test_db_url_flag must be route-specific")
    if readiness.get("repository_adapter_staging_db_url_flag") != "AICRM_AGENT_RUNS_STAGING_DATABASE_URL":
        blockers.append("agent_runs_readiness.repository_adapter_staging_db_url_flag must be route-specific")
    if readiness.get("idempotency_audit_rollback_scaffold_completed") is not False:
    ...
    paused_entries = state.get("paused_candidates") if isinstance(state.get("paused_candidates"), list) else []
    if not any(
        isinstance(item, dict)
        and item.get("route_family") == AGENT_RUNS
        ...
        for item in paused_entries
    ):
        blockers.append("paused_candidates must record agent-runs repository adapter parity completion for #692")
    ...
    if unexpected:
        blockers.append(f"unexpected changed files for Phase 4CG: {unexpected}")
    ...
    return {
        ...
        "blockers": blockers,
        "warnings": warnings,
        "details": {"changed_files": sorted(changed), "bundle_type": bundle.get("type"), "route_family": AGENT_RUNS},
    }
...
def _write_outputs(report: dict[str, Any], output_json: str | None, output_md: str | None) -> None:
    ...
    if output_md:
        lines = [
            "# Phase 4CG Agent Runs Repository Adapter Parity Bundle Check",
            "",
