...
ADAPTER = ROOT / "aicrm_next/automation_engine/task_group_sqlalchemy_repository.py"
HARNESS = ROOT / "tools/run_phase4ca_task_groups_adapter_parity.py"
TEST = ROOT / "tests/test_phase4ca_task_groups_repository_adapter_parity_bundle.py"

TASK_GROUPS = "/api/admin/automation-conversion/task-groups*"
WORKFLOWS = "/api/admin/automation-conversion/workflows*"
NEXT_BUNDLE = "phase_4cb_workflows_repository_adapter_parity_bundle"
COMPLETED_STEP = "phase_4ca_task_groups_repository_adapter_parity_completed"
REQUIRED_CHANGED = {
...
def build_report() -> dict[str, Any]:
    ...
    if data.get("status") != "phase_4ca_task_groups_repository_adapter_parity_bundle":
        blockers.append("status must be Phase 4CA task-groups repository adapter parity bundle")
    bundle = data.get("bundle") if isinstance(data.get("bundle"), dict) else {}
    if bundle.get("type") != "repository_adapter_parity_bundle" or bundle.get("route_family") != TASK_GROUPS:
        blockers.append("bundle must be repository_adapter_parity_bundle for task-groups")
    ...
    if adapter.get("backend_flag") != "AICRM_TASK_GROUPS_REPO_BACKEND":
        blockers.append("adapter backend flag must be route-specific")
    if set(adapter.get("database_url_flags") or []) != {"AICRM_TASK_GROUPS_TEST_DATABASE_URL", "AICRM_TASK_GROUPS_STAGING_DATABASE_URL"}:
    ...
    for field in (
        ...
        "route_specific_test_db_required",
        "refuse_production_looking_urls",
    ...
    if "SqlAlchemyTaskGroupRepository" not in adapter_text:
        blockers.append("adapter must define SqlAlchemyTaskGroupRepository")
    if "automation_task_groups" not in adapter_text or "automation_task_group_idempotency" not in adapter_text or "automation_task_group_audit_log" not in adapter_text:
        blockers.append("adapter must cover task group main/idempotency/audit tables")
    if "TASK_GROUP_TEST_DATABASE_URL_ENV" not in repo_text or "TASK_GROUP_STAGING_DATABASE_URL_ENV" not in repo_text:
        blockers.append("repo must expose route-specific DB URL flags")
    ...
    if state.get("active_candidate") != WORKFLOWS or state_update.get("active_candidate") != WORKFLOWS:
        blockers.append("phase state must advance active candidate to workflows")
    if state.get("last_merged_pr") != "#685":
        blockers.append("phase state last_merged_pr must record #685")
    if state.get("last_attempted_action") != "phase_4ca_task_groups_repository_adapter_parity_bundle":
        blockers.append("phase state last_attempted_action must be Phase 4CA")
    if state.get("last_created_pr") != "#686":
        blockers.append("phase state last_created_pr must be #686")
    if state.get("recommended_next_pr") != NEXT_BUNDLE:
        blockers.append("phase state recommended_next_pr must be Phase 4CB")
    if set(state.get("next_allowed_actions") or []) != {NEXT_BUNDLE}:
        blockers.append("phase state next_allowed_actions must be Phase 4CB")
    if COMPLETED_STEP not in set(state.get("completed_steps") or []):
        blockers.append("completed_steps must include Phase 4CA")
    ...
    if unexpected:
        blockers.append(f"unexpected changed files for Phase 4CA: {unexpected}")
...
def _write_outputs(report: dict[str, Any], output_json: str | None, output_md: str | None) -> None:
    ...
    if output_md:
        lines = [
            "# Phase 4CA Task Groups Repository Adapter Parity Bundle Check",
            "",
