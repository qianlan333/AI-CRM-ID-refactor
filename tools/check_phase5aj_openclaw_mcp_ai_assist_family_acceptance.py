...
DOC = ROOT / "docs/development/phase_5aj_openclaw_mcp_ai_assist_family_acceptance.md"
PLAN_YAML = ROOT / "docs/development/phase_5aj_openclaw_mcp_ai_assist_family_acceptance.yaml"
STATE = ROOT / "docs/development/phase_execution_state.yaml"
TEST = ROOT / "tests/test_phase5aj_openclaw_mcp_ai_assist_family_acceptance.py"
NEXT_BUNDLE = "phase_5ak_questionnaire_external_submit_contract_fake_stub_bundle"
COMPLETED_STEP = "phase_5aj_openclaw_mcp_ai_assist_family_acceptance_completed"
ALLOWED = {
...
def build_report() -> dict[str, Any]:
    ...
    required = {"phase_5af_contract_fake_stub", "phase_5ag_live_adapter_behind_flag", "phase_5ah_staging_live_canary_evidence", "phase_5ai_production_canary_readiness_tooling"}
    if not required <= stages:
        blockers.append("completed_stages missing OpenClaw/MCP stages")
    matrix = _dict(data.get("capability_matrix"))
    for key in ("adapter_contract_complete", "fake_stub_complete", "live_adapter_behind_flag_complete", "staging_canary_gate_complete", "production_canary_readiness_tooling_complete", "cleanup_runner_complete"):
        if matrix.get(key) is not True:
            blockers.append(f"capability_matrix.{key} must be true")
    for key in ("real_mcp_call_executed", "real_openclaw_call_executed", "real_llm_call_executed", "deepseek_call_executed", "outbound_send_executed", "timer_or_automation_executed", "prompt_leak_detected", "credential_leak_detected", "route_owner_switched", "fallback_removed", "production_compat_changed"):
    ...
    if decision.get("production_canary_passed") is not False:
        blockers.append("production_canary_passed must remain false")
    rollout = _dict(data.get("rollout_boundary"))
    for key in ("wider_rollout_authorized", "production_live_call_default_authorized", "outbound_send_authorized", "timer_execution_authorized", "automation_execution_authorized", "delete_ready"):
        if rollout.get(key) is not False:
            blockers.append(f"rollout_boundary.{key} must be false")
    for key in ("route_owner_switch_deferred", "fallback_removal_deferred", "production_compat_change_deferred"):
    ...
    if state.get("last_merged_pr") != "#748":
        blockers.append("phase_execution_state.last_merged_pr must be #748")
    ...
    for claim in ("real mcp call enabled", "real openclaw call enabled", "real llm call enabled", "deepseek call enabled", "outbound send enabled", "automation execution enabled", "prompt leakage enabled", "credential leakage enabled", "route owner switched", "fallback removed", "production_compat changed", "delete_ready true", "delete_ready: true"):
    ...
    if unexpected:
        blockers.append(f"changed files outside Phase 5AJ allowlist: {unexpected}")
