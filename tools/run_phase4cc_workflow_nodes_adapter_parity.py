...
from aicrm_next.automation_engine.repo import WORKFLOW_NODE_TEST_DATABASE_URL_ENV, build_automation_repository
...
def run_harness() -> dict[str, Any]:
    db_url = str(os.getenv(WORKFLOW_NODE_TEST_DATABASE_URL_ENV) or "").strip()
    if not db_url:
        return {
            "ok": True,
    ...
    repo = build_automation_repository(workflow_node_backend="sqlalchemy", workflow_node_engine=engine)
    rows, total = repo.list_workflow_nodes({"workflow_id": 1, "limit": 20, "offset": 0})
    details.append({"name": "list_workflow_nodes", "status": "passed", "total": total, "row_count": len(rows)})
    payload = {
        "program_id": 9,
        "workflow_id": 9,
        "node_name": "Phase 4CC adapter parity",
        "node_code": "phase4cc_adapter_parity",
        "node_type": "metadata",
        "metadata": {"source": "phase4cc_test_db"},
        "operator": "phase4cc",
    }
    created = repo.create_workflow_node(payload, idempotency_key="phase4cc-create", operator="phase4cc")
    replay = repo.create_workflow_node(payload, idempotency_key="phase4cc-create", operator="phase4cc")
    audit_events = repo.list_workflow_node_audit_events()
    checks = {
        "create_with_idempotency": created.get("idempotent_replay") is False,
        "idempotency_replay": replay.get("idempotent_replay") is True and replay["node"]["id"] == created["node"]["id"],
...
def _write_outputs(report: dict[str, Any], output_json: str | None, output_md: str | None) -> None:
    ...
    if output_md:
        lines = [
            "# Phase 4CC Workflow Nodes Adapter Parity Harness",
            "",
