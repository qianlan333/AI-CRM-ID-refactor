...
DOC = ROOT / "docs/development/phase_4co_task_groups_production_dry_run_readiness_bundle.md"
PLAN_YAML = ROOT / "docs/development/phase_4co_task_groups_production_dry_run_readiness_bundle.yaml"
STATE = ROOT / "docs/development/phase_execution_state.yaml"
RUNNER = ROOT / "tools/run_phase4co_task_groups_production_readonly_dry_run.py"
TEST = ROOT / "tests/test_phase4co_task_groups_production_dry_run_readiness_bundle.py"
TASK_GROUPS = "/api/admin/automation-conversion/task-groups*"
WORKFLOWS = "/api/admin/automation-conversion/workflows*"
COMPLETED_STEP = "phase_4co_task_groups_production_dry_run_readiness_completed"
NEXT_BUNDLE = "phase_4cp_workflows_production_dry_run_readiness_bundle"
REQUIRED_CHANGED = {
...
def build_report() -> dict[str, Any]:
    ...
    if data.get("status") != "phase_4co_task_groups_production_dry_run_readiness_bundle":
        blockers.append("status must be Phase 4CO task-groups production dry-run readiness bundle")
    if bundle.get("type") != "production_read_only_dry_run_readiness_bundle" or bundle.get("route_family") != TASK_GROUPS:
        blockers.append("bundle must be production_read_only_dry_run_readiness_bundle for task-groups")
    ...
    if runtime.get("runner_path") != "tools/run_phase4co_task_groups_production_readonly_dry_run.py":
        blockers.append("runtime runner path is incorrect")
    for flag in (
        "AICRM_PHASE4CO_PRODUCTION_READONLY_DRY_RUN_APPROVED=1",
        "AICRM_PHASE4CO_PRODUCTION_CONFIG_REVIEWED=1",
        "AICRM_TASK_GROUPS_REPO_BACKEND=sqlalchemy",
        "AICRM_TASK_GROUPS_READONLY_DRY_RUN_DATABASE_URL",
        "--read-only",
        "--confirm-no-writes",
    ):
    ...
    forbidden_runner_fragments = (
        ...
        'AICRM_TASK_GROUPS_TEST_DATABASE_URL',
        'AICRM_TASK_GROUPS_STAGING_DATABASE_URL',
        ".create_task_group(",
    )
    ...
    if state.get("last_merged_pr") != "#703":
        blockers.append("phase state last_merged_pr must record #703")
    if state.get("last_attempted_action") != "phase_4co_task_groups_production_dry_run_readiness_bundle":
        blockers.append("phase state last_attempted_action must be Phase 4CO")
    if state.get("last_created_pr") != "#704":
        blockers.append("phase state last_created_pr must be #704")
    if state.get("active_candidate") != WORKFLOWS:
        blockers.append("phase state active_candidate must advance to workflows")
    if state.get("recommended_next_pr") != NEXT_BUNDLE or set(state.get("next_allowed_actions") or []) != {NEXT_BUNDLE}:
        blockers.append("phase state next action must advance to Phase 4CP workflows production dry-run readiness")
    if COMPLETED_STEP not in set(state.get("completed_steps") or []):
        blockers.append("completed_steps must include Phase 4CO")
    ...
    if not any(isinstance(item, dict) and item.get("route_family") == TASK_GROUPS and item.get("slice") == "task_groups_production_readonly_dry_run_readiness" for item in dry_run_slices):
        blockers.append("production_dry_run_readiness_slices must record task-groups readiness")
    task_groups = state.get("task_groups_readiness") if isinstance(state.get("task_groups_readiness"), dict) else {}
    for field in ("production_dry_run_readiness_bundle_completed", "production_readonly_dry_run_runner_completed", "production_readonly_evidence_gate_completed", "production_readonly_blocked_evidence_output_completed"):
        if task_groups.get(field) is not True:
            blockers.append(f"task_groups_readiness.{field} must be true")
    if task_groups.get("production_readonly_dry_run_executed") is not False:
        blockers.append("task_groups_readiness.production_readonly_dry_run_executed must be false")
    if task_groups.get("production_readonly_db_url_flag") != "AICRM_TASK_GROUPS_READONLY_DRY_RUN_DATABASE_URL":
        blockers.append("task_groups_readiness production readonly DB flag mismatch")
    for field in ("production_owner_switch_ready", "production_write_ready", "fallback_removal_ready", "production_repository_route_enablement_ready", "delete_ready"):
        if task_groups.get(field) is not False:
    ...
    if unexpected:
        blockers.append(f"unexpected changed files for Phase 4CO: {unexpected}")
    ...
    return {
        ...
        "blockers": blockers,
        "warnings": warnings,
        "details": {"changed_files": sorted(changed), "bundle_type": bundle.get("type"), "route_family": TASK_GROUPS},
    }
...
def _write_outputs(report: dict[str, Any], output_json: str | None, output_md: str | None) -> None:
    ...
    if output_md:
        lines = [
            "# Phase 4CO Task Groups Production Dry-Run Readiness Bundle Check",
            "",
