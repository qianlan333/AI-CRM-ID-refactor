...
STAGING_RUNNER = ROOT / "tools/run_phase5am_questionnaire_external_submit_staging_canary_evidence.py"
PROD_REVIEW = ROOT / "tools/run_phase5am_questionnaire_external_submit_production_readiness_review.py"
TEST = ROOT / "tests/test_phase5am_questionnaire_external_submit_staging_canary_evidence.py"
NEXT_BUNDLE = "phase_5an_questionnaire_external_submit_production_canary_readiness_bundle"
COMPLETED_STEP = "phase_5am_questionnaire_external_submit_staging_canary_evidence_completed"
...
def _static_blockers(path: Path) -> list[str]:
    ...
    if imports & {"requests", "httpx", "aiohttp", "urllib"}:
        blockers.append(f"{path.relative_to(ROOT)} imports forbidden network modules")
    if calls & {"send", "run_due", "execute_due", "dispatch"}:
        blockers.append(f"{path.relative_to(ROOT)} calls forbidden execution names")
    for token in ("change_production_compat", "switch_route_owner", "enable_oauth_callback_cutover", "write_tag_back_live(", "write_identity_mapping_live("):
        if token in text:
...
def build_report() -> dict[str, Any]:
    ...
    if data.get("route_family") != "/api/h5/questionnaires*":
        blockers.append("route_family must be /api/h5/questionnaires*")
    if data.get("capability_owner") != "aicrm_next.questionnaire":
    ...
    if canary.get("default_blocked") is not True or canary.get("single_submit_attempt_only") is not True:
        blockers.append("staging canary must be default blocked and single submit only")
    for key in ("batch_submit_allowed", "batch_tag_write_allowed"):
        if canary.get(key) is not False:
            blockers.append(f"staging_canary.{key} must be false")
    required_args = {"--execute-staging-canary", "--confirm-live-call", "--confirm-staging-only", "--confirm-approved-target", "--confirm-no-production-write", "--confirm-no-outbound-send", "--idempotency-key", "--slug", "--submission-id"}
    ...
    for key in ("single_submit_attempt_only", "single_approved_target_only", "raw_openid_output_forbidden", "raw_unionid_output_forbidden", "raw_external_userid_output_forbidden", "raw_token_output_forbidden", "raw_secret_output_forbidden"):
        if target.get(key) is not True:
            blockers.append(f"target_safety.{key} must be true")
    for key in ("batch_submit_allowed", "batch_tag_write_allowed", "production_public_submit_write_allowed", "production_identity_write_allowed", "production_tag_write_allowed"):
    ...
    for key in ("production_public_submit_write_executed", "production_identity_write_executed", "production_tag_write_executed", "live_oauth_callback_cutover_executed", "outbound_send_executed"):
    ...
    if state.get("last_merged_pr") != "#751":
        blockers.append("phase_execution_state.last_merged_pr must be #751")
    ...
    for arg in ("--staging-evidence-json", "--confirm-no-production-write", "--confirm-no-production-tag-write", "--confirm-no-outbound-send"):
    ...
    if unexpected:
        blockers.append(f"changed files outside Phase 5AM allowlist: {unexpected}")
