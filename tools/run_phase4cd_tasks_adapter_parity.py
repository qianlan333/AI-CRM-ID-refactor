...
from aicrm_next.automation_engine.repo import TASK_TEST_DATABASE_URL_ENV, build_automation_repository
...
def run_harness() -> dict[str, Any]:
    db_url = str(os.getenv(TASK_TEST_DATABASE_URL_ENV) or "").strip()
    if not db_url:
        return {
            "ok": True,
    ...
    repo = build_automation_repository(task_backend="sqlalchemy", task_engine=engine)
    rows, total = repo.list_tasks({"workflow_id": 1, "limit": 20, "offset": 0})
    details.append({"name": "list_tasks", "status": "passed", "total": total, "row_count": len(rows)})
    payload = {
        "program_id": 9,
        "workflow_id": 9,
        "node_id": 9,
        "group_id": 9,
        "task_name": "Phase 4CD adapter parity",
        "task_code": "phase4cd_adapter_parity",
        "task_type": "metadata",
        "metadata": {"source": "phase4cd_test_db"},
        "operator": "phase4cd",
    }
    created = repo.create_task(payload, idempotency_key="phase4cd-create", operator="phase4cd")
    replay = repo.create_task(payload, idempotency_key="phase4cd-create", operator="phase4cd")
    audit_events = repo.list_task_audit_events()
    checks = {
        "create_with_idempotency": created.get("idempotent_replay") is False,
        "idempotency_replay": replay.get("idempotent_replay") is True and replay["task"]["id"] == created["task"]["id"],
...
def _write_outputs(report: dict[str, Any], output_json: str | None, output_md: str | None) -> None:
    ...
    if output_md:
        lines = [
            "# Phase 4CD Tasks Adapter Parity Harness",
            "",
