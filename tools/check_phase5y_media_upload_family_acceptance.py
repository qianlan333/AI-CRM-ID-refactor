...
TEST = ROOT / "tests/test_phase5y_media_upload_family_acceptance.py"
NEXT_BUNDLE = "phase_5z_payment_commerce_adapter_contract_fake_stub_bundle"
COMPLETED_STEP = "phase_5y_media_upload_family_acceptance_completed"
...
def build_report() -> dict[str, Any]:
    ...
    expected = {"phase_5u_contract_fake_stub", "phase_5v_live_adapter_behind_flag", "phase_5w_staging_live_canary_gate", "phase_5x_production_canary_tooling"}
    actual = {str(item.get("stage")) for item in _list(data.get("completed_stages")) if isinstance(item, dict) and item.get("complete") is True}
    if expected - actual:
        blockers.append(f"completed_stages missing: {sorted(expected - actual)}")
    matrix = _dict(data.get("capability_matrix"))
    for key in ("adapter_contract_complete", "fake_stub_complete", "live_adapter_behind_flag_complete", "staging_canary_gate_complete", "production_canary_tooling_complete", "cleanup_runner_complete"):
        if matrix.get(key) is not True:
            blockers.append(f"capability_matrix.{key} must be true")
    for key in ("production_canary_passed", "route_owner_switched", "fallback_removed", "production_compat_changed", "public_publish_enabled", "destructive_delete_enabled", "batch_upload_enabled", "outbound_send_enabled"):
    ...
    if decision.get("status") not in set(_list(decision.get("allowed_values"))):
        blockers.append("acceptance_decision.status must be allowed")
    if decision.get("production_canary_passed") is not False:
        blockers.append("production_canary_passed must be false")
    rollout = _dict(data.get("rollout_boundary"))
    for key in ("wider_rollout_authorized", "public_publish_authorized", "batch_upload_authorized", "destructive_delete_authorized", "delete_ready"):
    ...
    if next_family.get("route_family") != "/api/admin/wechat-pay*":
        blockers.append("next_family route_family must be /api/admin/wechat-pay*")
    if next_family.get("capability_owner") != "aicrm_next.commerce":
        blockers.append("next_family capability_owner must be aicrm_next.commerce")
    ...
    if state.get("last_merged_pr") != "#737":
        blockers.append("phase_execution_state.last_merged_pr must be #737")
    ...
    for claim in ("wider rollout enabled", "route owner switched", "fallback removed", "production_compat changed", "public media url publication enabled", "destructive delete enabled", "batch upload enabled", "delete_ready true", "delete_ready: true"):
        if claim in doc_text:
            blockers.append(f"doc contains forbidden claim: {claim}")
    changed = _changed_files()
    unexpected = sorted(path for path in changed if path not in ALLOWED_CHANGED_FILES)
    if unexpected:
        blockers.append(f"changed files outside Phase 5Y allowlist: {unexpected}")
