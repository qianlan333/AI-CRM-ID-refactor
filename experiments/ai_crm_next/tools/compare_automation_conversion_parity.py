...
from aicrm_next.automation_engine.parity_spec import (  # noqa: E402
...
def _fetch_http(base_url: str, endpoint_name: str) -> dict[str, Any]:
    ...
    with httpx.Client(timeout=10.0) as client:
        response = client.request(spec.method, url, json=spec.body if spec.method == "POST" else None)
    try:
...
def run_compare(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for endpoint_name in DEFAULT_SAFE_ENDPOINTS:
        ...
        if args.old_fixture_dir:
            ...
            if spec.method != "GET" and not args.allow_old_writes:
                old_result = {"status_code": spec.expected_status, "payload": _load_fixture(Path(args.fixture_fallback_dir), endpoint_name)["payload"]}
            else:
        ...
        issues = []
        issues.extend(compare_status_code(int(old_result["status_code"]), int(next_result["status_code"]), expected_status=spec.expected_status))
    ...
    return {
        ...
        "side_effect_safety": {
            "note": "Default comparison is fixture/fake safe. Do not run write endpoints against old production without explicit approval.",
        },
        "results": results,
    }
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Compare old Flask and AI-CRM Next automation conversion API parity.")
    parser.add_argument("--old-base-url", default="", help="Old Flask AI-CRM base URL. Write endpoints are fixture-protected by default.")
    parser.add_argument("--next-base-url", default="", help="AI-CRM Next base URL.")
    parser.add_argument("--old-fixture-dir", default="", help="Directory containing old automation conversion JSON response fixtures.")
    parser.add_argument("--fixture-fallback-dir", default="tests/fixtures/old_automation_conversion", help="Fixture fallback for old HTTP write endpoints.")
    parser.add_argument("--next-testclient", action="store_true", help="Use AI-CRM Next FastAPI TestClient instead of HTTP.")
    parser.add_argument("--allow-old-writes", action="store_true", help="Dangerous: allow non-GET requests against old-base-url.")
