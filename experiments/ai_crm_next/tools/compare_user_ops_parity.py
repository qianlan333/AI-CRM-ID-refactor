...
from aicrm_next.ops_enrollment.parity_spec import (  # noqa: E402
...
def _endpoint_names(include_write_endpoints: bool) -> list[str]:
    if include_write_endpoints:
        return list(ENDPOINT_SPECS)
    return list(DEFAULT_SAFE_ENDPOINTS)


def run_compare(args: argparse.Namespace) -> dict[str, Any]:
    endpoints = _endpoint_names(args.include_write_endpoints)
    results = []
    ...
    return {
        ...
        "mode": {
            ...
            "include_write_endpoints": args.include_write_endpoints,
        },
        "side_effect_safety": {
            "default_skips_write_endpoints": not args.include_write_endpoints,
            "note": "DND and execute are excluded unless --include-write-endpoints is supplied.",
        },
        "results": results,
    }
...
def write_markdown_report(report: dict[str, Any], path: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        "# User Ops Parity Report",
        "",
        f"- overall: {'PASS' if report['ok'] else 'FAIL'}",
        f"- old mode: {report['mode']['old']}",
        f"- next mode: {report['mode']['next']}",
        f"- include write endpoints: {report['mode']['include_write_endpoints']}",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Compare old Flask and AI-CRM Next User Ops API parity.")
    ...
    parser.add_argument("--old-fixture-dir", default="", help="Directory containing old User Ops JSON response fixtures.")
    parser.add_argument("--next-testclient", action="store_true", help="Use AI-CRM Next FastAPI TestClient instead of HTTP.")
    parser.add_argument("--include-write-endpoints", action="store_true", help="Include dry-run/fake write endpoints such as execute.")
