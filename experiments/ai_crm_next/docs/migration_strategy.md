...
6. Frontend compatibility copy: copy old templates/static into the experiment and wire them to adapter routes. Current status: admin shell templates, `admin_user_ops.html`, `admin_console/*`, and `static/admin_console/*` are copied under `src/aicrm_next/frontend_compat/`.
