...
from tools import seed_old_flask_automation_sample as seed_tool


SAFE_URL = "postgresql://aicrm_old_flask_test:secret@127.0.0.1:5432/aicrm_old_flask_test"


def test_seed_guard_accepts_only_local_old_flask_test_db() -> None:
...
def test_seed_guard_rejects_unsafe_urls(url: str) -> None:
    with pytest.raises(ValueError):
        seed_tool.validate_database_url(url)


def test_seed_default_is_dry_run_without_writes() -> None:
    result = seed_tool.seed_sample(SAFE_URL, apply=False)
    assert result["ok"] is True
    assert result["dry_run"] is True
    assert result["sample"]["external_userid"] == "external_user_masked_automation_001"
    assert "automation_member" in result["tables"]
    assert "automation_workflow_execution" in result["tables"]


def test_seed_tool_does_not_import_old_backend() -> None:
