...
def _normalize_custom_params(value: Any) -> dict[str, Any]:
    source = safe_json_loads(value, default={}) if isinstance(value, str) else value
    if source in (None, ""):
        return {}
...
def _normalize_config_payload(payload: dict[str, Any], *, validate_dns: bool = True) -> dict[str, Any]:
...
def _public_config(config: dict[str, Any] | None) -> dict[str, Any]:
    ...
    return {
        ...
        "webhook_url": _normalized_text(config.get("webhook_url")),
        "push_type": _normalized_text(config.get("push_type")),
...
def get_product_external_push_config(product_id: int) -> dict[str, Any]:
    if not product_repo.get_product_by_id(int(product_id)):
        raise ExternalPushError("商品不存在")
    return _public_config(repo.get_product_config(int(product_id)))


def save_product_external_push_config(product_id: int, payload: dict[str, Any], *, operator: str = "") -> dict[str, Any]:
    if not product_repo.get_product_by_id(int(product_id)):
        raise ExternalPushError("商品不存在")
    normalized = _normalize_config_payload(payload)
    saved = repo.upsert_product_config(int(product_id), normalized, operator=operator)
...
def _attempt_delivery(delivery: dict[str, Any], *, config: dict[str, Any], payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    headers = {
        ...
        "X-AICRM-Event": _normalized_text(delivery.get("event_type")) or EVENT_TRANSACTION_PAID,
        "X-AICRM-Delivery-Id": delivery_id,
        "X-AICRM-Timestamp": timestamp,
    }
    secret = _normalized_text(config.get("secret"))
    if secret:
        headers["X-AICRM-Signature"] = sign_webhook_payload(secret, timestamp, raw_body)
    else:
        headers["X-AICRM-Signature"] = ""
    ...
    try:
        status_code, response_body, final_url = _send_http_post(
            ...
            raw_body=raw_body,
            headers=headers,
            timeout=float(current_app.config.get("EXTERNAL_PUSH_WEBHOOK_TIMEOUT_SECONDS", 5) or 5),
        )
        ...
        updated = repo.update_delivery_result(
            delivery_id,
            status=status,
...
def enqueue_transaction_paid_event(order: dict[str, Any]) -> dict[str, Any] | None:
    product = _product_for_order(order)
    payload = {
    ...
    return repo.insert_outbox_event(
        tenant_id=DEFAULT_TENANT_ID,
...
def process_transaction_paid_outbox(outbox: dict[str, Any]) -> dict[str, Any]:
    ...
    order = wechat_pay_repo.get_admin_order_by_id(order_id)
    if not order:
        repo.mark_outbox_status(int(outbox["id"]), status="skipped")
        return {"ok": True, "skipped": True, "reason": "order_not_found"}
    product = _product_for_order(order)
    product_id = int(product.get("id") or 0)
    config = repo.get_product_config(product_id)
    if not config:
        repo.mark_outbox_status(int(outbox["id"]), status="skipped")
        return {"ok": True, "skipped": True, "reason": "config_not_found"}
    config_with_secret = repo.get_config_with_secret(int(config["id"])) or config
    ...
    delivery = repo.create_delivery_once(
        {
            "tenant_id": DEFAULT_TENANT_ID,
    ...
    if _normalized_text(delivery.get("status")) == "success":
        repo.mark_outbox_status(int(outbox["id"]), status="success")
    ...
    result = _attempt_delivery(delivery, config=config_with_secret, payload=push_payload)
    # Delivery retries are tracked on external_push_delivery. Once an outbox
    # event has materialized its delivery, the outbox item is complete and can
    # stop being scanned on every worker pass.
    repo.mark_outbox_status(int(outbox["id"]), status="success")
    return result


def run_due_external_push_events(*, limit: int = 20) -> dict[str, Any]:
    events = repo.list_due_outbox_events(limit=limit)
    results = [process_transaction_paid_outbox(event) for event in events]
    return {
        "ok": True,
...
def run_due_external_push_retries(*, limit: int = 20) -> dict[str, Any]:
    deliveries = repo.list_due_deliveries(limit=limit)
    results = []
    for delivery in deliveries:
        config = repo.get_config_with_secret(int(delivery.get("config_id") or 0)) or {}
        results.append(_attempt_delivery(delivery, config=config))
    return {"ok": True, "retried_count": len(results), "items": results}


def send_webhook_delivery(delivery_id: str) -> dict[str, Any]:
    delivery = repo.get_delivery_by_delivery_id(delivery_id)
    if not delivery:
        raise ExternalPushError("外推记录不存在")
    config = repo.get_config_with_secret(int(delivery.get("config_id") or 0)) or {}
    return _attempt_delivery(delivery, config=config)
...
def send_product_external_push_test(product_id: int) -> dict[str, Any]:
    ...
    if not product:
        raise ExternalPushError("商品不存在")
    config = repo.get_product_config(int(product_id))
    if not config:
    ...
    try:
        ...
        raise ExternalPushError(str(exc)) from exc
    delivery = repo.create_test_delivery(
        {
            "tenant_id": DEFAULT_TENANT_ID,
            "config_id": int(config["id"]),
            "delivery_id": repo.generate_delivery_id(),
    ...
    return _attempt_delivery(delivery, config=config_with_secret, payload=payload)


def list_order_deliveries(order_id: int) -> list[dict[str, Any]]:
    if not wechat_pay_repo.get_admin_order_by_id(int(order_id)):
        raise ExternalPushError("订单不存在")
    return repo.list_deliveries_for_order(int(order_id))


def retry_order_delivery(order_id: int, delivery_id: str) -> dict[str, Any]:
    order = wechat_pay_repo.get_admin_order_by_id(int(order_id))
    if not order:
        raise ExternalPushError("订单不存在")
    delivery = repo.get_delivery_by_delivery_id(delivery_id)
