      ...
      "items": [
        {"workflow_name": "意向高-跟进流", "execution_count": 120, "latest_execution_at": "2026-04-27 09:00:00"}
                ...
                },
                {
                    ...
                    "method": "POST",
                    "path": "/admin/automation-conversion/reply-monitor/toggle",
                    "summary": "切换自动接话监控开关",
                    "description": "开启或关闭自动接话监控。enabled=1 为开启，enabled=0 为关闭。",
                    "auth": "session",
                    "params": [
                    ...
                    "request_example": 'POST /admin/automation-conversion/reply-monitor/toggle\nContent-Type: application/x-www-form-urlencoded\n\nadmin_action_token=TOKEN&enabled=1',
                    "response_example": '{"ok": true, "message": "监控已开启"}',
                },
                {
                    "id": "post-reply-monitor-capture",
                    "method": "POST",
                    "path": "/admin/automation-conversion/reply-monitor/capture",
                    "summary": "立即扫描新消息",
                    "description": "手动触发一次自动接话扫描，将最新未处理消息抓取入队。正常情况由定时任务自动执行。",
                    "auth": "session",
                    "params": [
                        {"name": "admin_action_token", "type": "string", "required": True, "description": "操作令牌"},
                    ],
                    "request_example": 'POST /admin/automation-conversion/reply-monitor/capture\n\nadmin_action_token=TOKEN',
                    "response_example": '{"ok": true, "status": "captured", "message": "扫描完成，新增 3 条入队"}',
                },
                {
                    "id": "post-reply-monitor-run-due",
                    "method": "POST",
                    "path": "/admin/automation-conversion/reply-monitor/run-due",
                    "summary": "立即放行到期队列",
                    "description": "手动触发放行逻辑，处理当前到期的自动接话队列，生成 Agent 话术输出。",
                    "auth": "session",
                    "params": [
                        {"name": "admin_action_token", "type": "string", "required": True, "description": "操作令牌"},
                    ],
                    "request_example": 'POST /admin/automation-conversion/reply-monitor/run-due\n\nadmin_action_token=TOKEN',
