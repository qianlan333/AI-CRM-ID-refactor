...
function customerSessionKey(externalUserId) {
  return `customer-profile-live-tags:${externalUserId || ""}`;
}
...
function renderQuestionnaireAnswers(payload) {
  ...
  if (!stateNode || !wrapNode || !bodyNode) {
    return;
  }
...
function loadLiveTags(root) {
  const externalUserId = root.dataset.externalUserid || "";
  const url = root.dataset.tagsUrl;
  const cacheKey = customerSessionKey(externalUserId);
  ...
  return requestJson(url)
    .then((payload) => {
      if (!payload || payload.ok === false) {
        throw new Error((payload && payload.error) || "当前无法加载实时标签");
      }
      setSessionValue(cacheKey, JSON.stringify(payload));
      renderLiveTags(payload);
      return payload;
    })
    .catch((error) => {
...
function loadQuestionnaireAnswers(root) {
  return requestJson(root.dataset.questionnaireUrl)
    .then((payload) => {
      if (!payload || payload.ok === false) {
        throw new Error((payload && payload.error) || "当前无法加载问卷记录");
      }
    ...
    .catch((error) => {
      showSectionError(
        document.querySelector("[data-profile-questionnaire-state]"),
        error.message || "当前无法加载问卷记录",
      );
      return null;
    });
}
...
function loadMessages(root, fetchAll) {
  const baseUrl = root.dataset.messagesUrl;
  ...
  return requestJson(url.toString())
    .then((payload) => {
      if (!payload || payload.ok === false) {
        throw new Error((payload && payload.error) || "当前无法加载聊天记录");
      }
