...
Questionnaire old Flask readonly route registrations are retired/tombstoned in D5; Questionnaire admin readonly, public readonly, and result readonly ownership now lives in `aicrm_next/questionnaire` plus `aicrm_next/frontend_compat`. Legacy submit, OAuth, admin write, diagnostics, and external-push fallback files remain dependencies and are not part of D5 deletion.
