...
| `aicrm_next/automation_engine/customer_webhooks.py` | customer webhook retry/retry-due planning | Customer automation webhook delivery requested/retried | none | customer webhook runtime gates | side-effect plan IDs only | none | side-effect plan / External Effect planning | `external_effect_only` | covered by legacy customer webhook retry marker for admin path | No | P2 | Consider `customer_webhook.delivery_requested` only if Push Center needs canonical event fan-out. |
| `aicrm_next/automation_engine/timers.py`; `aicrm_next/automation_engine/workspace_runtime.py`; `aicrm_next/automation_engine/member_actions.py`; `aicrm_next/cloud_orchestrator/run_due.py` | run-due planners and member actions | automation due run / member state transition / outbound dispatch planned | none | runtime-specific dry-run/plan gates | side-effect plan IDs | none | side-effect plans and command bus only | `needs_product_decision` | no | No | P2 | Treat automation runtime as separate P1/P2 event family if auto-execute observability is required. |
| `aicrm_next/automation_engine/automation_program_admission.py` | automation program entry publish / operation task scheduling | automation program published; operation task queued | none | program runtime gates | broadcast job idempotency for task rows | none | inserts `broadcast_jobs`; creates side-effect plan metadata | `event_missing` | no | No | P2 | Candidate events: `automation_program.published`, `operation_task.queued`; otherwise document as queue-native out of scope. |
