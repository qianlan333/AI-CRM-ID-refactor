...
### 4. run_due preview 需要 internal token

部分生产复验因缺 `AUTOMATION_INTERNAL_API_TOKEN` 只能验证到 HTTP 401
`internal_token_required`，再结合 diagnostics 的 pair allowlist counters 证明阻断。

后续正式 P1 验收应准备：

```bash
AUTOMATION_INTERNAL_API_TOKEN=<approved internal token>
```
