...
`scripts/run_external_push_worker.py` is the system runner for product-payment external webhooks. Payment success enqueues `transaction.paid` into `domain_event_outbox`; this worker turns due outbox rows into `external_push_delivery` attempts and retries failed deliveries on their scheduled retry time.
