...
- 旧 `overview`、`operations`、`flow-design`、`member-ops`、`operations/executions` 入口已 302 到默认方案
- `agent-config` 已兼容跳到 `/admin/automation-conversion/shared/agents`
- `run-center` 已兼容跳到 `/admin/automation-conversion/runtime`
- `automation_workflow` 和 `automation_workflow_execution` 已加入 `program_id`，历史数据 backfill 到默认方案
- `auto-reply`、reply monitor、router callbacks、message activity sync、run-center runtime/log/debug/sync 仍保持模块级运行时能力
...
- `/admin/automation-conversion/overview`
  - 当前由 `admin_automation_conversion_overview()` 渲染
  - 读取 `get_overview_payload()`、dashboard、消息活跃同步摘要、reply monitor 摘要等
  - Phase 1 方案内只承载和该方案相关的摘要，仍可从默认方案兼容读取

- `/admin/automation-conversion/operations`
  - 当前由 `admin_automation_conversion_operations()` 渲染
...
- `/admin/automation-conversion/operations/executions`
  - 执行记录
  - 如果产品上不单独出二级导航，可先并入“运营编排”下的 execution tab
...
- 共享资源
  ...
  - Phase 1 可以先把旧 `/admin/automation-conversion/agent-config` 兼容到共享资源页
...
- 运行时中心
  ...
  - Phase 1 可以先把旧 `/admin/automation-conversion/run-center` 兼容到 runtime
...
- `GET /admin/automation-conversion/programs/<program_id>/operations/executions`
  - 方案执行记录
...
Phase 1 可以不一次拆完页面，只要明确旧 `agent-config` 是共享资源，不是方案内页面。
...
Phase 1 可以继续复用当前 `run-center` 模板，只需产品口径明确其是模块级运行时中心。

### 旧路由兼容跳转策略

旧路由不删除，先 302 到默认方案或共享/运行时入口：

- `/admin/automation-conversion/overview`
  - 302 到 `/admin/automation-conversion/programs/<default_program_id>/overview`

- `/admin/automation-conversion/operations`
  - 302 到 `/admin/automation-conversion/programs/<default_program_id>/operations`

- `/admin/automation-conversion/operations/workflows/new`
  - 302 到 `/admin/automation-conversion/programs/<default_program_id>/operations/workflows/new`

- `/admin/automation-conversion/operations/workflows/<workflow_id>/edit`
  - 302 到默认方案对应 workflow 编辑页

- `/admin/automation-conversion/operations/workflows/<workflow_id>/nodes`
  - 302 到默认方案对应 workflow 节点页

- `/admin/automation-conversion/operations/executions`
  - 302 到默认方案执行记录

- `/admin/automation-conversion/flow-design`
  - 302 到默认方案 flow-design

- `/admin/automation-conversion/member-ops`
  - 302 到默认方案 member-ops

- `/admin/automation-conversion/agent-config`
  - 302 到 `/admin/automation-conversion/shared/agents`

- `/admin/automation-conversion/auto-reply`
  - 302 到 `/admin/automation-conversion/runtime/reply-monitor`，或 Phase 1 暂时保持旧页面但标记模块级

- `/admin/automation-conversion/run-center`
  - 302 到 `/admin/automation-conversion/runtime`
...
- 初始数据库自动有默认方案
- 默认方案可进入 overview
- 旧 `/admin/automation-conversion/overview` 302 到默认方案 overview
- 旧 `/admin/automation-conversion/operations` 302 到默认方案 operations
- 旧 `/admin/automation-conversion/flow-design` 302 到默认方案 flow-design
- 旧 `/admin/automation-conversion/member-ops` 302 到默认方案 member-ops
- 共享资源和运行时旧入口仍可访问或兼容跳转
...
旧方案内页面跳默认方案；共享和运行时页面跳模块级共享资源或运行时中心。
