...
| MCP / OpenClaw | `aicrm_next.integration_gateway` | context/tool reads via Next boundary | `openclaw_service/` and `legacy_flask/openclaw_legacy/` physically removed after explicit owner approval | MCP/OpenClaw fake contracts accepted; real external calls blocked; new runtime imports frozen; server OpenClaw-named jobs removed with backup | openclaw_shim_physical_deletion_recorded | keep D7.7 adapter boundary; do not reintroduce OpenClaw legacy imports |
| D8 Legacy Shell | `aicrm_next.main` default plus explicit fallback | readonly owner migration already staged | `legacy_flask_app.py`, `wecom_ability_service/`, and legacy maintenance commands retained; `openclaw_service/` is absent after D9.6 and is not a D8 fallback owner | D8.2 preflight only; no runtime lockdown guard or archive package restored | lockdown_preflight_ready | separate approval before any runtime guard, archive package, command removal, or DB migration implementation planning |

Production traffic has not been cut by D9.6. Real external adapters remain disabled and production DB migration is not executed. The OpenClaw shim/archive paths were physically removed after explicit owner approval; D7.7 adapter boundary remains the retained architecture path.
