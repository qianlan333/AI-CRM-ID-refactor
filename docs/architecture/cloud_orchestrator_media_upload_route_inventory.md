...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `/admin/cloud-orchestrator/campaigns` | `wecom_ability_service/templates/admin_console/cloud_campaigns_workspace.html` | Deprecated legacy campaign step image upload bridge; current inline editor primarily uses image library picker | `/api/admin/cloud-orchestrator/media/upload` | POST | multipart `image` | `api_cloud_orchestrator_media_upload` | `UploadCloudOrchestratorMediaCommand` | `wecom.media.upload`, `adapter_mode=production`, real WeCom image upload only | `tests/test_cloud_orchestrator_media_upload_frontend_contract.py` |
