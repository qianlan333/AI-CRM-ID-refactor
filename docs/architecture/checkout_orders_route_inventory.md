...
Legacy Exit group 28 closes the public checkout and order API wildcard rollback. The exact Next routes are owned by `aicrm_next.commerce.api`; unknown checkout/order child paths are controlled Next 404/410 responses. Provider notify/return, admin payment, and H5 payment wildcards stay out of scope.
...
| Payment provider | WeChat Pay notify / fallback family | `/api/wechat-pay/*` | mixed | out of scope | out of scope | fake/real_blocked | not changed in this group | retained out of scope | `/api/wechat-pay/smoke` still routes to production_compat |
| Payment provider | Alipay notify / return / fallback family | `/api/alipay/*` | mixed | out of scope | out of scope | fake/real_blocked | not changed in this group | retained out of scope | provider notify/return not removed |
...
- Provider notify/return, admin payment, H5 payment, real WeChat Pay, real Alipay, and real callbacks remain out of scope for this group.
