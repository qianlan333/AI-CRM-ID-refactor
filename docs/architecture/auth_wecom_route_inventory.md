...
| `/auth/wecom/start` | `GET`, `OPTIONS` | Admin login docs/tests: `docs/admin_auth_rbac.md`, `wecom_ability_service/domains/admin_api_docs/service.py`, `tests/test_admin_slim_phase1.py`, `tests/test_admin_mcp_console.py`, `tests/test_admin_jobs_console.py`, `tests/test_automation_program_phase1.py`, `tests/automation_channel_admission_helpers.py` | `next_native` default blocked / gated real flow | `next_native` default blocked / gated real flow | Admin WeCom SSO start | high / gated_real | Default JSON response remains blocked; HTML blocked state redirects back to `/login`; real authorize URL only when `AICRM_WECOM_ADMIN_AUTH_ENABLE_REAL=true` and required config exists | `deletion_locked` | `tests/test_auth_wecom_exact_routes.py`, `tests/test_auth_wecom_no_real_external_calls.py`, `tests/test_auth_wecom_real_flow.py`, `tests/test_auth_wecom_registry_lifecycle.py` |
