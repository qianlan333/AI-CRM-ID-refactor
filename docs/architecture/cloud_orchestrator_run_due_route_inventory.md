...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| cron / timer caller | `scripts/run_campaign_scheduler.py` documents the legacy scheduler entry; no Next page caller | Plan due campaign delivery | `/api/admin/cloud-orchestrator/campaigns/run-due` | POST | `api_plan_cloud_campaign_run_due` | `PlanCloudCampaignRunDueCommand` | No real scheduler, no campaign runtime, no automation runtime, no WeCom send | `cloud_orchestrator.campaign.run_due`, `adapter_mode=real_blocked`, `requires_approval=true`, blocked ExternalCallAttempt | `deletion_locked`, `legacy_fallback_allowed=false` | POST run-due 200, `source_status=next_run_due_plan` |
