...
Legacy Exit group 18 moves Cloud Orchestrator campaign read/workspace surfaces to Next exact routes. This group does not execute campaigns, does not run run-due, does not send WeCom messages, and does not run the automation runtime.
...
Read/workspace GET routes are Next exact primary with `legacy_fallback_allowed=true` retained only as rollback during validation. Campaign write, step mutation, batch-start, and run-due routes stay active/out-of-scope in production_compat and are not marked `deletion_locked` in this group.
