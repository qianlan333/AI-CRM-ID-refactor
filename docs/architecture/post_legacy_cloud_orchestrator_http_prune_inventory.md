...
| Tests / monkeypatch | `tests/integration/test_pg_compat_smoke.py` previously monkeypatched `wecom_ability_service.http.cloud_orchestrator_campaigns`; legacy split route module assertions have been retired; `tests/test_campaign_hard_delete.py` used legacy Flask Cloud endpoints. | Migrated to Next API / Next command boundary and domain service tests. |
...
| --- | --- | --- | --- | --- | --- |
| `wecom_ability_service/http/cloud_orchestrator_endpoint.py` | legacy route aggregator for Cloud pages, plans, campaigns, segments, media | `aicrm_next.cloud_orchestrator.api`, `aicrm_next.post_legacy_deferred.api` | Removed from `wecom_ability_service/http/__init__.py`; no runtime import remains. | `deleted` | `tests/test_production_route_resolution.py`, `tests/test_post_legacy_cloud_orchestrator_legacy_handlers_removed.py` |
...
| `wecom_ability_service/http/cloud_orchestrator_plans.py` | plan, audit, observability handlers | `aicrm_next.cloud_orchestrator.api`, `aicrm_next.post_legacy_deferred.api` | Audit/observability are Next deferred API; plans remain Next-owned. | `deleted` | `tests/test_post_legacy_deferred_api_routes.py`, `tests/test_post_legacy_baseline_still_green.py` |
| `wecom_ability_service/http/cloud_orchestrator_segments.py` | segment list/detail/preview handlers | retained domain tests and Next ownership where applicable | Only imported by the retired aggregator. | `deleted` | `tests/test_production_route_resolution.py` |
