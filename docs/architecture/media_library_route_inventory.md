...
| Cloud Orchestrator upload bridge | Next primary with production_compat rollback retained | media upload fallback | `/api/admin/cloud-orchestrator/media/upload` | POST/OPTIONS | `aicrm_next.cloud_orchestrator.api` | guarded Next media upload adapter | no real WeCom media upload; out of group 16 execution scope | `tests/test_cloud_orchestrator_media_upload_route_precedence.py` |
