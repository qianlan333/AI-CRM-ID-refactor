...
1. Campaign orchestration uses `campaigns`, `campaign_segments`, `campaign_steps`, and `campaign_members`.
   These tables are active because cloud-orchestrator read/write routes, run-due, and AI Assist external campaign creation still use them.
...
- Do not delete `campaigns` or `campaign_members` while run-due and AI Assist creation depend on them.
