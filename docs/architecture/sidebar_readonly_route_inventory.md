...
Status: Legacy Exit group 4 Next-native readonly replacement and seal.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `/api/sidebar/customer-context` | GET | read | `aicrm_next.customer_read_model` | `aicrm_next.customer_read_model` | customer read model, recent messages projection, timeline projection | none | Keep exact Next route; add stable diagnostics | readonly sealed; no legacy fallback | `tests/test_sidebar_readonly_next_native.py` |
| `/api/sidebar/profile` | GET | read | not registered before group 4 | `aicrm_next.customer_read_model` | customer read model profile query | none | Add exact Next route alias for sidebar profile reads | readonly sealed; no legacy fallback | `tests/test_sidebar_readonly_next_native.py` |
| `/api/sidebar/tags` | GET | read | not registered before group 4 | `aicrm_next.customer_read_model` | customer read model tag projection | none | Add exact Next route alias for sidebar tag reads | readonly sealed; no legacy fallback | `tests/test_sidebar_readonly_next_native.py` |
| `/api/sidebar/binding-status` | GET | read | not registered before group 4 | `aicrm_next.identity_contact` | identity/contact binding read model, customer read model in production | none | Add exact Next route alias for binding reads | readonly sealed; no legacy fallback | `tests/test_sidebar_readonly_next_native.py` |
| `/api/sidebar/contact-binding-status` | GET | read | `aicrm_next.identity_contact` | `aicrm_next.identity_contact` | identity/contact binding read model, customer read model in production | none | Keep exact Next route; add stable diagnostics | readonly sealed; no legacy fallback | `tests/test_sidebar_readonly_registry_lifecycle.py` |
| `/api/sidebar/lead-pool/status` | GET | read | `aicrm_next.customer_read_model` route with legacy sidebar facade source | `aicrm_next.customer_read_model` | customer read model class status and sidebar context projection | none | Replace legacy read facade with Next read model derived status | readonly sealed; no legacy fallback | `tests/test_sidebar_readonly_next_native.py` |
| `/api/sidebar/signup-tags/status` | GET | read | `aicrm_next.customer_read_model` route with legacy sidebar facade source | `aicrm_next.customer_read_model` | customer read model class status, tags, marketing profile | none | Replace legacy read facade with Next read model derived status | readonly sealed; no legacy fallback | `tests/test_sidebar_readonly_next_native.py` |
| `/api/sidebar/marketing-status` | GET | read | `aicrm_next.customer_read_model` route with legacy sidebar facade source | `aicrm_next.customer_read_model` | customer read model marketing summary/profile and class status | none | Replace legacy read facade with Next read model derived status | readonly sealed; no legacy fallback | `tests/test_sidebar_readonly_next_native.py` |
...
- `aicrm_next/customer_read_model/api.py` does not import `legacy_sidebar_read_facade` for the group 4 readonly routes.
