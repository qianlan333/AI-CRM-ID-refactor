# Phase 4CN Agent Runs Staging Readiness Bundle
...
- Staging smoke plan for `/api/admin/automation-conversion/agent-runs*`.
...
- Live external calls.
- Timer, workflow, task, run creation, run execution, replay, orchestration, LLM, DeepSeek, OpenClaw/MCP, or outbound-send execution.
...
This bundle adds a staging readiness runner only. By default it does not connect to a DB, does not execute staging smoke, and does not write staging data. It returns blocked evidence until `AICRM_AGENT_RUNS_STAGING_DATABASE_URL`, `AICRM_AGENT_RUNS_REPO_BACKEND=sqlalchemy`, and `AICRM_PHASE4CN_STAGING_SMOKE_APPROVED=1` are explicitly present.
...
- `python3 tools/check_phase4cn_agent_runs_staging_readiness_bundle.py --output-md /tmp/phase4cn_agent_runs_staging_readiness_bundle.md --output-json /tmp/phase4cn_agent_runs_staging_readiness_bundle.json`
- `python3 tools/run_phase4cn_agent_runs_staging_readiness.py --output-md /tmp/phase4cn_agent_runs_staging_readiness.md --output-json /tmp/phase4cn_agent_runs_staging_readiness.json`
...
- `python3 -m pytest tests/test_autonomous_development_loop.py tests/test_automerge_eligibility.py tests/test_codex_autopilot_runtime_contract.py tests/test_phase4cn_agent_runs_staging_readiness_bundle.py -q`
- `python3 -m py_compile tools/check_phase4cn_agent_runs_staging_readiness_bundle.py tools/run_phase4cn_agent_runs_staging_readiness.py tools/check_autonomous_development_loop.py tools/check_automerge_eligibility.py tools/run_codex_autopilot_tick.py`
...
This gives operators an explicit, repeatable staging gate for agent runs after fixture/native runtime and adapter parity are complete. It prevents missing staging configuration from being interpreted as readiness while moving Phase 4 toward staging evidence without production risk.
...
No production write by default. No external calls by default. No timer, workflow, task, run creation, run execution, replay, orchestration, LLM, DeepSeek, OpenClaw/MCP, or outbound-send execution by default. No production owner switch, fallback removal, deploy config change, destructive migration, canary approval, or delete-ready approval.
...
Proceed to `phase_4co_task_groups_production_dry_run_readiness_bundle` for `/api/admin/automation-conversion/task-groups*`, keeping production read-only dry-run behavior disabled by default with blocked evidence when production dry-run config is absent.
