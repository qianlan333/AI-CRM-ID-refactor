# Phase 4CH Task Groups Staging Readiness Bundle
...
- Staging smoke plan for `/api/admin/automation-conversion/task-groups*`.
...
This bundle adds a staging readiness runner only. By default it does not connect to a DB, does not execute staging smoke, and does not write staging data. It returns blocked evidence until `AICRM_TASK_GROUPS_STAGING_DATABASE_URL`, `AICRM_TASK_GROUPS_REPO_BACKEND=sqlalchemy`, and `AICRM_PHASE4CH_STAGING_SMOKE_APPROVED=1` are explicitly present.
...
- `python3 tools/check_phase4ch_task_groups_staging_readiness_bundle.py --output-md /tmp/phase4ch_task_groups_staging_readiness_bundle.md --output-json /tmp/phase4ch_task_groups_staging_readiness_bundle.json`
- `python3 tools/run_phase4ch_task_groups_staging_readiness.py --output-md /tmp/phase4ch_task_groups_staging_readiness.md --output-json /tmp/phase4ch_task_groups_staging_readiness.json`
...
- `python3 -m pytest tests/test_autonomous_development_loop.py tests/test_automerge_eligibility.py tests/test_codex_autopilot_runtime_contract.py tests/test_phase4ch_task_groups_staging_readiness_bundle.py -q`
...
This gives operators an explicit, repeatable staging gate for task-groups after fixture/native runtime and adapter parity are complete. It prevents missing staging configuration from being interpreted as readiness while moving Phase 4 toward staging evidence without production risk.
...
Proceed to `phase_4ci_workflows_staging_readiness_bundle` for `/api/admin/automation-conversion/workflows*`, keeping staging smoke behavior disabled by default with blocked evidence when staging config is absent.
