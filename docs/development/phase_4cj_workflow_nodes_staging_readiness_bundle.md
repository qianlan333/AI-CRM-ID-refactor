# Phase 4CJ Workflow Nodes Staging Readiness Bundle
...
- Staging smoke plan for `/api/admin/automation-conversion/workflow-nodes*`.
...
This bundle adds a staging readiness runner only. By default it does not connect to a DB, does not execute staging smoke, and does not write staging data. It returns blocked evidence until `AICRM_WORKFLOW_NODES_STAGING_DATABASE_URL`, `AICRM_WORKFLOW_NODES_REPO_BACKEND=sqlalchemy`, and `AICRM_PHASE4CJ_STAGING_SMOKE_APPROVED=1` are explicitly present.
...
- `python3 tools/check_phase4cj_workflow_nodes_staging_readiness_bundle.py --output-md /tmp/phase4cj_workflow_nodes_staging_readiness_bundle.md --output-json /tmp/phase4cj_workflow_nodes_staging_readiness_bundle.json`
- `python3 tools/run_phase4cj_workflow_nodes_staging_readiness.py --output-md /tmp/phase4cj_workflow_nodes_staging_readiness.md --output-json /tmp/phase4cj_workflow_nodes_staging_readiness.json`
...
- `python3 -m pytest tests/test_autonomous_development_loop.py tests/test_automerge_eligibility.py tests/test_codex_autopilot_runtime_contract.py tests/test_phase4cj_workflow_nodes_staging_readiness_bundle.py -q`
...
This gives operators an explicit, repeatable staging gate for workflow-nodes after fixture/native runtime and adapter parity are complete. It prevents missing staging configuration from being interpreted as readiness while moving Phase 4 toward staging evidence without production risk.
...
Proceed to `phase_4ck_tasks_staging_readiness_bundle` for `/api/admin/automation-conversion/tasks*`, keeping staging smoke behavior disabled by default with blocked evidence when staging config is absent.
