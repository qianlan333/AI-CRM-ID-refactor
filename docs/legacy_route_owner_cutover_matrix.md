...
| D8 | Flask factory/http registrar and maintenance commands | `aicrm_next.main` default runtime remains active | `legacy_flask_app.py` plus `wecom_ability_service/` explicit legacy fallback; no `legacy_flask/` archive package on current main | lockdown_preflight_ready | D8.2 preflight only; no runtime guard, shell deletion, command deletion, DB migration execution, or production cutover |
| D9 | OpenClaw legacy adapter | `aicrm_next.integration_gateway` D7.7 fake/staging-disabled contracts | repo-side `openclaw_service/` path absent after D9.6 | openclaw_shim_physical_deletion_recorded | no reintroduction of the deleted repo shim/archive paths; `openclaw-wecom-postgres.service` remains only a DB/env service |
