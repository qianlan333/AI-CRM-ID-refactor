...
D4 legacy retirement note: `historical retired wecom_ability_service reference (admin_user_ops.py)` is no longer a current route-owner file. AI-CRM Next owns the User Ops readonly surface; the legacy domain helpers below remain relevant only for DND, batch-send, deferred jobs, and other write/external fallback paths.

建议先读：

1. [`wecom_ability_service/templates/admin_user_ops.html`](aicrm_next/frontend_compat/templates/admin_user_ops.html)
2. [`aicrm_next/ops_enrollment`](../aicrm_next/ops_enrollment)
3. [`wecom_ability_service/domains/user_ops/page_service.py`](#historical-reference)
4. [`wecom_ability_service/services.py`](#historical-reference)
