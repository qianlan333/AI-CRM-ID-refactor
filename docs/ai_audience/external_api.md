...
- `AICRM_AI_AUDIENCE_SPEC_ALLOW_NON_VERIFY_PREFIX=false`

默认只允许 `prod_verify_` 前缀，不允许 publish。
...
curl -sS -X POST https://www.youcangogogo.com/api/external/ai-audience/spec/dry-run \
  ...
  -d '{"spec_markdown":"...","package_key_prefix":"prod_verify_"}'
```
...
curl -sS -X POST https://www.youcangogogo.com/api/external/ai-audience/spec/apply \
  -H "Authorization: Bearer $AICRM_AI_AUDIENCE_SPEC_API_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"spec_markdown":"...","package_key_prefix":"prod_verify_","publish":false,"operator":"codex"}'
```
...
curl -sS -X POST https://www.youcangogogo.com/api/external/ai-audience/spec/publish \
  -H "Authorization: Bearer $AICRM_AI_AUDIENCE_SPEC_API_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"package_key":"prod_verify_q101_submitted_added_wecom","version_id":123,"operator":"codex"}'
```
...
```bash
curl -sS -X POST https://www.youcangogogo.com/api/external/ai-audience/packages/prod_verify_q101_submitted_added_wecom/archive \
  -H "Authorization: Bearer $AICRM_AI_AUDIENCE_SPEC_API_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"operator":"codex"}'
```
